# Requirements: OmniRepo v1.0 — Archived

**Defined:** 2026-04-14
**Shipped:** 2026-04-17
**Status:** ✅ All 175 requirements delivered.
**Milestone archive:** `milestones/v1.0-ROADMAP.md`
...
The source `.planning/REQUIREMENTS.md` carried 35 unticked checkboxes at
audit time despite the code implementing every item — the checkbox-update
step was skipped at several phase closes. During milestone archival all
boxes were re-ticked after spot-checking each against shipping code. No
requirement was dropped. No requirement was adjusted.
...
- [x] **REPO-05**: Project members can edit per-repo description (markdown README), `auto_scan` flag, `block_on_severity` enum, and `public_read` flag
- [x] **REPO-06**: Project members can soft-delete a repo; the repo and its contents move to trash with retention
- [x] **REPO-07**: Project members can wipe a repo's contents (delete all artifacts and metadata rows; keep the repo row); audit event emitted
...
- [x] **REPO-09**: `public_read` flag, when true, allows anonymous GET requests to the repo's read paths; writes and deletes still require auth and project membership

### OCI / Docker Protocol (OCI)

- [x] **OCI-01**: Docker registry served at `/v2/<project>/<repo>/<image>/...` per OCI Distribution spec
- [x] **OCI-02**: `docker login` flow returns `401 WWW-Authenticate: Bearer realm="<host>/v2/token",service="omnirepo"`; client exchanges Basic creds at `/v2/token` for a short-lived HMAC-JWT (default 60 minutes, configurable in `settings`)
- [x] **OCI-03**: Docker push supports chunked blob upload (POST → PATCH ... → PUT), monolithic upload (POST + body), and cross-repo blob mount (`?from=<repo>`)
- [x] **OCI-04**: Docker pull supports manifest GET (with `Accept` content negotiation for OCI/Docker manifest list types), blob GET (range supported), `HEAD` for manifest and blob existence
- [x] **OCI-05**: Tag list, tag delete, manifest delete, and `/v2/_catalog` (scoped per-project, gated by membership) implemented
- [x] **OCI-06**: Each blob digest is stored once in the CAS; per-repo `docker_manifests` rows reference the shared blobs; `docker_blobs.ref_count` is incremented in the same transaction as a manifest insert and decremented on delete
- [x] **OCI-07**: `docker-content-digest` response header is set on every applicable response
- [x] **OCI-08**: Project members can pull a Docker image from an external registry into one of their repos with optional retag (e.g. `docker.io/nginx:1.25` → `dxc/oracle/nginx:1.25-internal`); supports anonymous and Basic-auth upstream
- [x] **OCI-09**: Project members can promote (retag) a Docker image from one local Docker repo to another in the same or another project they belong to; no blob copy
- [x] **OCI-10**: Cosign signature verification displays a signed/unsigned badge on tag list views (read-only; OmniRepo does not sign)
...
- [x] **RAW-01**: RAW repo served at `/<project>/raw/<repo>/<path>` as a pass-through file store
- [x] **RAW-02**: Project members can upload arbitrary files at any subpath via REST API or UI dropzone
- [x] **RAW-03**: GET returns the file with correct `Content-Type` (best-effort via `mime.TypeByExtension` plus magic-number fallback) and `Content-Length`
- [x] **RAW-04**: PUT to an existing path overwrites atomically; DELETE removes the file
- [x] **RAW-05**: Directory listings (HTML or JSON via `Accept`) returned for collection paths

### Sync Jobs (SYNC)

- [x] **SYNC-01**: System runs an in-process job dispatcher with two pools: a sync pool (default 4 workers) and a scan pool (default 2 workers)
- [x] **SYNC-02**: Job rows live in `sync_jobs` and `scans` tables with `status`, `attempts`, `last_error`, `next_run_at` columns; dispatcher leases pending rows and updates status atomically
- [x] **SYNC-03**: On boot, any `running` job older than 10 minutes is reset to `pending` for retry
- [x] **SYNC-04**: Failed jobs retry with exponential backoff (1m, 5m, 30m, fail at 5 attempts)
...
- [x] **SCAN-01**: System invokes Trivy as a subprocess for every scan job; Trivy binary and DB are bundled in the container image
- [x] **SCAN-02**: Trivy is always invoked with `--cache-dir /var/lib/omnirepo/trivy/cache --offline-scan --skip-db-update` (no automatic network)
- [x] **SCAN-03**: System auto-scans on upload by default (per-repo `auto_scan` toggle, default on)
- [x] **SCAN-04**: Project members can manually rescan any artifact via UI button or REST endpoint
- [x] **SCAN-05**: Docker scans materialize the manifest + layers into a temporary OCI layout under `tmp/` and run `trivy image --input <dir>`; RPM/DEB/PyPI/RAW use `trivy rootfs` or `trivy fs` against an extracted tmp tree
- [x] **SCAN-06**: Scan results are parsed via tolerant JSON decoding and inserted into `scans` and `vulnerabilities` rows; snapshot tests guard the parser against Trivy schema drift
- [x] **SCAN-07**: Per-repo `block_on_severity` enum (`none|low|medium|high|critical`) blocks downloads of artifacts whose latest scan reports findings at or above the threshold; clients receive `403` with a clear message; `none` = advisory-only
- [x] **SCAN-08**: System generates SBOM (CycloneDX by default; SPDX selectable) for Docker images and language packages on demand and stores them under `/var/lib/omnirepo/sboms/<scan-id>.json`
...
- [x] **SCAN-12**: System tracks `blob_uploads` in-flight registry; GC excludes any digest in `blob_uploads` and any digest with `last_touched_at < now - 1h`
...
- [x] **OPS-06**: Super-admin can trigger garbage collection (mark + sweep); orphan blobs and trash entries past retention are hard-deleted; CAS refcounts cross-checked
...
- [x] **SRCH-01**: System maintains FTS5 virtual tables: `repos_fts(name, description_md, project_name)`, `artifacts_fts(repo_id, artifact_name, version, tags)`, `cves_fts(cve_id, package, title, description)`
- [x] **SRCH-02**: FTS rows are inserted/updated/deleted in the same transaction as the underlying entity (no out-of-band reindex)
...
- [x] **TEST-02**: Per-protocol conformance tests boot the full app on a random port and exercise it with the real client: `crane` (OCI), `aws-sdk-go-v2` (S3), `dnf` (RPM, DinD), `apt-get` (APT, DinD), `pip` and `uv pip` (PyPI), `helm` (Helm), `git` CLI (Git)
