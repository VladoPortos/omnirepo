...
# Go package set for the unit-test target. The `go` tool's `./...` does NOT
# skip directories named node_modules, so once `make frontend` has run, a
# vendored frontend dependency that ships a .go file (flatted/golang) gets
# pulled into `go test ./...`, making Go CI depend on npm package contents.
# Filter node_modules out explicitly so Go and frontend toolchains stay
# decoupled. Evaluated once at parse time; harmless before node_modules exists.
GO_TEST_PKGS := $(shell $(GO) list -mod=vendor ./... 2>/dev/null | grep -v '/node_modules/')
...
test: lint-protocol-redaction check-contrast lint-typography lint-spacing-carveout lint-axe-devdep lint-reset-beforeEach lint-docs lint-go-vet-oci
	$(GO) test -mod=vendor $(GO_TEST_PKGS)
	$(MAKE) test-airgap

test-airgap:
	$(GO) test -mod=vendor ./test/airgap/...

# test-perf: runs the build-tagged perf500 suite with
...
# test-live-oci: live E2E against
...
# opt-in target. Mirrors the test-perf / perf500 pattern.
...
# cover the full Handle round-trip).
test-live-oci:
	@set -e; \
...
# test-live-git: live E2E mirror of a real public GitHub
# HTTPS repo via git.SyncHandler. Default upstream is
...
#
# NOT a prerequisite of `test` — live endpoints belong behind an
# opt-in target. Mirrors the test-perf and test-live-oci pattern.
# See internal/protocol/git/sync_live_test.go
# for the test body and the scope-guard rationale (single E2E
# scenario; hermetic tests cover correctness).
test-live-git:
...
# Deterministic 200 MB bare-repo fixture for the git memory
...
# Git clone memory benchmark. Launches omnirepo as a child
...
# The earlier `grep-cdn` static-text gate was retired in v1.2 —
...
# lint-protocol-redaction enforces error-redaction for the protocol handler tree:
...
lint-protocol-redaction:
	...
	if [ -n "$$matches" ]; then \
		echo "ERROR: protocol handler emits %v-interpolated error to client (leak):"; \
		echo "$$matches"; \
		echo ""; \
...
# DinD conformance gates. Each target drives a real
...
# Cross-protocol denial conformance for
...
# Bench throughput: upload + API throughput benchmarks
bench-throughput:
	$(GO) test -tags=bench -mod=vendor -count=1 -timeout=10m -bench=. ./test/bench/throughput/...

# Bench target: run all benchmarks
bench: bench-sqlite bench-git bench-throughput
	@echo "All benchmarks complete"

# ---------------------------------------------------------------------------
# Visual-foundation gates
# ---------------------------------------------------------------------------

# check-contrast: parses web/src/index.css :root, extracts the
...
# lint-typography: grep gate that fails any new file under
...
# text-4xl). The typography scale is exactly 2 weights (400 regular,
# 600 semibold) and exactly 4 sizes (text-xs, text-sm, text-lg, text-2xl).
#
# scripts/typography-allowlist.txt enumerates older files that
# legitimately contain these classes (shadcn primitives, v1.0 pages);
# they are excluded from the grep. New files MUST NOT appear in the
# allowlist.
lint-typography:
	@set -e; \
	...
	if [ -n "$$weight_hits" ]; then \
		echo "ERROR: forbidden font-weight class in new code (typography discipline):"; \
		echo "$$weight_hits"; \
		echo ""; \
		echo "Allowed weights: default (400 regular) or font-semibold (600)."; \
		echo "Allowed sizes:   text-xs, text-sm, text-lg, text-2xl."; \
		echo "If a file legitimately uses a forbidden class,"; \
		echo "add its basename to scripts/typography-allowlist.txt."; \
		exit 1; \
	fi; \
	if [ -n "$$size_hits" ]; then \
		echo "ERROR: forbidden text-size class in new code (typography discipline):"; \
		echo "$$size_hits"; \
		echo ""; \
		echo "Allowed sizes: text-xs, text-sm, text-lg, text-2xl."; \
		echo "Add its basename to scripts/typography-allowlist.txt if it legitimately uses one."; \
		exit 1; \
	fi; \
	echo "lint-typography: clean"

# lint-reset-beforeEach: every Playwright spec under
...
#
# Rationale: POST /api/v1/admin/_reset backs the resetServerState() helper,
# rolled out across all specs. This gate prevents future specs from
# silently regressing to the shared-state flake pattern.
lint-reset-beforeEach:
	@set -e; \
	...
	if [ -n "$$missing" ]; then \
		...
		echo "Import from './helpers/auth'."; \
		exit 1; \
	fi; \
	echo "lint-reset-beforeEach: clean"

# lint-go-vet-oci: narrow go vet gate for the
...
# deferred. Lifts to wider subtrees as future cleanup lands.
lint-go-vet-oci:
	@$(GO) vet ./internal/protocol/oci/...
	@echo "lint-go-vet-oci: clean"

# lint-spacing-carveout: the 6px copy-button inset (right-1.5 /
# top-1.5) was originally grandfathered to the v1.0 SnippetPanel and
# OneTimeReveal files as a spacing exception. A later refactor
...
# chrome (generated by the shadcn CLI) and
...
lint-spacing-carveout:
	...
	if [ -n "$$hits" ]; then \
		...
		echo "$$hits"; \
		echo ""; \
		echo "New copy-button placements MUST use 8px (right-2 top-2)."; \
		echo "The 6px carve-out is limited to OneTimeReveal.tsx and the"; \
		echo "shadcn-generated ui/sidebar.tsx (both v1.0)."; \
		exit 1; \
	fi; \
...
# lint-docs: shellcheck the canonical
...
# Linting:    default severity bar (error + warning) — NO `-S`
...
# with a skip note when absent so `make test` stays green on
