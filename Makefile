...
# top-1.5) was grandfathered to the v1.0 SnippetPanel and OneTimeReveal files
# per 06-UI-SPEC §Spacing Exceptions. The shadcn ui/sidebar.tsx primitive also
# ships it as generated chrome and is grandfathered (generated by the shadcn
# CLI in plan 05-02, predates Phase 6). New copy-button placements MUST use
# 8px (right-2 top-2) per UI-SPEC.
lint-spacing-carveout:
	@set -e; \
	echo "lint-spacing-carveout: 6px inset outside SnippetPanel/OneTimeReveal/sidebar"; \
	hits=$$(grep -rnE --include='*.tsx' --include='*.ts' \
		--exclude='SnippetPanel.tsx' --exclude='OneTimeReveal.tsx' --exclude='sidebar.tsx' \
		'\b(right-1\.5|top-1\.5)\b' web/src/ 2>/dev/null || true); \
	if [ -n "$$hits" ]; then \
		echo "ERROR: 6px inset (right-1.5 / top-1.5) found outside the three grandfathered files:"; \
		echo "$$hits"; \
		echo ""; \
		echo "Phase 6+ new copy-button placements MUST use 8px (right-2 top-2)."; \
		echo "The 6px carve-out is limited to SnippetPanel.tsx, OneTimeReveal.tsx,"; \
		echo "and the shadcn-generated ui/sidebar.tsx (all v1.0 pre-Phase-6)."; \
		exit 1; \
	fi; \
