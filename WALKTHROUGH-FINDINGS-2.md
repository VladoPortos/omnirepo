...
- **F-16 (R, real-issue) FIXED** `internal/api/git_browse.go` — chi routes `/tree/{ref}/*`, `/blob/{ref}/*`, `/commits/{ref}`, `/blame/{ref}/*` captured `{ref}` as a single path segment, so branch/tag names with `/` (e.g. `feature/x`, `release/v1.2`) resolved to ref=first-segment and 404'd. Fix: converted those four routes to catch-all (`/tree/*` etc.) and added `splitRefAndPath()` which iterates possible ref prefixes from longest to shortest and returns the first one that resolves via `resolveRef`. Commits-style routes use `refOnlyFromCatchAll` (no trailing path). Added `TestGitBrowse_RefWithSlash` covering `/tree`, `/blob`, `/commits` against both `refs/heads/feature/x` and `refs/tags/release/v1.2` — all PASS. Live retest: pushed `feature/x` to walkthru2/git/tools; `GET /api/v1/projects/walkthru2/repos/git/tools/tree/feature/x` → 200 with `[README.md, feature.md]`; `/blob/feature/x/feature.md` → 200; `/commits/feature/x` → 200 with the `F-16 feature/x branch` commit. Clone/push Smart-HTTP (go-git backend) unaffected.
- **F-11 (m, minor) FIXED** Breadcrumb on NotFoundPage now renders every segment as a non-link `BreadcrumbPage` when the current route is the nested not-found catch-all. Implementation: `id: 'not-found'` tag on the `path: '*'` element in App.tsx + `useMatches()` check in Breadcrumbs.tsx. Live retest on `/projects/walkthru2/repos/helm/charts` (invalid URL) shows all 5 segments now non-clickable (`hasLink=false, hasPage=true`) except the root Dashboard link.
- **F-15 (m, minor) FIXED END-TO-END** Trash listing now surfaces all three UI-expected fields: `original_location` (existing sidecar), `deleted_by` (new: `storage.TrashEntry.DeletedByUser` persisted in sidecar; `Trash.Move` signature gained an `actor string` arg, all 7 callers updated to pass `auth.ActorLoginFromContext(ctx)`), and `retention_countdown` (computed in `admin_trash.go` against the default 7-day GC window via `formatRetention`). New `auth.ActorLoginFromContext` helper keeps protocol packages off the auth.Actor struct. Tests: `TestTrashMoveRestore` asserts actor persists through sidecar round-trip; `TestAdminTrash_SoftDeleteShowsInList` asserts `deleted_by="root"` + positive `retention_countdown`. Live retest: fresh RAW-file delete now reports `deleted_by:"admin"`, `retention_countdown:"6d 23h"`, `original_location:"/var/lib/omnirepo/repos/walkthru2/raw/files/f15-test.txt"`.

## Session summary — 2026-04-21 (final)
- Commits added this session: 10 (F-13 `d73dd6b`, F-12 `418dd6e`, F-14 `104b12a`, F-15 partial `94a50a6`, F-11 `934ffee`, F-15 rest `549bd3b`, F-16 `bd96030`, + 3 docs).
- Running total since v1.2 tag `f4081c7`: **37 commits ahead of main, not pushed.**
- Test gates: `go test ./internal/...` 32/32 packages clean, `npx tsc --noEmit` clean, `npx vitest run` 94/94, `PRAGMA foreign_key_check` empty on live DB.
- Walkthrough §0-§9 fully complete; every finding either fixed inline or rolled into a prior-session commit. No remaining deferred items.
- Smoke container `omnirepo-smoke` running on :18080 / :18443 with /tmp/omnirepo-smoke-data persistent volume; healthy after last rebuild.
- Ready for v1.3 tag + push pending user approval.
