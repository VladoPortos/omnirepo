...
- **F-15 (m, minor)** Trash row shows placeholder-generated Name `1776762004-raw-file-16` (epoch+kind+id) and empty "Original Location" / "Deleted By" / "Retention" columns. The underlying DB has the original path + deleter; trash emission (or the list handler) is dropping them when inserting for RAW files. Fix direction: ensure `admin_trash.go` projects `details_json.original_path`, `actor_login`, and `retention_until` into the listing response.
