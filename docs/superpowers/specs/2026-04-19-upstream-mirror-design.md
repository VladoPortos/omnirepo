...
| M6: Integration tests (fake upstreams × 5) + Playwright e2e + Codex rescue pass | Mixed | 1.5 days |
...
### Milestone M6 — Integration tests, Playwright e2e, Codex rescue  ☐ not started

**Goal:** full test coverage + rescue pass per CLAUDE.md rule.
...
- [ ] **M6.10** Codex rescue: invoke
      `Agent(subagent_type="codex:codex-rescue", ...)` per CLAUDE.md
      global rule. Prompt includes: mirror feature summary, correctness
      questions (idempotency, leakage, race around sync_jobs, cred
      decryption paths), any leftover REVIEW findings. Apply valid findings,
      discard noise.
- [ ] **M6.11** Commit: `test(mirror): integration, e2e, codex findings applied`.
- [ ] **M6.12** Phase SUMMARY.md written, STATE.md bumped, ROADMAP.md phase
      marked complete.
