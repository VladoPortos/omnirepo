...
| F-08.2 | **B** | `internal/scan/parse.go` | Trivy `--scanners vuln,secret,misconfig` runs for every Helm scan, but `trivyReportBlock` only decoded `Vulnerabilities` — every misconfiguration finding was silently dropped. Helm scans summarized all zeros, so `repos.block_on_severity` never had a non-zero severity to compare against — gate effectively a no-op for Helm. Fix: parse `Misconfigurations`, bucket FAIL status into Summary by severity, fold each finding into `Vulnerabilities` with `CVEID="KSV-####"` + `Package=block.Target`. | `14773c7` | ⬜ Pending | ✅ Passed (`TestParseTrivyMisconfigurationsCountInSummary` + `TestParseTrivyVulnsPlusMisconfigsAdditive`; vulny-0.1.0.tgz rescans as `{high:16,medium:10,low:24}`; pull@gate=high → 403 blocked_by_scan; clean chart → 200) | ✅ Closed (Codex pending) |
