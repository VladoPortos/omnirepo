...
| F-05.3 | R latent | scan aggregation vs OCI image index | Tag pointing at multi-arch image index shows `Not scanned` forever — child manifests ARE scanned, but UI queries `scans.artifact_id = tag.digest` and the index digest has no row. All Docker Hub official images are multi-arch → universally broken user experience. Single-arch push works (verified). | _follow-up_ | ✅ Confirmed real-issue | — | 🟨 Open |
| F-05.4 | R | `web/src/pages/repo/DockerRepoPage.tsx:308` | Delete-tag icon button has **no onClick handler** — clicking does nothing. OCI DELETE `/v2/.../manifests/<ref>` backend works (verified via `crane delete`). Pure frontend-wiring gap. | _follow-up_ | ✅ Confirmed real-issue | — | 🟨 Open |
...
| F-05.6 | R | `DockerRepoPage.tsx:473` | Promote/Retag button only toasts "API not yet connected." — backend route `POST /api/v1/projects/{name}/repos/docker/{repo}/promote` is fully implemented. Same shape as F-05.4. Siblings flagged (`RpmRepoPage` Sync, `AptRepoPage` Sync, `RepoPageLayout` Wipe) for later batches. | _follow-up_ | ✅ Confirmed real-issue | — | 🟨 Open |
