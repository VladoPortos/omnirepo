...
| F-06.1 | R | `internal/protocol/rpm/put.go` | NEVRA-filename check promised by step-5 doc comment never implemented. Uploads with mismatched filename publish `primary.xml <location href>` pointing at the canonicalFilename() NEVRA, but the on-disk key uses the URL name → dnf 404s every download. Fix: compare `parsed.canonicalFilename()` against `res.filename`, reject 400 `filename_mismatch` + audit `rpm.upload rejected reason=filename_nevra_mismatch`. | `563a7ee` | ⬜ Pending | ✅ Passed (`TestRPMPut_RejectsFilenameNEVRAMismatch` + curl 400 + canonical 201) | 🟨 Open |
| F-06.2 | m | `web/src/lib/snippets.ts` | Every snippet hard-coded `https://` regardless of served scheme — copy-paste-and-run failed on HTTP-served UIs. Fix: optional `scheme` param (default `'https'`); SnippetList reads `window.location.protocol` and passes matching value. | `9f860f3` | ⬜ Pending | ✅ Passed (HTTP UI renders `baseurl=http://…`) | 🟨 Open |
...
| F-06.4 | m | `RpmRepoPage.tsx:289` + `AptRepoPage.tsx:343` | Mirror empty state said "Upload your first artifact using the snippet below" — but mirror uploads 403 and the snippet is pull-only. Fix: is_mirror-aware copy — title "Mirror not yet synced" + sync-hint description. | `dd982e2` | ⬜ Pending | ✅ Passed (`/empty-mirror-check` renders sync-hint copy) | 🟨 Open |
| F-06.5 | R | `internal/api/sync_actions.go` SyncActorBridge | F-05.1's 11th site: user-owned API keys dropped the owning-user id on the bridge → `POST .../sync` with `-u alice:<user-key>` 403'd "not a project member" for every CI / cron / script caller. Fix: when `ProjectScope==nil && OwnerKind==user`, set `UserID = a.ID` (matches `auth.ResolveMembership` shape). | `2b12a70` | ⬜ Pending | ✅ Passed (`TestSyncActorBridge_*` + curl 202) | 🟨 Open |
| F-06.6 | R | `web/src/hooks/useJobProgress.ts` | Two polling bugs: (a) pending+attempts>=1+last_error showed "Preparing…" with no error pill for up to 96 min while the runner backed off retries; (b) repo-delete mid-sync left the hook polling `/sync-jobs/{id}` → 404 every 500 ms forever (100+ errors observed). Fix: `computeJobProgress` wraps retry-backoff as `transient/job.retrying`; `pollingDecision` halts on 4xx (5xx still retries); hook's TanStack `retry` returns false on 4xx. | `f487e2e` | ⬜ Pending | ✅ Passed (5 new unit tests; Playwright retest deferred to batch close) | 🟨 Open |
| F-06.7 | m | `web/src/lib/snippets.ts` (rpm) | `dnf config` snippet used `gpgcheck=1` → dnf rejected every non-OmniRepo-signed package with "wrong key(s)?". OmniRepo signs repomd, not packages (pass-through mirror). Fix: emit `repo_gpgcheck=1 gpgcheck=0` — verify the signed index, trust the packages. | `9f860f3` | ⬜ Pending | ✅ Passed (dnf reaches transaction-test end-to-end) | 🟨 Open |
