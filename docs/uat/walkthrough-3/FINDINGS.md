...
| F-10.0 | n | batch-10 test text | Test 10.1 says "CreateRepoDialog for Git in non-mirror mode shows NO mirror checkbox" — contradicts commit 4170f65 (Git in MIRROR_PROTOCOLS). Actual invariant: checkbox shown; config fields only when ticked. Doc-only note. | _no fix_ | — | ✅ Accepted | ✅ Closed |
| F-10.1 | R | `internal/httpx/middleware_logger.go:27` | `http.request` slog records always emitted `actor_id=""` with stale "Phase 2 auth middleware fills it in" TODO. Fix: `auth.LoginBox` (mutable per-request holder) + `httpx.LoginBoxSeeder` callback (breaks auth → httpx cycle). `StructuredLogger` seeds box on ctx; `auth.WithActor` auto-populates; logger reads after next.ServeHTTP. | `246a262` | ✅ Clean (Q1 noise) | ✅ Passed (`actor_id:"alice"` / `""` for anon in server.log) | ✅ Closed |
| F-10.2 | R | `internal/api/projects_full.go:72` | Git item_count badge counted symbolic HEAD row (already filtered by `/refs` endpoint) — UI showed "2 refs" for 1-branch repo, "4 refs" for 2 branches + 1 tag. Fix: narrow SQL to `type IN ('branch','tag')` (Codex minor follow-up: future-proof over initial `<> 'symbolic'`). | `246a262` + `a11a736` | ✅ Clean | ✅ Passed (UI header shows "3 refs" correctly) | ✅ Closed |
| F-10.5 | **B** | `internal/api/git_browse.go` × 6 handlers | Backend emitted shapes that violated `openapi.yaml` schemas the TS client was typed against — Files table rendered Size="--", file clicks no-op'd, Commits tab crashed on undefined `author_name.split`. Rewrote handleGitTree/Blob/Commits/Commit/Blame/Compare to canonical GitTreeEntry / GitFileContent / GitCommit / GitDiff / GitBlame shapes. Added `diffTrees`/`changePatch`/`parentSHAs` helpers. Accept both `..` and `...` compare spec; reject 4-dot (Codex Q4). | `246a262` + `a11a736` | ✅ Clean (Q4 noise on main path; minor 4-dot fixed in follow-up) | ✅ Passed (Playwright: blob viewer renders, commits list + detail diff render, zero console errors) | ✅ Closed |
| F-10.7 | **B** | `internal/protocol/git/middleware.go` + `internal/auth/policy.go` | public_read=true didn't enable anonymous git clone. Two-part root cause + one Codex-found info leak: (a) middleware rejected Actor-less requests with 401 before auth.Can; (b) policy anonymous short-circuit only allowed `ActionRepoRead`, not `ActionGitRepoRead`; (c) **Codex real-issue:** ResolveRepoFromURL 404'd missing repos while auth returned 401 for private → anonymous could enumerate via status sniffing. Fix: new `AnonymousGitRead` middleware + chain reorder; policy extended; `writeMissingOrChallenge` normalizes missing+private to 401 for anonymous callers. | `246a262` + `a11a736` | ✅ Real-issue (info leak) + 1 minor (4-dot) flagged + fixed | ✅ Passed (anon clone on public ✓, private 401 ✓, missing 401 for anon / 404 for authed ✓, auth clone ✓) | ✅ Closed |
| F-11.1 | **B** | `internal/api/mirror_validate.go` + `web/src/components/CreateRepoDialog.tsx` | Creating a git mirror via UI or REST returned 400 "mirror_filter JSON does not match the protocol SyncFilter shape". `validateMirrorFilter` switch covered deb/rpm/pypi/helm with default-reject; git (in `mirrorSupportedTypes`) had no case. Frontend always posted `mirror_filter: {}`. Fix: frontend omits the field for `repoType==='git'` (no filter API per GITMIRROR-01); backend adds `case "git":` accepting only `{}` / empty. | `f877e91` | ✅ Clean (Q1 noise) | ✅ Passed (UI + curl create 200) | ✅ Closed |
| F-11.2 | R | symptom of F-11.3 | Pre-sync clone/info/refs returned 200 empty advertisement instead of the designed 503 `mirror.not_yet_synced` envelope. Root cause was F-11.3 (HEAD file existed because InitBare ran for mirrors). Resolved by F-11.3 fix. | `f877e91` | ✅ Clean | ✅ Passed (pre-sync clone → 503 envelope) | ✅ Closed |
| F-11.3 | R | `internal/app/app.go` composedRepoCreateHook git block | `*git.Handler.OnRepoCreate` in `reporepo.go` had the correct is_mirror short-circuit (commit 7920876) but was never registered as the hook. The live path was `composedRepoCreateHook` with an inline InitBare block that unconditionally created the bare skeleton. Mirrors on disk were fully initialised at create time → `mirror.not_yet_synced` 503 guard never fired; first `PlainCloneContext` fought `ErrTargetDirNotEmpty`. Fix: port the is_mirror check (reading under the writer tx for WAL visibility) into the inline block; non-mirror path preserved verbatim. Codex flagged the orphaned OnRepoCreate as dead code — deferred cleanup to batch-15. | `f877e91` | ✅ Clean (Q2, Q3, Q4 noise) | ✅ Passed (mirror create leaves no `.git/` skel; first sync populates cleanly) | ✅ Closed |
| F-11.4 | R | `internal/storage/trash.go` + `internal/api/admin_trash.go` | Deleting an unsynced git mirror silently swallowed `os.ErrNotExist` from Trash.Move (no on-disk tree), creating no trash sidecar. Admin trash list had no entry, and POST /admin/trash/{id}/restore could not target the orphaned-soft-deleted DB row. Fix: Trash.Move now writes a metadata-only sidecar (`trashMetadata.Empty=true`) and returns the sentinel so callers can still distinguish "nothing moved" from "moved N bytes". handleRestoreTrash branches on e.Empty — restores the DB row + cleans the holder dir, no rename attempt. admin_phase1 continues treating ErrNotExist as success. | `1688974` | ✅ Codex-flagged + fixed | ✅ Passed (create empty-git-mirror → delete pre-sync → trash lists → restore 200 → upstream_url preserved) | ✅ Closed |
| F-11.5 | n | `web/e2e/create-repo-git-mirror.spec.ts:206` | Stale e2e assertion: `expect(body.mirror_filter).toEqual({})` would fail after F-11.1's frontend change (field now omitted). Update to `toBeUndefined()`. | `1688974` | ✅ Codex-flagged + fixed | ✅ Accepted | ✅ Closed |
| F-12.1 | R | `internal/protocol/raw/handler.go` validateRawPath | `%2e%2e` URL-encoded traversal passed validation because chi keeps wildcard params undecoded — file landed on disk at `repos/acme/raw/files/foo/%2e%2e/%2e%2e/outside.txt`. No filesystem escape (filepath.Join doesn't URL-decode) but the validator's "reject traversal" contract was violated and the Location header decoded the `%2e%2e` to `..`, creating a confusing mismatch with the stored name. Fix: strict-mode validateRawPath now percent-decodes each segment and rejects if the decoded form is `.`, `..`, `""`, NUL-carrying, or has malformed `%xx`. Strict only applies on PUT so GET/HEAD/DELETE of any already-stored `%2e%2e` rows keep working (backward-compat per Codex Q3). | pending | ✅ Real-issue flagged by Codex + fix applied | ✅ Passed (strict PUT→400 for `%2e%2e`; lenient GET→200 and DELETE→204 on a simulated legacy row) | ✅ Closed |
| F-12.2 | R | `internal/protocol/raw/handler.go` validateRawPath | 4500-character filename returned 500 "internal" because `PathStore.Put`'s rename surfaced ENAMETOOLONG to the error handler. Test plan 12.6 requires a clean 4xx. Fix: validateRawPath now caps per-segment at 255 bytes and total rel-path at 1024 bytes (well under Linux NAME_MAX / PATH_MAX), returning 400 "invalid path" up-front. Length cap applies on reads too so a 5000-char GET also 400s rather than stat'ing the FS. | pending | ✅ Clean | ✅ Passed (300-char PUT → 400; normal PUT → 201) | ✅ Closed |
| F-12.3 | n | raw protocol error shape | raw GET/HEAD/DELETE return plain-text `"not found"` / `"invalid path"` / `"internal"` instead of the project's JSON error envelope. By architectural decision — `internal/httperr/envelope.go:4` states "Protocol handlers in internal/protocol/{oci,rpm,deb,pypi,helm,raw,s3,git} do NOT use this package — they emit protocol-native error shapes." Codex confirmed as by-design. | _no fix_ | ✅ Codex confirmed by-design | ✅ Accepted | ✅ Closed |
| F-12.4 | m | `internal/protocol/raw/put.go:79` detectMIMEFromExt | PUT with explicit `Content-Type: application/xml` was overridden to stored MIME `text/xml; charset=utf-8` (ext-sniff). Test plan 12.3 wording "(or sniffed correctly)" allows current behaviour; but from a user POV the client-supplied C-T silently vanishing is surprising. **Deferred — batch-15 cross-cutting** (needs a whitelist + XSS story: user could upload HTML with `Content-Type: image/png` and later serve it back as HTML). | _deferred_ | ✅ Codex confirmed minor | — | 🟨 Open (deferred) |
| F-12.5 | n | batch-12 test plan | Spec says audit event name is `raw.upload`, code emits `raw.put`. Test plan drift — code is self-consistent, `raw.put` is the correct verb. Update test plan 12.2/12.5 to match. | _no fix_ | — | ✅ Accepted | ✅ Closed |
| F-12.S1 | n | batch-12 test plan 12.12 | Spec says `aws --endpoint-url http://localhost:18080 s3 ...`, actual implementation requires `/s3` prefix (`internal/protocol/s3/handler.go:49`). `aws` against the bare root hits the SPA catch-all and gets back HTML with HTTP 200, which the AWS SDK then surfaces as "Internal Server Error" — confusing but not a bug in OmniRepo. Update test plan to `http://localhost:18080/s3`. | _no fix_ | ✅ Codex confirmed by-design | ✅ Accepted | ✅ Closed |
| F-12.S2 | m | `web/src/pages/ProfilePage.tsx` Create S3 Key dialog | Dialog shows only a Project combobox — no Label input — but the DB schema has `s3_access_keys.label NOT NULL` and W2 §3f spec called out "per-project unique label". Backend auto-fills `"Profile-created YYYY-MM-DD"`. Harmless default but UI no longer lets users distinguish multiple keys in the same project. **Deferred — batch-15 cross-cutting.** | _deferred_ | — | — | 🟨 Open (deferred) |
| F-12.S3 | n | `DELETE /api/v1/me/s3-keys/{id}` | The path-param `{id}` is the numeric row id, not the AKID string. Passing an AKID like `AKIA4T73IUOLDKLWDOXD` 400s with a non-descriptive envelope. OpenAPI spec matches the numeric-id design; curl-by-AKID returning 400 is correct behavior, but the error message could mention `id` must be numeric. Test-plan wording "Revoke from UI" is the intended path. Observation only. | _no fix_ | — | ✅ Accepted | ✅ Closed |
