// Package api — admin database health endpoint.
//
...
//     written by internal/metadata.RunBootIntegrityCheck at boot).
...
//     so we never run a runtime `PRAGMA journal_mode` probe.
//
// Never runs `PRAGMA integrity_check` inline — that pragma is a heavy scan
// that would wedge the size-1 writer pool. Manual refresh is the
// POST /admin/db/health/check endpoint, which appends to the
// same mountAdminDBHealth router group and shares the dbHealthJob lease
// defined here.
//
// Budget: <100 ms on a 500 MB DB. Fast merge-gate in
...
// Constants pinned to the v1.4 vendor.
const (
	// integrityRateLimitWindow is the 1/hour throttle that
	// the POST /admin/db/health/check endpoint enforces
	// via dbHealthJob.lastRunAt. The GET endpoint reads this to compute
	// `next_available_at` so the UI can server-drive eligibility
	// instead of client-clock-drifting.
	integrityRateLimitWindow = time.Hour

	// walWarnOverBytes is the 100 MB threshold: over this
	// size the UI renders the card in the `warning` variant.
...
// integrity-check manual-refresh flow. The GET handler reads a lease
// snapshot and surfaces running / can_run_now / next_available_at; the
// POST handler appends to the same mount function and mutates state under
// the same mutex.
//
...
type integrityCheckJob struct {
	...
	// manual run. Consumed by the POST handler for 409 `already_running`
	// and 429 `rate_limited` envelopes.
	lastStatus string
}

// dbHealthJob is the process-wide single-flight tracker shared between
// the GET handler (read-only snapshot) and the POST handler
...
func (j *integrityCheckJob) Snapshot() leaseSnapshot {
	...
	// rate-limit window. Server-driven so client clock drift does
...
// readCachedIntegrity loads the three cache rows written by
// RunBootIntegrityCheck + the manual-refresh goroutine. Missing
...
// added later, prefer it over the computed path here, following the
...
// (the source-of-truth) and lowercases it for the response —
...
//
//   - GET  /admin/db/health        — read cached payload
//   - POST /admin/db/health/check  — trigger manual integrity_check
//
...
//
// Never runs `PRAGMA integrity_check`. Never probes
// `PRAGMA journal_mode`. Never takes a writer tx.
func (d Deps) handleDBHealth(w http.ResponseWriter, r *http.Request) {
	...
	if dbPath != "" {
		...
		// WAL sidecar naming convention: "<dbfile>-wal" (per SQLite docs).
		// modernc.org/sqlite honors this without quirks. `-shm` is NOT
		// included — it's a shared-memory index file whose size is not
	...
	// last_manual_run_at: prefer the in-process lease (written by the
	// POST handler) but fall back to the settings row which
	...
	// --- 6. Response assembly. Field names are fixed — the frontend
	// consumes these verbatim. Adding or removing keys here is a schema
...
// -----------------------------------------------------------------------------
// POST /admin/db/health/check — manual-trigger integrity_check
// -----------------------------------------------------------------------------
...
// Production value is metadata.RunIntegrityCheckNow.
//
...
// for the boot-time call; duplicated here so the manual-trigger
...
//
// Flow:
//  1. Acquire dbHealthJob.mu (blocking Lock — the critical section is
...
//     the lease on panic. RunIntegrityCheckNow handles
...
//
// Lifecycle:
//  1. defer recovers panics (the lease must be released even
