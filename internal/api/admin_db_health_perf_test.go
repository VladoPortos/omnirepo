...
// DBHEALTH-07 / SC3 assertion. Invoked via `make test-perf`, not as part
...
func TestAdminDBHealth_PerfBudget_500MB(t *testing.T) {
	const targetBytes = 500 * 1024 * 1024 // 500 MB per DBHEALTH-07 / SC3.
	...
	if p95 > 100*time.Millisecond {
		t.Fatalf("GET /admin/db/health p95 %v exceeds 100 ms budget on 500 MB DB (DBHEALTH-07 / SC3)", p95)
	}
