...
//
// The 500-MB authoritative perf test lives in
...
// merge-gate per the linear-extrapolation argument.
package api_test
...
// TestAdminDBHealth_BootCacheIsPopulated is the cross-package coupling gate
// for the boot-integrity wiring. If
...
//
// Harness shape note: `newTestServer` in this package does
// NOT drive app.Run — it wires api.Deps directly off a sqlitetest DB.
// To avoid silently bypassing the boot wiring, this test explicitly
...
func TestAdminDBHealth_BootCacheIsPopulated(t *testing.T) {
	...
	if checkedAt == "" {
		t.Fatal("db.integrity_check.checked_at empty — boot wiring broken (RunBootIntegrityCheck did not write the cache)")
	}
	if _, err := time.Parse(time.RFC3339, checkedAt); err != nil {
...
//
// Why 10 MB is sufficient as a proxy for the 500 MB
...
//
// The authoritative 500-MB assertion lives in
...
// frontend. If any key name changes, this test breaks —
// forcing the change to go through the interfaces lock.
// -----------------------------------------------------------------------------
