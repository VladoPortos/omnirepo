...
// GET /api/v1/admin/jobs/summary handler tests.
//
// The endpoint is super-admin-gated (RequireCan(ActionTriggerGC), reused
// rather than introducing a new policy action) and returns the 5-key fixed
// shape:
...
// asserts the handler returns the expected shape with the expected counts +
...
func TestAdminJobsSummary_SuperAdmin_ReturnsShape(t *testing.T) {
	...
	// Shape: all 5 keys present (even when values are null).
...
// is rejected at the policy gate (ActionTriggerGC reused).
