...
func newTestServer(t *testing.T, opts ...testServerOpt) *testServer {
	...
	mux := chi.NewRouter()
	// Stamp UUID v7 incident IDs on every request in the test server so
	// integration tests can assert the ApiErrorEnvelope.incident_id ↔
	// X-Incident-Id response header parity. Production wiring lives in
	// internal/httpx.New.
...
func TestLoginTimingOracle(t *testing.T) {
	...
	// argon2 time (not an earlier microsecond LoginValid short-circuit).
...
func TestChangePassword_WrongCurrent(t *testing.T) {
	...
	seedTestUser(t, s.db, "alice", "a@x", false, false)
	cookie, _, _ := s.login(t, "alice", "pw-alice")
	// New must satisfy auth.PasswordValid floor so the wrong-current
	// branch is the one that fires; previously this used New:"x" which
	// now short-circuits at validation.
...
// change-password rejects a weak new password BEFORE verifying current.
// Symmetric with setup; admin force-reset path is covered by api_test.go
// where applicable.
...
// DELETE /me must reject when the actor is the only live super-admin.
// Without this guard the operator can self-delete their way out of the
// air-gapped instance, leaving no path back into /admin/*.
...
// When a SECOND super-admin exists, self-delete of the first is allowed
// (the invariant is "at least one remaining", not "no super-admins ever
// delete").
...
// TestDeleteUser_CannotDeleteSelf pins the safety check: an admin
...
// TestDeleteUser_CannotDeleteLastSuperAdmin pins the safety check:
...
// TestRepoDelete_MissingOnDiskTreeSucceeds is the regression gate
...
func TestMCPRESTReturns403OnEveryMutation(t *testing.T) {
	...
	for _, c := range forbidden {
		...
		// Auth middleware emits the canonical ApiErrorEnvelope with
		// dotted code + class=permission.
...
// TestTLSUpload_HonorsCustomPaths pins that when Deps.TLSCertPath
...
// -----------------------------------------------------------------------------
// Success criteria — simplified end-to-end
// -----------------------------------------------------------------------------
