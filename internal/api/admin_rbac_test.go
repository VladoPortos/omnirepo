...
func TestHandleAddMember_DefaultsToViewer(t *testing.T) {
	s := newTestServer(t)
	// super creates project (super-admin → not auto-added to project_members).
...
func TestHandleMe_ProjectRoles_MemberUser(t *testing.T) {
	...
	// Create two projects (super-admin is not auto-added to project_members).
...
func TestHandleMe_ProjectRoles_SuperAdminEmpty(t *testing.T) {
	...
	// project_roles must be absent or an empty map.
...
// role=viewer emits a member.added audit row with details_json.role == "viewer".
...
// keys: user, old_role, new_role. Missing old_role is a violation.
func TestHandlePatchMember_AuditEmit_RecordsOldAndNewRole(t *testing.T) {
	...
	if details["old_role"] != "maintainer" {
		t.Fatalf("audit member.role_changed: old_role=%v, want 'maintainer'", details["old_role"])
	}
...
// member.added with details_json.role == "viewer" (application default).
func TestHandleAddMember_AuditDefaultRole(t *testing.T) {
	...
	if details["role"] != "viewer" {
		t.Fatalf("audit member.added default: role=%v, want 'viewer'", details["role"])
	}
}
