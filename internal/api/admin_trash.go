// Package api — admin trash management endpoint.
//
...
// soft-deleted repo. File stays in trash; admin hard-deletes or
// re-creates the repo first.
...
// unix timestamp digit. Projects live in the DB, not the filesystem
...
func (d Deps) handleListTrash(w http.ResponseWriter, r *http.Request) {
	...
	// Filesystem trash entries. Surface OriginalPath + DeletedByUser
	...
	// Database-sourced trash: soft-deleted projects. Projects
...
func (d Deps) handleRestoreTrash(w http.ResponseWriter, r *http.Request) {
	...
	// Database-sourced trash: soft-deleted project.
	if strings.HasPrefix(id, projectTrashPrefix) {
		...
		// one tx so no TOCTOU window exists between
	...
	for _, e := range entries {
		...
		// Drift kind dispatch. Drift trash holders carry a row_snapshot
		// in the sidecar; restore re-inserts the row via the per-protocol
		// Insert (which is an UPSERT) and moves the on-disk file back.
		// Unknown drift kinds fall through to the generic path (which will
		// fail via the e.Empty / childPath branches — belt-and-braces).
		switch e.Kind {
		...
		// Metadata-only entries (soft-delete of a never-synced git
		// mirror etc.) have no tree to move back.
		...
		// Prefer the sidecar-persisted OriginalPath so the restore lands
		// at the exact pre-delete location. Previous behavior
		...
		// Detect destination collision up-front so the user gets an
		// actionable 409 instead of a generic transient 500
...
func (d Deps) handlePurgeTrash(w http.ResponseWriter, r *http.Request) {
	...
	// Database-sourced trash: hard-delete project row.
	if strings.HasPrefix(id, projectTrashPrefix) {
		if d.Projects == nil {
			writeJSONError(w, r, http.StatusNotFound, ErrNotFound, "projects not configured")
			return
