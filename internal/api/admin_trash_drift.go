// Package api — drift-purge restore handler.
//
...
// gets re-inserted (UPSERT) before the on-disk file is moved back.
package api
...
func (d Deps) handleDriftRestore(
	...
	// Resolve childPath + dstPath from sidecar OriginalPath.
	if e.OriginalPath == "" {
		writeJSONError(w, r, http.StatusInternalServerError, ErrInternal,
