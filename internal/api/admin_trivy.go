// Package api — admin Trivy DB management endpoints.
//
// GET  /api/v1/admin/trivy/db/status       — Trivy DB metadata
// POST /api/v1/admin/trivy/db              — upload Trivy DB tarball
// POST /api/v1/admin/trivy/db/pull         — start online Trivy DB pull (async)
...
// tests and dev builds keep working.
...
func (d Deps) handleTrivyDBUpload(w http.ResponseWriter, r *http.Request) {
	...
	// Verify the extracted tarball actually contains a Trivy DB
	...
	// success. The previous implementation RemoveAll'd the old DB before
	// the rename, which could leave the scanner with no DB at all if the
	// rename then failed. SwapDir restores the old dir on failure.
	//
	// Serialize every rotation through trivyRotateMu so a concurrent
	// upload + online-pull cannot interleave rename-aside and rename-in
	// steps on the same dbDir.
	dbDir := d.trivyDBDir()
	trivyRotateMu.Lock()
...
func (d Deps) runTrivyDBPull(userID *int64) {
	...
	// Resolve the trivy binary via Deps so an operator-set
	...
	dbDir := d.trivyDBDir()
	// Serialize with the upload path through trivyRotateMu so a
	// simultaneous operator-initiated upload cannot interleave its
	// SwapDir with the pull's.
	trivyRotateMu.Lock()
