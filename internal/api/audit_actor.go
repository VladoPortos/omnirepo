...
// the correct nullable-column combination per the audit-attribution
// rules. It is the
...
// it (and reintroduce the project-owned-API-key bug).
//
// Mapping:
//
...
//     This surfaces the S3 access-key id directly so
...
// `d.recordAudit(r, audit.Event{...})` call shape.
//
// Package placement: either `internal/audit/`
// or `internal/api/` would work. The audit package cannot import internal/auth
// without creating a build-time import cycle
// (audit → auth → httpx → audit/sync_rest.go), so the helper lives here
// in the api package. The call-site migration consumes
...
func SetActor(e *audit.Event, actor auth.Actor) {
	...
	case auth.ActorKindAPIKey:
		...
		case auth.OwnerKindProject:
			...
			// against users(id) this helper exists to prevent.
			kid := actor.APIKeyID
			e.ActorUserID = nil
			e.ActorAPIKeyID = &kid
			e.ActorS3KeyID = nil
	...
	case auth.ActorKindS3Key:
		// Surface the resolved S3 access-key
