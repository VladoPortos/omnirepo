...
// Integration test — every state-changing project-scoped
// endpoint × {session, user-key, project-key} → asserts the correct
// (actor_user_id, actor_api_key_id) shape.
//
// This test is the forcing function: any future
...
// message. This locks the bug class out for good.
//
// Per-row rules:
//
...
// authMode bundles a single (cookie OR bearer) plus the expected
...
// assertActorShape enforces the three rules at the audit_log row
// boundary. The "FK-violation guard" failure message explicitly names
// the bug so a future maintainer who hits it understands the
// bug class.
func assertActorShape(t *testing.T, am authMode, row auditRow) {
	...
	if !am.wantUserIDNonNull && row.actorUserID != nil {
		// FK-violation guard — the regression detector.
...
// asserts each resulting audit row matches the expected shape.
//
...
// audit attribution decision is reached. This targets audit-row
...
// user fk) and separate work. Skip the project_key/s3_access_key cell
// with an explicit reason so the matrix grid stays readable and a future
// change can pick this up.
func runS3KeyCreateRevoke(t *testing.T, f *auditAttrFixture) {
	...
	if f.am.name == "project_key" {
		t.Skipf("s3_access_keys.created_by_user_id is NOT NULL FK to users(id); " +
			"project-owned-key creator attribution is out of scope")
	}
...
// This is the canonical example showing all three actor shapes plus the
...
// rows carry the correct (actor_user_id, actor_api_key_id) shape.
//
// Forcing function: the project_key/* subtree fails with
