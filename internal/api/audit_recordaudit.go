...
// `uid := actor.ID; ActorUserID: &uid` pattern that introduced the
// project-owned-API-key bug (project-owned API keys silently writing actor_user_id = 0,
// violating the FK against users(id) and dropping the audit row).
//
// Every documented call site is migrated to this helper.
//
// Authentication-layer call sites (admin_phase1.go handleLogin /
// handleLogout / handleChangePassword) are intentionally
...
//
// Mirrors the InterceptPutObjectForTest pattern: tests
