// Package api — dashboard endpoint.
//
...
// column is currently never written, so reading it
...
// functions.
//
// Ref-counted shared-blob bytes: each blob's bytes are split across
...
func (d Deps) handleDashboard(w http.ResponseWriter, r *http.Request) {
	...
	// visibleProjectIDs handles super-admins,
	...
	// Log DB errors instead of silently swallowing. COALESCE still
	...
	// Storage: live sum across artifact tables per-repo. `repos.size_bytes`
	...
	// owned by a visible, live project. Deleted buckets and buckets
	...
	// row with "× N". scopeClause is empty for super-admin, so the
	...
	// introducing the duplication this was meant to fix.
	...
	if scopeClause == "" {
		...
		// surface audit rows for that project's repos.
...
func (d Deps) handleStorageDetail(w http.ResponseWriter, r *http.Request) {
	...
	// Used bytes — live computed. Also adds S3 bucket contents which
	...
	// soft-deleted, matching the behaviour at /api/v1/dashboard.
	...
	// Per-repo breakdown sorted by size DESC (live-computed).
	// Filter soft-deleted projects too — otherwise a hidden project's repos
	// keep appearing in the breakdown list.
	repoArgs := make([]any, len(scopeArgs))
	copy(repoArgs, scopeArgs)
	...
	// S3 buckets as type="s3" rows. Every non-deleted bucket is
