...
// TestDashboard_HighSeverityDedupes verifies the High-Severity Findings
...
func TestDashboard_HighSeverityDedupes(t *testing.T) {
	...
	// previously the dashboard would return all 5 as separate rows.
...
// TestDashboard_HighSeverityCollapsesAcrossSeverities verifies that if the
// same (cve, package, repo) tuple exists with
...
// the duplication this was meant to eliminate.
...
// TestDashboard_SoftDeletedProjectDoesNotBleed verifies that a soft-deleted
// project's repos and their vulnerabilities
...
// TestDashboard_ProjectCountExcludesSoftDeleted verifies the dashboard's
...
func TestDashboardStorage_ReturnsRepoBreakdown(t *testing.T) {
	...
	// bytes. The storage handler ignores `repos.size_bytes` and
...
// TestDashboardStorage_RefCountsSharedBlobs verifies that a blob referenced
