...
// sites to the canonical internal/httperr envelope.
//
...
// Stable error codes consumed by UI and legacy tests. Each is
// normalized to a dotted code in writeJSONError before the
...
// mechanical widening applied across every /api/v1 handler.
//
...
// Without it, chi's default handler emits a zero-byte body,
...
// writeEnvelope is the first-class path for handlers that
...
// unchanged (newer callers supply dotted codes directly).
...
func normalizeLegacyCode(code string) string {
	...
	// "errors.go:123" or "/home/…/foo.db" reach the wire body (caught by
	// the regression gate in errors_envelope_test.go).
...
//
// Standardizes body-size defense so new handlers
