...
// universal ERR-01 / ERR-03 / ERR-07 invariants plus class-specific
...
// message or hint (ERR-03). httperr.IsInternalString is the primary
...
// invariants declared in the plan-04 must-haves:
//
...
// user in the context. Plan 06-04 migrated the middleware to emit the
...
func TestEnvelope_NotFound_UnknownAPIRoute(t *testing.T) {
	...
	raw, _ := json.Marshal(body)
	e := assertEnvelope(t, raw)
	// 404 → validation class per plan-02 inferClassFromStatus table
...
// TestEnvelope_IncidentIDMatchesHeader pins the ERR-07 parity
...
// TestEnvelope_NoInternalLeakage_AcrossHandlers is the sweep gate for
// ERR-03: walk every representative failure path we can force on a
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if resp.StatusCode < tc.minStatus {
				...
				// primary assertion is ERR-03 leakage; status drift is
...
// for ERR-03: an attacker's payload in the login field MUST NEVER be
...
// TestEnvelope_OperatorClass_DevRoute pins ERR-05: class
// operator_action_required MUST carry operator_route + operator_label
// so the UI renders the deep-link CTA. operator_route must be a
// same-origin path (starts with "/") to respect T-06-03-03 (no open
// redirects).
