// Package api — mirror-repo request-shape validators.
//
...
// Raw/S3/Docker remain out of scope; Git is HTTPS+PAT only, with
...
// Rejects file://, javascript:, ftp://, bare
// paths, and missing hosts regardless of repoType.
...
// keys so REST/CLI callers can use the same casing convention as
...
// the legacy PascalCase keys and the new snake_case keys. The
...
func validateMirrorFilter(repoType string, filter json.RawMessage) (bool, json.RawMessage) {
	...
	case "helm":
		...
		return reencodeFilter(f)
	case "git":
		// Git mirrors are all-refs, no per-ref filter API.
...
// caller can chain it with the rest of the validator suite.
//
// The 100/6h rate-limit phrase is the
...
// the UI does not lowercase-normalize upstream URLs before storing.
func refuseDockerHubWithoutCred(upstreamURL, credKind string) *httperr.Error {
	...
	if credKind != "" {
		// Any non-empty cred kind unblocks. The expected kind is
		// "basic"; this validator only enforces "something
