...
func TestValidateMirrorUpstreamURL(t *testing.T) {
	...
	//
	// The signature is (raw, repoType) so the oci://
	// scheme is accepted only for helm mirrors. "deb" is used here to keep
	// these non-helm cases asserting the non-helm behavior unchanged.
	cases := []struct {
		in string
		ok bool
	}{
...
// TestClassifyHelmUpstream covers the classifier that tags a helm
// mirror upstream URL as http vs oci (or unsupported). Only http/https/oci
// schemes are accepted; bare-host strings without a scheme are rejected
// (Helm SDK accepts them, but our validator path requires an
...
// TestValidateMirrorUpstreamURL_HelmAcceptsOCI verifies the widening:
// helm mirrors accept oci:// in addition to http(s); other repo types
// continue to reject oci://. Invariant: file://, javascript:,
...
func TestValidateMirrorFilter_AcceptsSnakeCase(t *testing.T) {
	...
	// encoding). Decoding accepts snake_case keys too while
...
// TestMirrorValidate_GitAccepted — widening the
...
func TestMirrorValidate_GitAccepted(t *testing.T) {
	if _, ok := mirrorSupportedTypes["git"]; !ok {
		t.Fatalf("mirrorSupportedTypes must include \"git\"")
	}
...
// TestMirrorValidate_GitRejectsOCI — git is HTTPS-PAT only.
// oci:// is reserved for helm mirrors and must not leak into
// the git branch.
func TestMirrorValidate_GitRejectsOCI(t *testing.T) {
	if validateMirrorUpstreamURL("oci://reg/foo/chart", "git") {
		t.Fatal("git must NOT accept oci:// (HTTPS-PAT only)")
	}
}
...
func TestMirrorValidate_ExistingProtocolsUnchanged(t *testing.T) {
	...
	if !validateMirrorUpstreamURL("oci://reg.io/charts/nginx", "helm") {
		t.Fatal("helm must still accept oci:// after widening")
	}
...
// TestRefuseDockerHubWithoutCred covers the Docker Hub gate.
// registry-1.docker.io OCI upstreams MUST carry a basic
...
//
// The envelope code is STABLE — tests assert the literal string.
func TestRefuseDockerHubWithoutCred(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Message must mention the 100/6h rate
