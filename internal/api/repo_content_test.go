...
// TestRepoContent_DockerListsTags covers the case where the content endpoint
// used to early-return [] for docker repos, so the UI showed "No artifacts yet"
...
func TestRepoContent_DockerListsTags(t *testing.T) {
	...
	// A legacy tag with empty image, same digest.
	_, err = s.db.Writer.ExecContext(ctx,
...
// TestRepoContent_DockerIndexTag_AggregatesChildrenScans is the
