// Package api — repo mutation endpoints.
//
// PATCH /api/v1/projects/{name}/repos/{type}/{repo}
...
// the orphan directory is swept by GC.
package api
...
// Canonical envelope codes emitted by CreateRepo and PatchRepo
// mirror-validation branches. Dotted wire form satisfies the envelope schema
// regex (^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$) while the second segment keeps
// the operator-facing token verbatim so tests and docs resolve directly.
const (
	...
	// codeRepoDriftPurgeMirrorOnly: drift_purge=true is only meaningful on
	// mirror repos. A non-mirror repo has no upstream to diff against;
	// accepting the flag would be a silent no-op. Reject explicitly so
	// operators learn the invariant at call time.
...
//
// IsMirror / MirrorUpstreamURL are accepted on the wire but always rejected
// with 400 repo.mirror_url_immutable so the wire shape matches
// CreateRepoRequest and the UI library can reuse one TypeScript interface for
// both requests. The three editable fields (MirrorFilter / MirrorCredID /
// ScanOnSync) flow through to UpdateFields.
type repoPatchRequest struct {
	...
	// state (Go pointer semantics). This lets the Docker Hub cred-gate
	// distinguish "patch leaves cred alone" from "patch clears cred".
	MirrorCredIDRaw json.RawMessage `json:"mirror_cred_id,omitempty"`
	ScanOnSync      *bool           `json:"scan_on_sync,omitempty"`
	// DriftPurge: mirror-only opt-in. PATCH'ing drift_purge=true on a
	// non-mirror repo is rejected with codeRepoDriftPurgeMirrorOnly.
	// drift_purge=false is always allowed.
...
//
// Mirror fields are echoed on GET so the UI can render the Sync Now button +
// Mirror config card conditionally on `is_mirror`. `mirror_filter_json` is a
// raw JSON string (TEXT column) —
...
type repoResponse struct {
	...
	// ItemCount renders "42 packages · 180 MB" in the repo header.
	...
	// DriftPurge: per-mirror opt-in for drift purge on sync. Non-mirror
	// repos always serialize false.
...
func (d Deps) handleGetRepo(w http.ResponseWriter, r *http.Request) {
	...
	// repos.size_bytes is never written; overlay the live aggregate so
	...
	// Overlay live item count so header renders "42 packages · 180 MB".
...
// handlePatchRepo updates editable repo fields. The per-route middleware
// already enforced project membership via RequireCanWith(ActionUpdateRepo),
// so any caller reaching here is authorized.
func (d Deps) handlePatchRepo(w http.ResponseWriter, r *http.Request) {
	...
	// Reject attempts to change is_mirror or mirror_upstream_url via PATCH.
	// The constants below (codeRepoMirror*) live at the top of this file so
	// they resolve against a single canonical source.
	...
	// drift_purge=true is mirror-only. Non-mirror repos have no upstream
	// diff to trigger purge against; reject explicitly with a typed envelope
	// rather than silently storing a flag that has no effect.
	// drift_purge=false is always allowed (no-op on non-mirror; idempotent
	// on mirror).
	...
	// Docker Hub cred gate on PATCH. The invariant is post-patch: after
	// this PATCH commits, the mirror MUST NOT be (Docker Hub oci://, no
	// cred). The effective
...
// handleWipeRepo empties a repo. Only docker + raw types are supported for
// now; other types return 501.
