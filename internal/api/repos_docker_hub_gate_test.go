...
// API-layer Docker Hub credential gate on POST
// /api/v1/projects/{name}/repos and PATCH
...
// accepts "any non-empty cred kind" (kind is locked to 'basic'
// semantically; the validator only enforces "something attached"), so a
// helm-kind cred unblocks too.
