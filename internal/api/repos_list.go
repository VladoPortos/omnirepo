// Package api — repos list + sync job log endpoints.
//
...
//
// Mirror fields are echoed so the UI can filter / badge mirror repos in list
// views. Same shape as repoResponse for the single-repo endpoint; keeping the
// projections aligned lets the UI reuse one TypeScript interface for both.
type repoListItem struct {
	...
	// ItemCount is the per-type artifact count (see repoItemCountExpr).
...
//
// The projection includes the byte-level progress triple (progress_bytes,
// total_bytes, current_step) populated by the ProgressWriter helper in
// internal/jobs/progress.go. The UI polls this endpoint every 500 ms while
// a Sync Now / Docker clone modal is open. progress_bytes and total_bytes
// are always emitted (serialized as 0 when no progress yet) so the UI
// renders a deterministic `0 / N bytes` at job start rather than an "n/a".
// current_step is emitted as "" when empty for the same reason.
type syncJobItem struct {
	...
	// payload_json is deliberately NOT exposed: it is internal job input and
	// can carry inline upstream credentials (e.g. pull-external
	// src_username/src_password, mirror creds) in cleartext. Returning it to
	// any project member leaked those secrets. The UI does not consume it.
	Log           string `json:"log,omitempty"`
	ProgressBytes int64  `json:"progress_bytes"`
	TotalBytes    int64  `json:"total_bytes"`
	CurrentStep   string `json:"current_step"`
	// FilesSynced counts files newly added during sync. Written once at sync
	// completion by each protocol handler via SyncJobsRepo.SetFilesSynced
	// (NOT through the throttled progress path). 0 for running jobs; the UI
	// pill renders the "N files" piece of "Sync complete · N files · X MB"
	// when this is > 0.
	...
	// `drift_purged: int` and, when the percent-threshold guard tripped,
	// `drift_blocked: int`. Future summary writers add sibling keys via
	// json_set so consumers can parse only the keys they care about.
