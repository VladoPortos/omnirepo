...
// Sync-job GET endpoint tests covering the progress_bytes / total_bytes /
// current_step triple.
//
...
// pair).
//
// Tests live here rather than in admin_jobs_test.go because
// admin_jobs_test.go covers a different endpoint
// (GET /api/v1/admin/jobs/summary) that does not return per-job rows.
// Placing the progress-field tests next to the list tests keeps them with
// the handler they exercise.
...
// TestSyncJobs_DoNotLeakPayloadSecrets is a regression: payload_json can
// carry inline upstream credentials (pull-external src_username/src_password,
// mirror creds) in cleartext. Neither the by-id nor the list sync-jobs endpoint
// may return payload_json to project members.
func TestSyncJobs_DoNotLeakPayloadSecrets(t *testing.T) {
	s := newTestServer(t)
	cookie := bootProjectAndRepo(t, s, "secp", "deb", "r1")
	repoID := lookupRepoID(t, s.db, "secp", "deb", "r1")

	const secret = "sup3r-s3cret-upstream-pw"
	payload := `{"upstream_url":"https://example.test","inline_user":"robot","inline_pass":"` + secret + `"}`
	res, err := s.db.Writer.ExecContext(context.Background(),
		`INSERT INTO sync_jobs(kind, repo_id, status, payload_json, log) VALUES ('pull_external', ?, 'done', ?, '{}')`,
		repoID, payload)
	if err != nil {
	...
	assertNoSecret := func(label string, body map[string]any) {
		if _, present := body["payload_json"]; present {
			t.Errorf("%s: response exposes payload_json key", label)
		}
		raw, _ := json.Marshal(body)
		if strings.Contains(string(raw), secret) {
			t.Errorf("%s: response leaks upstream secret; body=%s", label, raw)
		}
	}

	getPath := "/api/v1/projects/secp/repos/deb/r1/sync-jobs/" + strconv.FormatInt(jobID, 10)
	resp, body := s.do(t, "GET", getPath, cookie, nil)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("GET sync-job: code=%d body=%+v", resp.StatusCode, body)
	}
	assertNoSecret("by-id", body)

	listResp, listBody := s.do(t, "GET", "/api/v1/projects/secp/repos/deb/r1/sync-jobs", cookie, nil)
	if listResp.StatusCode != http.StatusOK {
		t.Fatalf("list sync-jobs: code=%d body=%+v", listResp.StatusCode, listBody)
	}
	assertNoSecret("list", listBody)
}
...
// TestGetSyncJob_IncludesFilesSynced verifies the files_synced column.
//
// REST-layer regression following the filter.Suites pattern: the
...
// TestGetSyncJob_IncludesSummary verifies the summary field.
//
