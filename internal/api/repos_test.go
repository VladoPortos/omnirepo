...
// TestCreateRepo_InvalidName_ErrorSaysRepoNotProject locks the fix for a
// copy bug: the repo-create validator error must refer to the *repo* name,
// not the project name. The UI echoes this wire message verbatim in the
// validation envelope; reusing `ProjectNameValid` leaked "invalid project
// name ..." into the Create Repository dialog.
func TestCreateRepo_InvalidName_ErrorSaysRepoNotProject(t *testing.T) {
	...
	// half of the error-handling fix.
...
// --------------------------------------------------------------------------
// CreateRepo + PatchRepo mirror validation.
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// drift_purge PATCH + GET coverage.
// --------------------------------------------------------------------------
...
func TestHandlePatchRepo_DriftPurgeOnMirror_Accepted(t *testing.T) {
	...
	// Audit diff captures the change.
	var details string
	err := s.db.Reader.QueryRowContext(context.Background(), `
		SELECT details_json FROM audit_log WHERE event_kind='repo.updated' ORDER BY id DESC LIMIT 1
	`).Scan(&details)
