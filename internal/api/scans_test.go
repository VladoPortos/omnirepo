...
// TestScansREST_Rescan_UserOwnedAPIKey_Accepted is the regression for a site
// found after the initial fix: Deps.actorIsProjectMember used to early-return
// on `actor.Kind != ActorKindUser`, denying user-owned API keys on every
// scan/content/artifact endpoint even when the owning user was a project
// member. The fix drops that Kind guard so Actor.ID (already the owning
// user's id per the Actor.ID doc) flows into the project_members lookup.
//
...
func TestScansREST_Rescan_UserOwnedAPIKey_Accepted(t *testing.T) {
	...
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != 202 {
		t.Fatalf("user-owned API key rescan got status=%d, want 202", resp.StatusCode)
	}
}
