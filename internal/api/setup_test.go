...
func TestSetup_SuperAdmin_ConcurrentRequestsCreateExactlyOneUser(t *testing.T) {
	// Regression guard against Codex's finding: a naive pre-flight empty
	// check + separate create tx let two concurrent bootstrap requests with
	// different logins both succeed. With the empty-check inlined into the
	// write tx, only one request wins; the loser must get 409 and the
	// database must contain exactly one user.
	s := newTestServer(t)
