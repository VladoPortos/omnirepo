...
// TestSyncActorBridge_UserOwnedAPIKey pins F-06.5 (wt3 batch 06):
// a user-owned API-key actor (OwnerKind=user, ProjectScope=nil) must surface
// the owning user's id as UserID on the bridged httpx.SyncActor — otherwise
// handleSync's `actor.UserID != 0` branch is skipped, the project-member
// lookup never runs, and every user-owned key 403s on POST .../sync. Same
// root-cause family as F-05.1 (11th open-coded membership site).
