...
type MeResponse struct {
	...
	// ProjectRoles Map of project name → caller's role in that project ("maintainer" or "viewer"). Omitted or empty for super-admin callers (they use is_super_admin for gating). Only projects the caller is a member of appear.
...
type Project struct {
	...
	Members *[]struct {
		...
		// Role Role of this member in the project. "maintainer" has full write access; "viewer" has read-only access.
...
// ProjectMembersRole Role of this member in the project. "maintainer" has full write access; "viewer" has read-only access.
type ProjectMembersRole string

// ProjectAPIKeyCreate Request body for POST /projects/{name}/api-keys. `role` is optional and
// defaults to "maintainer" at the application layer. Minting a
// project-scoped key with role="viewer" lets CI pipelines publish with
// the correct least-privilege role.
...
type SyncJob struct {
	...
	// ProgressBytes Bytes downloaded so far for the running sync. Always 0 for Helm (step-based progress).
	ProgressBytes *int64         `json:"progress_bytes,omitempty"`
	RepoId        *int64         `json:"repo_id,omitempty"`
	StartedAt     *time.Time     `json:"started_at,omitempty"`
	Status        *SyncJobStatus `json:"status,omitempty"`

	// TotalBytes Total bytes expected for the sync. Always 0 for Helm and when the upstream doesn't report sizes.
...
type AddProjectMemberJSONBody struct {
	// Role Role to assign. Defaults to "viewer" when omitted. Returns 422 codeValidationFailed for any other value.
