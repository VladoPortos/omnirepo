...
// are mounted. The base newTestServer does not include it, and api.Mount
...
// -------------------------------------------------------------------------
// Regression tests — legacy `kind="deb"` redirect.
// -------------------------------------------------------------------------

// assertDebMigrationEnvelope decodes the response body as an
// ApiErrorEnvelope and asserts every field required by the deb-redirect
// contract. Shared between POST + PATCH tests so the wire shape cannot
// drift between the two handlers.
...
// POST pre-check: POSTing kind="deb" returns HTTP 400 with the deb-redirect
// envelope, NOT the 422 that a generic unknown kind would emit.
...
// regression: proves the deb-redirect pre-check does NOT accidentally
