...
// Boot-recovery sweep tests.
//
// Two assertions:
//
//   BootSweepsOrphans     — seed a stale s3_multipart_uploads row,
//                           boot the app, assert the row is gone after
//                           the boot goroutine completes.
//   BootTimeoutGrepGate   — grep gate ensuring the goroutine wraps
...
// TestBootSweep_RemovesOrphanMultipart pins that a stale multipart upload
...
// imposes — if the goroutine is broken the test fails fast.
...
// TestBootSweep_BootContextTimeoutGrepGate — grep gate enforcing the
// goroutine bounds itself with context.WithTimeout(appCtx, 5*time.Minute)
// rather than context.Background(). A simple regex over app.go
