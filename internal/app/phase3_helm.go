...
// helm.RegenFor so uploads trigger index.yaml regeneration.
//
// The returned registry is what the caller shuts down on exit. The helm
// Mirror is constructed alongside but is NOT wired to
...
func (d helmDeps) wireHelm(router chi.Router) (*regen.Registry, *helm.Mirror, *helm.Handler) {
	...
	// Construct the forward-only Mirror alongside the
...
//
// The OCI manifestPut post-commit hook supplies the
