...
func TestRecordBestEffortOnNDJSONFailure(t *testing.T) {
	...
	// — the NDJSON mirror is best-effort.
...
func TestEveryStateChangingActionEmitsEvent(t *testing.T) {
	...
	kinds := []audit.EventKind{
		...
		// Mirror infrastructure widening. Emitted by helm sync_handler and
		// git sync_handler; enumerated here so the kinds compile + round-trip
		// through the audit sink exactly like their predecessors.
		audit.EvtOciTagRebound,
		audit.EvtMirrorSyncLFSDetected,
		// RBAC role-change event.
		audit.EvtMemberRoleChanged,
		// PyPI parser hardening. Emitted by
