...
type MembershipLister interface {
	...
	// non-deleted project the user is a member of, so the policy engine can
	// carry role alongside membership. MembersRepo implements this; the stub
	// in membership_test.go derives roles from ListProjectIDsForUser.
...
//
// Previously each protocol handler open-coded membership resolution with
...
//
// It calls ListProjectRolesForUser (returns map[int64]string) instead of
// ListProjectIDsForUser, so the policy engine can distinguish viewer vs
// maintainer role. Project-scoped API keys use actor.APIKeyRole (the minted
// role from api_keys.role); an empty APIKeyRole falls back to "maintainer"
// for legacy/backfilled keys.
//
// Shape dispatch:
//
...
func ResolveMembership(ctx context.Context, actor Actor, members MembershipLister) context.Context {
	...
	case ActorKindAPIKey:
		if actor.ProjectScope != nil {
			// Project-scoped key: use the key's minted role.
			// actor.APIKeyRole is populated by auth middleware from api_keys.role.
			role := actor.APIKeyRole
			if role == "" {
				role = "maintainer" // safe fallback for legacy/backfilled keys
			}
