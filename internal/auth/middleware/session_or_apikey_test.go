...
// TestSlidingSessionExpiryExtendsOnActivity is the regression gate for
// a fresh-minted session that is about to expire in a few minutes: it must be
...
// TestHardCapRejectsSessionPastSevenDays is the hard-cap regression
...
func TestRequireCanReturns403PasswordChangeRequired(t *testing.T) {
	...
	// The middleware emits the ApiErrorEnvelope shape. The stable policy
	// reason token maps to the dotted envelope code
