...
// Argon2id parameters. These are tuned for OWASP 2026 low-RAM defaults.
...
const (
	argonTime = uint32(3)
	// argonMemory is 65536 KiB = 64 MiB.
...
// closing the user-enumeration timing oracle. The plaintext it hashes
// is constant and intentionally never matches a real password.
//
// Computed at package init using the SAME argon2id parameters as
...
// HashPassword derives an argon2id hash of plain using the configured parameters
// and encodes the result in the canonical PHC-ish format:
//
...
// first-login flow. Entropy source is crypto/rand; each character is
