...
// The caller (rpm/sync_handler.go in plan 06-07) binds it from the
...
// Drift key per D-12: {name, version, arch}. This is intentionally
...
// across rebuilds. PATTERNS.md Pitfall 7 documents the trade-off.
...
func (a *rpmAdapter) Purge(ctx context.Context, tx *sql.Tx, row Row, actor string) error {
	pr, ok := row.(*rpmRow)
	if !ok {
		return fmt.Errorf("rpm adapter: unexpected row type %T", row)
	}
	inner := pr.inner
...
// rpmRow wraps *metadata.RPMPackage. Key per D-12 is {name, version, arch}.
type rpmRow struct {
