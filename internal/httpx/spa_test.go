...
// the user requests the directory (trailing slash). /swagger/ used to fall
// through to the React SPA shell because http.FileServer's
// directory-redirect (/swagger/index.html → ./) bounced us into the SPA
// fallback. Guards against the regression.
...
// TestSPAHandler_APIPathReturns404JSON guards that unknown /api/* must not
// fall through to index.html — otherwise HTTP 200 with a SPA body looks like
// the call succeeded until the caller tries to JSON-parse the body.
func TestSPAHandler_APIPathReturns404JSON(t *testing.T) {
	...
	for _, path := range []string{"/api/v1/missing", "/api/", "/v2/unknown/manifest/sha256:deadbeef"} {
		...
		// API-like NotFound paths emit the canonical
