...
// --------------------------------------------------------------------------
// Mirror-aware /sync + concurrency + body cap.
// --------------------------------------------------------------------------
...
// TestSync_MirrorBodyAcceptsForceDriftThreshold.
//
...
// narrow exception: only `force_drift_threshold` alone is
...
// (clean switch from the legacy `sync.sync_already_running` code). Payload
// shape is the full httperr.Envelope now, not the minimal {error, detail}
// shape the previous writeJSONErr emitted.
...
// --------------------------------------------------------------------------
// Git allow-list + generalized 409 envelope code `mirror.sync.in_flight`
// with {kind, job_id, started_at}.
// --------------------------------------------------------------------------
...
// TestSyncRest_409Envelope_NewShape asserts the generalized 409
...
// re-check). The file-body assertion makes the invariant enforceable
// regardless of test harness shape.
func TestSyncRest_LegacyCode_NotEmitted(t *testing.T) {
	...
	if strings.Contains(body, "sync.sync_already_running") {
		t.Errorf("sync_rest.go must not emit legacy 409 code `sync.sync_already_running`")
	}
	...
	// git widening invariant.
	if !strings.Contains(body, `"rpm", "deb", "pypi", "helm", "git"`) {
		t.Errorf("sync_rest.go allow-list must include git")
	}
}
