...
// considered abandoned and re-pended on boot.
const StuckJobThreshold = 10 * time.Minute

// helmBootPartialLog is the partial-log JSON payload written for
// kind='helm_sync' rows recovered at boot. Counts are null because boot
// recovery cannot know how many charts committed before the crash.
// Readers distinguish "boot-recovered" rows (nulls here) from
// "live-path recovered" rows (real counts written via
// MarkPermanentlyFailedWithLog) by checking whether files_persisted is null.
...
// status='failed' by the helm-specific sweep. SyncRecovered counts
// non-helm stale rows re-pended via the generic RecoverStale sweep.
// ScansRecovered counts stale scans rows re-pended.
...
//     payload. Must run FIRST — otherwise the generic RecoverStale below
...
//     RecoverStale (unchanged semantics; the helm sweep above already
//     consumed every matching helm row, so this one sees non-helm kinds
//     only).
...
func RecoverStuckJobs(ctx context.Context, db *metadata.DB) (RecoveryReport, error) {
	...
	err := db.WriteTx(ctx, func(tx *sql.Tx) error {
		// Helm-specific sweep runs FIRST so the generic RecoverStale below
		// never sees helm rows. Ordering is load-bearing — reversing it
		// would re-pend helm rows before they could be terminated.
