...
// Helm-specific boot-recovery sweep. Stale kind='helm_sync' rows
// terminate at status='failed' with a null-counts partial-log JSON
// BEFORE the generic RecoverStale sweep runs. Non-helm kinds continue
// through the existing generic path unchanged.
...
// them to 'pending' — the scope boundary at the boot path.
