// Package jobs — scaled SCAN-12 regression (Phase 02-14, UAT #5).
//
...
func TestGCScaled_1000Manifests_NoRegressions(t *testing.T) {
	...
	// simultaneously show up in blob_uploads — the SCAN-12 race.
	...
	go func() {
		...
		for i := 0; ; i++ {
			...
			// BEFORE the docker_blobs row insertion (SCAN-12).
	...
	for _, d := range active {
		if !f.blobExists(d) {
			t.Errorf("in-flight blob %s was deleted by GC (SCAN-12 violation)", d)
		} else {
			surviving++
		}
	}
