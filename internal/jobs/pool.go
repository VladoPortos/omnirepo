...
// sync_jobs.last_error (exposed via GET /sync-jobs/{id}). Handler errors
// can embed upstream response bodies or wrapped Go error chains; we
// sanitize + truncate before persist so clients of the public API never
// see a filesystem path, multi-KB HTTP body, or a stack-trace-like error
// chain. Matches the truncateErr convention in
...
// don't block the queue. Downstream code owns the population of this map
// at app.Run wire time.
...
// full the signal is dropped (the pending tick will handle it).
func (p *Pool) Kick() {
	select {
	case p.kick <- struct{}{}:
	default:
...
// Without this guard a panicking handler would tear down the worker
...
func (p *Pool) handle(ctx context.Context, j *JobView) {
	...
	if err == nil {
		if derr := p.db.WriteTx(ctx, func(tx *sql.Tx) error {
			...
			// 'running' and boot recovery will re-pend it. Don't
...
func (p *Pool) markFailed(ctx context.Context, j *JobView, herr error) {
	// Helm partial-sync errors terminate at status='failed' BYPASSING the
	// retry ladder, and the 3-field partial-log JSON is written in the
	// SAME UPDATE as the status flip (atomicity via
	// MarkPermanentlyFailedWithLog).
	//
	// This branch runs BEFORE the ctx.Err() shutdown bail below:
	// graceful-shutdown ctx-cancel is the most common trigger of a partial
	// sync, so losing the terminal write on cancel would leave the row
	// stuck 'running' until boot recovery. If ctx is already cancelled we
	// swap in context.Background() for the DB write so the metadata writer
	// pool (separate lifecycle) can still commit. Boot recovery is the
	// safety net if the DB is itself mid-close.
	...
	// Over-limit (artifact / metadata) errors are PERMANENT — the upstream
	// is over the configured cap, so retrying with the same input is
	// guaranteed to fail again. Short-circuit to terminal 'failed' status
	// BYPASSING the retry ladder, mirroring the partial-sync precedent
	// above. Same ctx-cancel fallback to context.Background() for
	...
	// The raw error goes to slog for operators; the client-visible
	// last_error is scrubbed.
	safeErr := sanitizeJobError(herr)
