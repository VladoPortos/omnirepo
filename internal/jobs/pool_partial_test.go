...
// the helm-to-jobs end-to-end path is covered separately.
...
// 3-field partial-log JSON written atomically.
...
func TestPool_HelmPartialSync_TerminalFailed(t *testing.T) {
	...
	handler := func(ctx context.Context, j *jobs.JobView) error {
		...
		// Wrap the partial error so errors.As must traverse the Unwrap
		// chain (not just the outer type). Proves the interface-target
		// discovery works through fmt.Errorf("%w") layers, which is how
		// the real helm handler returns wrapped cancel causes.
	...
	// Exactly one handler call — bypass of retry ladder.
	...
	// proof for the single-write contract).
...
// the error satisfies PartialSyncError. The generic retry ladder still
// applies.
...
// the ladder (guards against error-type confusion).
