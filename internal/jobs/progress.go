// Package jobs — ProgressWriter is the throttled helper protocol sync
// handlers use to advance sync_jobs.progress_bytes / total_bytes /
// current_step without hammering the writer pool.
//
// Throttle contract:
...
// progress writes. A hot-loop io.Read callback can fire thousands
...
// The step text embeds upstream-controlled fields like
// `pulling <Package>_<Version>` or `chart N of M · <filename>`,
...
func (p *ProgressWriter) Set(ctx context.Context, step string, done, total int64) error {
	...
	// Clamp upstream-controlled step text at the boundary.
	step = clampStep(step)
	p.mu.Lock()
	defer p.mu.Unlock()
