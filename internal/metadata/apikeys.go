...
//
// Role is populated from the api_keys.role column. For user-owned keys it
// is always nil (the key inherits role from the owner's current
// project_members rows resolved at request time). For project-owned keys
// it is "maintainer" or "viewer" — the role baked in at mint time. The
// middleware threads this into Actor.APIKeyRole so the policy engine can
// gate writes correctly for viewer-minted project tokens.
type APIKey struct {
	ID             int64
...
// NULL). Callers who need to display revoked keys for audit will use a
// distinct method (not exposed yet). This makes the middleware path
// trivially "lookup-or-401" without a branch on revoked_at.
...
// project-scoped tokens carry a minted role.
...
// project to be live. User-owned rows are unaffected by project lifecycle.
//
...
// after the project was soft-deleted (defense-in-depth alongside the
...
// /api/v1/projects/{name}/api-keys endpoint.
...
// middleware auth.
//
// Write via DBTimestampLayout (fixed-width ISO-8601) instead of raw
// time.Time binding — modernc/sqlite otherwise serializes as Go-%v
...
// cascade-revoked by THIS soft-delete. Idempotent: rows already revoked at
// a different timestamp are left untouched.
...
// whose revoked_at exactly equals priorTS are cleared. Returns the
...
// account deletion: users are soft-deleted, so the FK cascade that would
// normally clean up api_keys never fires — without this the
