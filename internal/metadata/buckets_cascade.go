...
// (LIFECYCLE-02, LIFECYCLE-04). No dedicated S3BucketsRepo struct exists in
// this package; bucket CRUD lives in internal/protocol/s3/backend. These
// helpers are project-cascade-only and live here so the ProjectsRepo cascade
// closure does not have to reach across package boundaries.
//
// Timestamp marker (D-01..D-05): the caller stamps `projects.deleted_at` first
// inside its WriteTx, reads the value back, and passes the literal string here
...
// by THIS project soft-delete (D-05). Buckets independently soft-deleted
