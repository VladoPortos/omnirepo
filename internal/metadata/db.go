...
//
// It implements the reader/writer split: two *sql.DB handles against
...
// serialize cleanly without ever returning SQLITE_BUSY.
//
// modernc.org/sqlite is imported here and registered as driver name "sqlite".
package metadata
...
// precision avoids the Go-%v format variability that broke timestamp
// comparison for audit, sessions, and api-keys.
//
...
// readerPoolSize is the reader-pool default (N=8).
...
// pragma list at open time — not just the first one. modernc.org/sqlite
...
// with driver defaults (foreign_keys=OFF, busy_timeout=0).
//
// Also transparently appends ?_txlock=immediate so every sql.Tx is promoted
// to BEGIN IMMEDIATE (see tx.go).
//
...
//     of DEFERRED.
//  2. The PRAGMA list via `_pragma=...` — every new connection on both
