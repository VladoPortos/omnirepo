...
// digest. SELECT-then-INSERT-OR-REPLACE pattern for simplicity.
...
// ResolveTx is Resolve but reads via the caller-supplied tx, for use inside a
// WriteTx where a Reader-pool read would deadlock against the in-flight writer
// (same rationale as DockerManifestsRepo.GetByDigestTx). found is false when
// the (repo,image,tag) row is absent — which lets callers distinguish a brand
// new tag binding from a re-push of an unchanged one (Upsert collapses both to
// an empty priorDigest).
func (r *DockerTagsRepo) ResolveTx(ctx context.Context, tx *sql.Tx, repoID int64, image, tag string) (digest string, found bool, err error) {
	err = tx.QueryRowContext(ctx,
		`SELECT digest FROM docker_tags WHERE repo_id=? AND image=? AND tag=?`,
		repoID, image, tag,
	).Scan(&digest)
	...
	if err != nil {
		return "", false, fmt.Errorf("docker_tags: resolve tx: %w", err)
	}
...
// cosign badge check: tag "sha256-<hex>.sig" presence → signed=true.
