...
// -- Per-protocol FTS5 helpers. Each insert/delete runs inline in the
// -- caller's writer tx so a search executed after tx.Commit() reflects
// -- the change immediately. Tables live in migration 014_protocol_fts.up.sql
// -- and share the shape
...
// helm_fts by repo_id), and conditionally prunes cves_fts: a CVE is
...
func PruneRepoFTS(ctx context.Context, tx *sql.Tx, repoID int64) error {
	...
	// Conditional cves_fts prune. Delete cves_fts rows whose every
...
// Used by Repos.Restore + Projects.Restore. Construction takes
