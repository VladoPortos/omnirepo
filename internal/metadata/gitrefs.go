// Package metadata — GitRefsRepo owns the git_refs mirror table (Phase 04
// Plan 02, D-36..D-37). Rows are rebuilt synchronously by the
// post-ReceivePack walker still holding the per-repo mutex: classify each
// ref, DELETE the old rows for the repo, batch-INSERT the new set.
//
// Batch INSERTs chunk at 200 rows per §P14 to stay well under
