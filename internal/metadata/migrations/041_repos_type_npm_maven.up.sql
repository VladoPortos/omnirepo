-- Migration 041_repos_type_npm_maven:
--
-- Adds 'npm' (npm registry) and 'maven' (Maven repository) to the
-- repos.type CHECK in a single 027/040-style rebuild (SQLite cannot ALTER
-- a CHECK constraint). Rows copied verbatim preserving ids so every FK TO
-- repos resolves after RENAME. Column set matches migration 040.

CREATE TABLE repos_new (
    id                   INTEGER PRIMARY KEY AUTOINCREMENT,
    project_id           INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    type                 TEXT NOT NULL CHECK(type IN ('rpm','deb','pypi','docker','helm','git','raw','go','npm','maven')),
    name                 TEXT NOT NULL,
