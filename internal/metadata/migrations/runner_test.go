...
// TestMigration026_LiveOnlyUnique verifies that after migration 026, the
...
func TestMigration026_LiveOnlyUnique(t *testing.T) {
	...
	if _, err := db.Writer.ExecContext(ctx,
		...
		t.Fatalf("re-create soft-deleted login: %v", err)
	}
...
func TestFTS5_TablesAcceptInserts(t *testing.T) {
	...
	err := db.WriteTx(ctx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `INSERT INTO repos_fts(repo_name, project_name, description, type) VALUES (?,?,?,?)`,
			"infra", "acme", "Infrastructure repo", "git"); err != nil {
			return err
		}
...
func TestPhase3MigrationsForwardAndDown(t *testing.T) {
	...
	// the schema is scrubbed. The runner itself has no revert-path (down
	// scripts exist for audit only), so we exec them directly.
	downOrder := []string{
	...
	// Every table from migrations 008..015 must now be gone.
...
// Migrations 016..019 (S3 access keys, git extensions,
...
func TestPhase4MigrationsForwardAndDown(t *testing.T) {
	...
	// the schema is scrubbed. Runner has no revert path, so exec directly.
	downOrder := []string{
	...
	// Every table from migrations 016..019 must now be gone.
	for _, tab := range []string{
