...
// pragmaDSNValues is the pragma list carried into every connection via
...
// DB-health endpoint (internal/api/admin_db_health.go) uses this
// as the SOURCE-OF-TRUTH for the `driver.pragmas` map and for the
// `journal_mode` response field.
//
// Why a snapshot, not a runtime `PRAGMA journal_mode` probe: per-connection
// pragma state in modernc.org/sqlite can drift
...
// Either missing surfaces a wrapped error naming the feature.
...
// Settings key names used by RunBootIntegrityCheck.
// Exported so admin endpoints and the Dashboard card can
...
// so the boot sequence and the manual-trigger goroutine share
// exactly one code path for the PRAGMA + cache write + audit emit.
//
// This runs once post-migrations and before HTTP Listen — no ticker, no
// GC-piggyback. Failure disposition: log + cache + continue; always returns nil.
...
//
// source="manual" — called from the POST /admin/db/health/check
//
...
//
// Failure disposition: log + cache + continue. Any error
...
// returns a (status, durationMs) pair. The Dashboard card
// surfaces the failure via the destructive variant.
//
// Reader-pool invariant: uses db.Reader.Conn(ctx) with one
...
//
// DSN invariant: integrity_check is NOT added to
...
// last_manual_at row. Errors are logged but not returned — the boot path
// must never propagate; the manual path follows the
...
func writeIntegrityCache(ctx context.Context, settings *SettingsRepo, status string, duration time.Duration, source string) {
	...
	// response field (read when the in-process lease is
...
// ok=false → failed. source is echoed into details.source so the
