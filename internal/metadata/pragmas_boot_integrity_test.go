...
// pragma list. This test asserts the function executes
...
// the function still returns nil (log+cache+continue) and
...
// -----------------------------------------------------------------------------
// RunIntegrityCheckNow tests
//
// RunIntegrityCheckNow is the extracted shared core invoked by both
// RunBootIntegrityCheck (source="boot") and the manual POST handler
// (source="manual"). The manual branch writes an extra settings
...
// three existing cache rows. The POST handler relies on this key
...
func TestRunIntegrityCheckNow_ManualWritesLastManualAt(t *testing.T) {
	...
	// Audit event must carry source="manual" (asserted in integration
	// tests — the kind is .completed on healthy DBs).
	calls := rec.snapshot()
	if len(calls) != 1 {
		t.Fatalf("audit.Record called %d times, want 1; calls=%+v", len(calls), calls)
	}
...
// returns (status, durationMs) so callers (the manual-run goroutine) can
