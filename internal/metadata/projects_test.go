...
func TestProjectsRepo_CreateAndFind(t *testing.T) {
	...
	got, err := r.FindByName(ctx, "acme")
	if err != nil {
		t.Fatalf("find by name: %v", err)
	}
	if got.Name != "acme" || got.DescriptionMD != "desc" {
		t.Fatalf("unexpected: %+v", got)
	}
}

// TestProjectsRepo_CreateInTxRollsBackWithMembers pins that when CreateInTx +
// MembersRepo.AddInTx are composed inside a single writer tx, a failure in the
// second step must leave the DB with NO project row (no orphan). Pre-fix,
// Projects.Create and Members.Add were independent transactions and a failure
// in the second left an orphan project.
...
// TestProjectsRepo_SoftDeleteCascade pins cascade behavior at the ProjectsRepo
// level. Soft-deleting a project must also revoke every live S3 access key,
// soft-delete every live S3 bucket, and revoke every live project-owned
// api_key for that project — all in one tx. User-owned api_keys for that
// project's members are EXPLICITLY untouched (owner_kind filter).
...
// -----------------------------------------------------------------------------
// Projects.SoftDelete cascades FTS prune to every live repo; Projects.Restore
// reverse-cascades FTS reindex only for repos cascaded by THIS soft-delete
...
func TestProjects_SoftDelete_CascadesPruneFTS(t *testing.T) {
	...
	// TIMESTAMP-affinity normalization (read-back returns ISO-8601 even when
	// the WHERE-equality compare needs the raw stored bytes).
