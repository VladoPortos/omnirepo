...
// PyPIFile mirrors one pypi_files row. Kind is "wheel" or "sdist";
// ProjectNormalized is the PEP-503 normalized name used as the Simple index
// grouping key.
...
// contract on twine-legacy + PEP 694 uploads without disturbing the
// mirror-sync idempotent-upsert path (which calls Insert directly).
...
// Used by the drift-purge engine so the per-protocol adapter can compute
// drift in one query rather than iterating ListProjects + ListByProject.
