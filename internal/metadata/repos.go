...
type Repo struct {
	...
	CreatedAt       time.Time
	DeletedAt       *time.Time
	// GitMaxPushBytes — per-repo override for the global git push-size cap.
	// NULL → inherit cfg.repos.git.max_push_bytes. Only meaningful when
	// Type == "git"; populated by migration 017.
	GitMaxPushBytes *int64

	// Upstream mirror fields added by migration 024. Only meaningful when
	// Type ∈ {deb,rpm,pypi,helm};
	...
	// DriftPurge: opt-in per-mirror flag — when true, a successful mirror sync
	// soft-deletes local rows whose upstream key vanished. Default false on
	// upgrade (migration 035) to preserve v1.4 additive-only behaviour.
	// Editable via PATCH; mirror-only invariant enforced in handlePatchRepo.
	DriftPurge bool
}
...
//   - CHECK(type IN ('rpm','deb','pypi','docker','helm','git','raw'))
//   - CHECK(block_on_severity IN ('none','low','medium','high','critical'))
//   - UNIQUE(project_id, type, name)
//
// The app layer (bootstrap validator, api handlers) validates the same
// set before INSERT so bad input surfaces a typed Go error rather than the
// raw CHECK-constraint string.
//
// SoftDelete + Restore drive the FTS5 prune and reindex steps in the same
// WriteTx as the row UPDATE. The optional reindexer (set via WithReindexer)
// drives Restore; nil-reindexer is the test-time default (no reindex
// side-effect).
...
// commit related rows alongside the repo row (e.g. rpm/deb signing-key
// generation in the same writer tx as the repos INSERT).
...
// creation. IsMirror + UpstreamURL are immutable post-creation; FilterJSON +
// CredID + ScanOnSync can be edited via ReposRepo.Update. CredID may be nil
// (public upstream archives).
...
// WriteTx. The `WHERE deleted_at IS NULL` filter on the UPDATE makes the
// operation idempotent — a second SoftDelete is a no-op cascade (PruneRepoFTS
// itself is idempotent because its DELETEs match zero rows).
...
// Idempotent — already soft-deleted repos are skipped via the
...
// /v2/_catalog endpoint, cursor-paginated lexicographically (strictly
...
//
// MirrorFilterJSON / MirrorCredID / ScanOnSync are the three editable mirror
// fields — is_mirror + mirror_upstream_url are intentionally absent from
// UpdateFields so the schema cannot be mutated post-creation via this path;
// the API-layer PATCH validator enforces the same rule at the request
// boundary.
type UpdateFields struct {
	...
	// DriftPurge: nil = no change. Non-nil sets the per-repo drift_purge flag.
	// The mirror-only invariant (drift_purge=true requires IsMirror=true) is
	// enforced one layer up in handlePatchRepo, NOT here — direct callers must
	// pre-validate.
	DriftPurge *bool
}
...
//
// CAS files are NEVER touched here: blobs may be shared with
...
// metadata_state helpers. The regen goroutine transitions clean -> dirty on
// each package write, clean/dirty -> regenerating when it starts, and
// regenerating -> clean (or dirty + last_regen_error) on completion.
