...
// TestS3KeysRepo_RevokeAllForProject pins cascade behavior.
...
// TestS3KeysRepo_RestoreCascadedForProject pins the reverse
...
// TestS3KeysRepo_FindByAKID_DeletedProject pins the rule: a key whose
...
// so this test isolates the FindByAKID JOIN filter from the soft-delete cascade.
// (The cascade would also revoke the key — masking the lookup-hardening
...
func TestS3KeysRepo_FindByAKID_DeletedProject(t *testing.T) {
	...
	// Soft-delete the project via raw UPDATE (decoupled from the soft-delete cascade).
	if _, err := db.Writer.ExecContext(ctx,
...
// TestS3KeysRepo_FindByAKID_NoOraclePreserved pins the rule: all three failure
