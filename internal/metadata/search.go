// Package metadata — FTS5 UNION search.
//
...
// filtering AFTER the DB returns results.
//
...
// a parameterised ? binding — never string-interpolated.
package metadata
...
func (db *DB) SearchAll(ctx context.Context, p SearchParams) ([]SearchResult, error) {
	...
	// Per-arm severity/project filters. Severity only meaningfully
	...
	if p.Kind == "" || p.Kind == "cve" {
		...
		// matches PruneRepoFTS's conditional cves_fts prune chain).
	...
	// If a severity filter was requested but the query asked for a
