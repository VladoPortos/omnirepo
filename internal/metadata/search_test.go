...
func seedSearchData(t *testing.T, db *metadata.DB) {
	...
	// All 7 search arms INNER JOIN repos+projects + filter
	// `r.deleted_at IS NULL AND p.deleted_at IS NULL` as defense-in-depth.
	// Tests must seed projects + repos rows that match the FTS rowid/repo_id
	// keys, otherwise the JOIN drops every result.
	if _, err := db.Writer.ExecContext(ctx, `INSERT INTO projects(id, name) VALUES (1, 'acme')`); err != nil {
	...
	if err := db.WriteTx(ctx, func(tx *sql.Tx) error {
		// repos_fts
		if err := metadata.IndexRepo(ctx, tx, 1, "nginx-proxy", "acme", "Reverse proxy repo", "docker"); err != nil {
			return err
		}
		if err := metadata.IndexRepo(ctx, tx, 2, "infra-tools", "acme", "Internal tools", "git"); err != nil {
			return err
		}
		// artifacts_fts
		...
		// this the cve arm correctly drops the row.
...
// TestSearchAll_SeverityFilterCaseInsensitive guards that Trivy writes
...
// -----------------------------------------------------------------------------
// Every search arm filters `r.deleted_at IS NULL AND p.deleted_at IS NULL`.
// The cve arm uses an EXISTS subquery via the verified
// vulnerabilities → scans → repos → projects chain.
// -----------------------------------------------------------------------------
...
// TestSearchAll_CVEFilteredViaRepoChain — CVE shared between live repo r1
// and soft-deleted repo r2 surfaces; CVE owned only by soft-deleted repos
// drops out.
