...
// on the sessions table. modernc/sqlite serializes time.Time via Go's %v
// format (variable fractional-second width) which breaks lex comparison
// against CURRENT_TIMESTAMP at sub-second boundaries. Using the fixed-width
// layout for every write + passing the current-time as an explicit
// parameter to FindByPrefixSha (instead of CURRENT_TIMESTAMP) keeps the
...
//
// Comparing against CURRENT_TIMESTAMP would fail because stored values are
// now DBTimestampLayout ('T'-separated) while CURRENT_TIMESTAMP is
// space-separated — lex ordering diverges at position 10. Pass time.Now()
// formatted with DBTimestampLayout as an explicit bind instead.
...
// The UPDATE is a no-op if newExpires is not strictly later than the
// existing expires_at, so two concurrent touches cannot walk the cap
// backwards.
