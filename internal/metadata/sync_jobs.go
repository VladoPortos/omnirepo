...
// can never lease the same row. This method MUST be called
...
type SyncJob struct {
	...
	// = 0 is legal (Helm sync is step-based).
	ProgressBytes int64
	TotalBytes    int64
	CurrentStep   string
}
...
// owns the backoff policy; this method does not compute it.
...
// retries). Used once attempts >= retry cap.
...
// helm partial-sync live-path: the pool's
...
//
// Atomicity invariant: any reader observing status='failed' on
...
// this helper is the single SQL mutator.
//
// total == 0 is legal (Helm step-based progress). step may be
...
// the sync_jobs.summary JSON blob. Emitted when
...
// sync_jobs.summary JSON blob.
// Emitted unconditionally by each protocol sync_handler after a
// successful drift run (including count=0 — run evidence).
...
// to enforce one-in-flight-sync-per-repo with
// a 409 envelope (generalized to `mirror.sync.in_flight`).
//
...
// statements so the check+insert pair becomes atomic.
...
// repo. Used by internal/httpx/sync_rest.go to populate the
...
// shape to CountRepoInflightTx. Future kind additions must not tighten
// this filter.
func (r *SyncJobsRepo) GetInflightTx(ctx context.Context, tx *sql.Tx, repoID int64) (InflightJob, bool, error) {
	...
	// the same wedge we hit last time this bit us). Scanning into
...
// POSTs. SQLite serialises writer-pool
...
// supplied threshold back to 'pending'. Returns the
...
// introduced for boot recovery. Rather
// than re-pending stale 'running' rows, it terminates them at the
// caller-chosen terminalStatus ('failed' for helm_sync) and
// populates sync_jobs.log with the caller-supplied JSON in the SAME
// UPDATE statement (atomicity). The kind filter in the WHERE
// clause preserves the scope boundary: non-matching sync kinds
// retain the existing generic RecoverStale retry semantics.
//
// last_error is set to a stable sentinel ("stale running row
// terminated at boot") so drift-purge consumers can distinguish
// crash-path terminations from the live-
