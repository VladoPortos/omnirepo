...
// exactly one must win. Proves lease atomicity under -race.
...
// new column round-trips. Covers the path for the "Sync
...
// TestSyncJobs_GetInflightTx — the generalized 409
...
// TestSyncJobsRepo_CountRepoInflightTx — the tx-scoped variant lets
// callers run check+Enqueue atomically to
// eliminate the documented race. Asserts shape parity with
...
// --------------------------------------------------------------------------
// Helm partial-sync terminal writers.
// --------------------------------------------------------------------------

// TestSyncJobsRepo_MarkPermanentlyFailedWithLog proves the atomicity
...
// TestSyncJobsRepo_RecoverStaleByKind_HelmOnly proves the scope
...
func TestSyncJobsRepo_RecoverStaleByKind_HelmOnly(t *testing.T) {
	...
	// Pypi row: UNTOUCHED — scope boundary proof. This is the
...
// TestSyncJobs_SetSummaryDriftPurged covers the JSON-merge
...
func TestSyncJobs_SetSummaryDriftPurged(t *testing.T) {
	...
	// Zero-count is legal (run-evidence path).
