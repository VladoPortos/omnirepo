// Package deb — Release-aware pool-path resolution. Replaces the naive
// filename-inference heuristic in relPoolPath with a Release-file-first
// reader that honors whatever Components line a non-default-layout repo
// declares, falling back to the legacy
// "pool/main/<initial>/<pkg>/<filename>" shape when the Release file is
// missing, malformed, or declares a traversal-unsafe component name.
package deb
...
//     (traversal mitigation), fall back to the legacy filename-inference
//     path `pool/main/<initial>/<pkg>/<filename>`.
//
...
// cannot escape the pool/ tree. Traversal mitigation: reject any value
