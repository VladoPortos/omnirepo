...
// Budget guard for DEB uploads: a 4 GiB authenticated DEB upload must
// complete within a documented heap-allocation budget (~50 MB on top of
// pre-upload baseline) rather than allocating proportional to the artifact
// size. Forcing function for any future regression that re-introduces a
// `bytes.Buffer` for body bytes in put.go.
//
...
// TestDEBPut_MemoryBudget proves a 4 GiB authenticated DEB upload completes
// within a documented HeapAlloc budget (50 MB) rather than allocating
// proportional to the artifact size. The body is a real ar
