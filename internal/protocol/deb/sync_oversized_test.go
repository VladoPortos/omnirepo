...
// integration coverage for DEB): when an upstream serves cap+1 bytes for
// either an artifact body OR a metadata index, the full mirror sync flow
// must fail explicitly with an error whose Error() text contains the
// streamio.Err{Artifact|Metadata}TooLarge sentinel string AND commit zero
// new rows to deb_packages.
//
...
func newDEBOversizedFixture(t *testing.T, upstreamClient *http.Client) *debOversizedFixture {
	...
	pid, err := projectsRepo.Create(ctx, "pp", "STREAMIO-08 oversized deb")
	if err != nil {
...
// TestDEBSync_OversizedArtifactRejected proves STREAMIO-08 (audit #4) for
...
// TestDEBSync_OversizedMetadataRejected proves STREAMIO-08 (audit #5) for
