...
//
// Auth is disabled here — Plan 10 middleware runs BasicOrAPIKey upstream,
// so gitkit MUST NOT double-check (D-31).
package gitkit
...
func (s *Server) Handler(repoPath string) http.Handler {
	...
	cfg := gk.Config{
		Dir:        parent,
		AutoCreate: false, // repos are pre-created by Plan 12 repo-create hook
		Auth:       false, // Plan 10 middleware handles auth upstream (D-31)
	}
	inner := gk.New(cfg)
