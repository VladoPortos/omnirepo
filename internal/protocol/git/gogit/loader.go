// Package gogit is the primary Git Smart-HTTP backend.
//
// This file defines ProductionLoader, the transport.Loader used by the
// middleware to resolve /git/<project>/<repo>.git URL paths to storage.Storer
// instances rooted at the on-disk bare repo. The lookup is metadata-backed.
package gogit
...
//
// Security: the filesystem path is derived from the DB row
// (project.Name + repo.Name), NEVER by concatenating the client URL.
// Project and repo name validators reject path-traversal slugs
// at creation time.
...
// pair. Exported so the middleware can resolve the repoPath it needs
