...
// `lfs.not_supported` for BOTH mirror and non-mirror Git repos (scope is
// universal — go-git has no LFS client, so gating only mirrors would leave
// a silent-success path on dev repos where a pointer file could slip in and
// route clients to the upstream LFS host on clone).
//
...
// TestLFSBatch_DevRepo_AlsoReturns501 pins the "BOTH dev and mirror
...
func TestLFSBatch_DevRepo_AlsoReturns501(t *testing.T) {
	...
	if resp.StatusCode != http.StatusNotImplemented {
		t.Fatalf("status = %d, want 501 (applies to dev repos too); lastPath=%q", resp.StatusCode, rec.lastPath)
	}
	if rec.lastPath != "" {
