...
// --- Test 12: resolveRepoFromURL parses URL and stashes repo + permission ---
...
func TestResolveRepoFromURL_UploadPack(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/git/dxc/myrepo.git/info/refs?service=git-upload-pack", nil)
...
// --- Test 13: resolveRepoFromURL 404 on unknown project ---
...
func TestRequireGitPermission_MemberWrite(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/git/dxc/myrepo.git/git-receive-pack", nil)
	req = req.WithContext(gitpkg.WithRepo(ctx, repo))
	w := httptest.NewRecorder()
	mw(inner).ServeHTTP(w, req)
...
// --- Test 10-11: project-scoped API key actor ---
...
// --- Test 14: PerRepoMutex acquires lock on write ---
...
func TestPerRepoMutex_ReceivePackAcquiresLock(t *testing.T) {
	...
	key := storage.RepoKey{Project: "dxc", Type: "git", Repo: "myrepo"}
	mu := locks.For(key)
...
func TestPerRepoMutex_UploadPackNoLock(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/git/dxc/myrepo.git/git-upload-pack", nil)
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
