...
func TestPushCap_UnderLimit(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/git/dxc/myrepo.git/git-receive-pack", body)
...
// --- Test: sideband message includes literal D-34 text ---
...
func TestPushCap_SidebandMessage(t *testing.T) {
	...
	// Check that the message contains the D-34 literal format.
	respStr := w.Body.String()
	if !strings.Contains(respStr, "push exceeds repo limit of") {
		t.Fatalf("sideband message missing D-34 text: %q", respStr)
	}
...
func TestPushCap_UploadPackBypassed(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/git/dxc/myrepo.git/git-upload-pack", body)
	req = req.WithContext(gitpkg.WithRepo(req.Context(), repo))
	w := httptest.NewRecorder()
	mw(inner).ServeHTTP(w, req)
