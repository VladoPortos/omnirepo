// Package git — git mirror sync handler.
//
...
//     client.WithHTTPClient (Pitfall: CA bundle / proxy / TLS live on
...
//     Diff against prior rows to compute refs_updated.
//   - LFS detect:  scan HEAD tree for .gitattributes blobs containing
//     "filter=lfs"; emit EvtMirrorSyncLFSDetected audit-only.
...
//   - Progress:    step-based ("cloning"|"fetching"|"pruning_refs"
//     |"regenerating_index"|"done"), total_bytes always 0 since go-git
//     does not surface a reliable total.
//
// Mirrors always pull all refs with no filter — SyncPayload has no Filter field.
// LFS surfacing is audit-only — no UI badge.
//
// Security mitigations:
//   - Cred leak via config: client.WithHTTPAuth does NOT embed
//     :password@ in the stored remote URL; integration test grep-asserts
//     this post-clone.
//   - Error leak: httpx.SanitizeUpstreamErr wraps
//     every go-git return path before it reaches the caller.
//   - Hostile upstream cert: CA bundle configured on the
...
// This string is registered in the /sync allow-list and wired to the
// handler below via phase3_sync.wireSync.
const SyncJobKind = "git_sync"

// SyncPayload is the JSON shape stored on the sync_jobs row for git_sync.
// There is NO Filter — git mirrors always pull every ref.
...
type SyncDeps struct {
	...
	// TLS/CA bundle/proxy/timeout. Pitfall: configure those on THIS
...
func (h *SyncHandler) Handle(ctx context.Context, payload string, projectID, repoID, jobID int64) error {
	...
	// Atomic refs rewrite under a single writer tx.
	...
	// LFS detection. Non-fatal — sync succeeds regardless.
	...
	// EvtSyncFinished with refs_updated + objects_received.
	if h.deps.Audit != nil {
		_ = h.deps.Audit.Record(ctx, audit.Event{
...
// with a different target. Used for the refs_updated audit detail.
...
// hits are collected.
//
// Audit-only — no UI surfacing. An empty return means either
...
// onto the step-based progress convention. Best-effort parse —
...
// callers for live progress (the step-based progress is therefore "best-effort
...
// a progress row with step="fetching" + done=X + total=0.
