...
// This is a known deviation: there is no kind='basic' value in the schema.
...
// TestGitSync_Integration_CredsNotEmbeddedInConfig pins the
...
func TestGitSync_Integration_CredsNotEmbeddedInConfig(t *testing.T) {
	...
	// open the bare repo's config file and grep for the
	...
	if strings.Contains(cfg, password) {
		t.Fatalf("leak: password literal found in bare-repo config:\n%s", cfg)
	}
	if strings.Contains(cfg, ":"+password+"@") {
		t.Fatalf("leak: :password@ URL form found in bare-repo config:\n%s", cfg)
	}
	...
	if strings.Contains(cfg, "@"+hostFromURL(srv.URL)) {
		...
		if strings.Contains(cfg, username+":") || strings.Contains(cfg, username+"@") {
			t.Fatalf("leak: user-bearing remote URL form found in bare-repo config:\n%s", cfg)
		}
	}
}
