...
//
// Forward-only mirror. The reverse direction (traditional PUT →
// synthesised OCI manifest) is deferred to v1.2.
//
...
// succeeded and its 201 MUST NOT regress.
type Mirror struct {
	db         *metadata.DB
...
//  7. On tx failure: pathStore.Delete (rollback pattern).
...
func (m *Mirror) MirrorToTraditional(ctx context.Context, projectName, repoName string, chartTgz io.Reader) error {
	...
	if err := m.db.WriteTx(ctx, func(tx *sql.Tx) error {
		...
		// 7. Rollback: tx failed AFTER pathStore.Put succeeded, so
