...
// Helm chart mirror (plan 07-04 S-03b). Given a chart.tgz reader for
...
// Helm SDK chart loader already validates; this is defence in depth (T-07-04-01).
func TestMirrorToTraditional_RejectsUnsafeChartName(t *testing.T) {
	...
	// unit grep in the plan acceptance criteria.
...
// TestMirrorToTraditional_RollsBackFileOnDBFailure asserts the HI-02 pattern:
...
func TestMirrorToTraditional_RollsBackFileOnDBFailure(t *testing.T) {
	...
	// inside the WriteTx failure branch, and grep-verified in the plan's
	// acceptance criteria.
	tgz := makeChartTGZ(t, "foo", "0.1.0", "", "", nil)
	m := helm.NewMirror(f.db, f.charts, f.repos, f.pathStore(), f.registry)
