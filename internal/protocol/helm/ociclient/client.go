...
//
//   - ClientOptEnableCache(false) — air-gap / single-volume invariant.
...
//   - ClientOptWriter(io.Discard). Helm's Pull prints a stderr warning
//     line when it accepts the legacy
//     application/tar+gzip media type (see vendor/.../client.go:583).
//     We ACCEPT this media type silently; redirecting the writer
...
//
// Pitfall: the Helm SDK's Pull uses context.Background() internally at
// vendor/.../client.go:538 — caller ctx
...
type Client interface {
	...
	// legacy application/tar+gzip media type is accepted silently.
...
func (h *httpHelmClient) newSDKClient(creds AuthCreds) (*registry.Client, error) {
	opts := []registry.ClientOption{
		...
		registry.ClientOptEnableCache(false), // MANDATORY
		registry.ClientOptWriter(io.Discard), // silences legacy media-type warning
	}
	if creds.User != "" {
