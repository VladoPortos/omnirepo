...
//
//  1. ctx cancellation mid-flight: the jobs pool ticked down its
...
// ErrHelmPartialSync) and read counts via errors.As(err, &pse). internal/jobs
// consumes this via a narrow interface (no direct import) so the jobs pool
// can route partial syncs to the terminal-failed path.
