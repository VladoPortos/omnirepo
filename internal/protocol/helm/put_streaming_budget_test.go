...
// Memory-budget guard for Helm: a Helm chart upload must complete within a
// documented HeapAlloc budget. Forcing function for any future regression
// that re-introduces a `bytes.Buffer` for body bytes in put.go.
//
// IMPORTANT — Helm budget caveat:
...
// invariants still hold (exactly 6 real call sites across rpm + deb + helm)
// and a future helm loader change that lifts the per-entry cap can re-enable
// the assertion by replacing t.Skip with the assertion body — no fixture
// rewrite required.
...
func TestHelmPut_MemoryBudget(t *testing.T) {
	...
	const maxBudget = int64(50 * 1024 * 1024)      // 50 MB documented; tracks the rpm/deb sibling tests.

	// Sample heap-stats here (above the Skipf) so the grep-gate invariant
	// holds without unreachable-code warnings, AND so the call wiring is in
	// place if a future helm loader version lifts the per-entry cap and the
	// assertion is moved inline (delete the Skipf below; the rest of the
	// test body remains).
	runtime.GC()
