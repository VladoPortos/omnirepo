...
// Helm mirror drift-purge integration test.
//
...
// helm_charts table; drift logic is identical for both.
...
func newHelmDriftFixture(t *testing.T) *helmDriftFixture {
	...
	pid, err := projectsRepo.Create(ctx, "dp", "helm drift")
	if err != nil {
...
// TestHelmMirrorSync_DriftPurge_SkipOnFailedSync proves, for helm
// specifically, that a ctx-cancel or upstream error returns before drift,
...
func TestHelmMirrorSync_DriftPurge_SkipOnFailedSync(t *testing.T) {
	...
	rows, _ := metadata.NewHelmChartsRepo(f.db).ListByRepo(context.Background(), f.repoID)
	if len(rows) != 3 {
		t.Errorf("after failed sync: rows = %d, want 3", len(rows))
	}
...
func TestHelmMirrorSync_DriftPurge_EmptyUpstreamGuard(t *testing.T) {
	...
	rows, _ := metadata.NewHelmChartsRepo(f.db).ListByRepo(context.Background(), f.repoID)
	if len(rows) != 3 {
		t.Errorf("after empty-upstream sync: rows = %d, want 3 (guard)", len(rows))
	}
	if got := helmTrashCount(t, f.trashRoot, "helm_chart_drift"); got != 0 {
