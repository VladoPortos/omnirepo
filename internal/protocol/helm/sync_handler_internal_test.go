...
// SUT emits so tests can inspect the Details map. Used by
// TestHelmSync_Fail_AuditEmitsPartialDetails to verify fail() mirrors
// PartialSyncErr counts into EvtSyncFailed.
...
// TestHelmSync_Fail_AuditEmitsPartialDetails locks the contract: when
// fail() is handed a *PartialSyncErr, EvtSyncFailed.Details MUST mirror
// the persisted/expected counts into audit observability. The
...
// details on the audit row (error-type confusion). Only *PartialSyncErr
// satisfies errors.As(err, &pse).
