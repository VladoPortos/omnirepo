...
// Phase 8 Plan 06 / M6.4 — Helm fake-upstream integration test for the
// TRADITIONAL HTTP helm-index sync path (index.yaml + .tgz over HTTPS).
//
// Scope note: two other test files in this module cover RELATED but DISTINCT
// flows, both shipped in Phase 7:
...
//   2. Progress row reaches the Helm-specific final state (D-11):
...
func TestMirrorSync_Helm_Idempotent(t *testing.T) {
	...
	// Progress row final state — Helm-specific shape (D-11).
	var (
