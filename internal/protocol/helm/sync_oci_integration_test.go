...
// OCI pull integration tests.
//
// Covers the real OCI fetch path: dedup on
...
// deleted via Trash.Move with kind "oci_tag_rebound", an EvtOciTagRebound
// audit event MUST fire with the full details_json shape, and the
// replacement helm_charts row MUST carry the new digest. The final state
// MUST have exactly one active row for (repo_id, name, version).
func TestOCISync_TagRebound(t *testing.T) {
	...
	if len(rowsAfter) != 1 {
		t.Fatalf("helm_charts rows after rebound = %d; want 1", len(rowsAfter))
	}
	if rowsAfter[0].Digest != newDigest {
		t.Errorf("row digest after rebound = %q; want %q", rowsAfter[0].Digest, newDigest)
	}

	// Exactly one EvtOciTagRebound audit event, with the details shape.
...
// NEVER the upstream oci:// URL. Proves OCI-sourced charts are served over
// HTTP.
	...
	for _, v := range idx.Entries["nginx"] {
		for _, u := range v.URLs {
			if strings.HasPrefix(u, "oci://") {
				t.Errorf("regenerated URL = %q; want HTTP-scheme (not oci://)", u)
			}
		}
	}
}
...
// --- commit-first ordering on tag-rebound ---
...
// TestOCISync_TagRebound_CommitFirst — when an OCI tag is rebound to a new
// digest, the rebound side-effects (Trash.Move + EvtOciTagRebound audit)
// MUST fire AFTER the helm_charts upsert commits, not before. If the
// upsert fails (disk full, sql error), the prior file must remain on disk
// un-trashed and no rebound audit must be emitted — otherwise the audit
// log claims a replacement that never landed and the trash holder orphans
// the only good copy of the prior chart.
//
...
func TestOCISync_TagRebound_CommitFirst(t *testing.T) {
	...
	// invariant: audit log never claims a replacement that did not commit.
	// The commit-first reordering places audit emission AFTER the DB
	// write, so a failed upsert means no audit ever fires.
