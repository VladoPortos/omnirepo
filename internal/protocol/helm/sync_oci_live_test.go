...
// Live E2E for OCIHELM-08: pull a real Bitnami chart from Docker Hub.
//
...
// sync_oci_integration_test.go (plan 11-03) already cover the dedup,
// rebound, mixed-index, and regen-HTTP-urls behavior. What the live
// test uniquely proves is that:
//
//   • The real ociclient + Helm SDK can talk OCI to Docker Hub.
//   • Basic credentials flow through the wrapper into the SDK.
//   • Canonical AND legacy Bitnami media types (D-03) both parse without
//     the wrapper emitting noise (enforced by ClientOptWriter(io.Discard)
...
const (
	...
	// maintained, documented in 11-04-PLAN.md as the primary target.
	// Fallback (if nginx drifts): registry-1.docker.io/bitnamicharts/redis.
...
func TestLiveOCIBitnamiSync(t *testing.T) {
	...
	// oras-side semver filter (see 11-01-SUMMARY) — any parseable semver
	// tag is acceptable.
	...
	// Touch the plan-11-02 EntrySourceOCI constant to assert it is
	// importable in the live file. This keeps the live test wired to
	// the classifier symbol the non-live integration tests exercise —
	// a refactor that renames EntrySourceOCI will break the live test
	// compile alongside the hermetic ones.
