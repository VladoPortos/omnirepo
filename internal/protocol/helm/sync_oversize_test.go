...
// Helm mirror sync must fail explicitly with streamio.ErrArtifactTooLarge
// when an upstream serves cap+1 bytes for an artifact body.
...
func TestDownloadAndHash_OversizedUpstream_ReturnsArtifactTooLarge(t *testing.T) {
	t.Parallel()
	...
	got, size, dgst, err := downloadAndHash(context.Background(), srv.Client(), srv.URL, AuthCreds{})
	if err == nil {
