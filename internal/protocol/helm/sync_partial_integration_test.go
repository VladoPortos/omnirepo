...
// End-to-end integration test that drives two back-to-back syncs against a
// 3-chart upstream fixture. The first sync stops with an upstream 500 on
// chart #3; the second sync (fixture healed) converges to the same row-set a
// clean first-run would have produced.
//
// This test demonstrates the full chain — handler → PartialSyncErr →
// regen.Kick → FindByDigest cheap-skip — works end-to-end against a real
// upstream-shaped fixture.
//
// Scope boundary:
//   - Does NOT call jobs.Pool.markFailed; the pool's terminal-failed
//     routing is covered by its own unit tests. Here we assert ONLY the
...
// (parallel fixture, not parameterised). Chart #3 (postgres) is served by a
// handler that reads *failMode and returns HTTP 500 when true. Tests toggle
// failMode between syncs to drive the partial-then-converge scenario without
// any test-only hooks on SyncDeps.
//
...
// disk. The convergence assertion (index.yaml lists all 3 charts after the
// second sync) depends on this wiring.
//
...
// the chart-dispatch goroutine loop with real concurrency — locks
// filesAdded/mutex bookkeeping under parallel fetches.
...
// TestHelmSync_PartialThenConverges is the combined partial-then-converge
// proof. See file-level doc comment for the scenario.
...
// handles real concurrency alongside the partial-error return. The
// serialized path alone doesn't prove the filesAdded / mu.Lock() bookkeeping
// is race-clean under parallel fetches.
...
func runPartialThenConverges(t *testing.T, maxParallel int) {
	...
	// regen.Kick fired on partial — index.yaml exists on disk after
	...
	// triple matches the fixture — convergence.
	...
	// Convergence on disk: index.yaml after the second sync lists all
