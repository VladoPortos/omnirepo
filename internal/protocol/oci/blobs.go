...
// with ActionUpdateRepo (Phase 02-05 D-33 reserved ActionRepoRead; write
...
func (h *Handler) blobUploadPost(w http.ResponseWriter, r *http.Request) {
	...
	u := uuid.NewString()
	// 1h default session TTL; matches Phase 1 blob_uploads 1h ttl and keeps
...
func (h *Handler) blobUploadPut(w http.ResponseWriter, r *http.Request) {
	...
	// SCAN-12 race close: register digest BEFORE cas rename so GC can
...
// Sets Docker-Content-Digest (OCI-07).
...
// masks a successful state change (Phase 1 OQ-9 semantics).
