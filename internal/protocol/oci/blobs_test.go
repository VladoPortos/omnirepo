...
// against a sqlitetest DB + tmp data root. Used by every blob test.
...
func TestBlobUploadChunked_HappyPath(t *testing.T) {
	...
	if b.RefCount != 0 {
		t.Fatalf("ref_count=%d, want 0 (manifest PUT will ++)", b.RefCount)
	}
...
// TestBlobGet_UnknownDigest_DoesNotLeakFSPath is the FS-path-leak regression:
...
func TestBlobDelete_WhenReferenced_405(t *testing.T) {
	...
	// that references the blob (the manifest PUT will do this naturally).
...
// TestBlobMount_ForbiddenSourceRead_StillFallsBack: when the from-source
// repo EXISTS but the actor lacks read access on it, the mount path must
// NOT return 201 Created (which would allow cross-repo data leakage). The
// current implementation enforces the source-read auth check; if it ever
// flipped to a successful mount the actor would have copied bytes they
// aren't allowed to see. We expect 403 DENIED on the source-read check.
...
// TestBlobUploadSurvivesConcurrentGC is the concurrent-GC regression gate.
// Sequence:
...
func TestBlobUploadSurvivesConcurrentGC(t *testing.T) {
	...
	go func() {
		...
		// just-promoted blob is in the candidate set if the Start-before-CAS
		// ordering is broken.
	...
	if err := <-gcResult; err != nil {
		t.Fatalf("concurrent-GC regression: %v", err)
	}
	...
	if startIdx < 0 || casIdx < 0 || startIdx > casIdx {
		t.Fatalf("ordering violated in source: Start at %d, cas at %d", startIdx, casIdx)
	}
}
...
// TestBlobUpload_MalformedUUIDRejected is the malformed-UUID regression.
// chi's default {uuid} regex is a greedy [^/]+ match, so without the
// isUploadUUID check any non-UUID string (including traversal-flavored
// payloads) gets interpolated into filesystem paths. The handler must
// reject malformed UUIDs with 400 BLOB_UPLOAD_INVALID before touching
// sess.Lookup or the tmp path.
...
// --------------------------------------------------------------------------
// MirrorGuard rejects OCI blob POST/PUT on mirror-flagged repos.
// --------------------------------------------------------------------------
