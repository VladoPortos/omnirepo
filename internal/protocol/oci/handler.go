...
// Deps bundles the subsystems the /v2 handler needs. Fields for the blob
// and manifests/tags/catalog surfaces are declared here together so their
// Mount wiring can land without churning this constructor's shape.
type Deps struct {
	DB       *metadata.DB
	...
	// Blob dependencies. Keeping these concrete on the Handler struct makes
	// it the single source of truth for every wiring concern.
	...
	// Manifests + tags + catalog + cosign dependencies.
	...
	ScanKick func()
	// SeverityGate (optional): plugs in the block_on_severity check.
	...
	HMACSecret []byte        // 32 random bytes
	...
	// entry rather than r.Host, closing host-header injection in the
	// Docker bearer realm.
	...
	// MediaTypeHelmChartConfigV1 on a helm-type repo. Implementations fetch
	// the chart-content layer blob from the CAS and mirror it into the
	// traditional helm charts tree via helm.Mirror.MirrorToTraditional. A
	// nil HelmMirror is a no-op (the mirror simply does not run — OCI push
	// semantics are unaffected). Failure is logged by the caller; the OCI
	// push continues regardless.
...
// delegates to helm.Mirror.MirrorToTraditional. This stays an interface
// instead of a concrete type so the oci package stays free of a direct
// helm package import (which would introduce a cycle at the wiring site).
type HelmMirrorHook interface {
	Mirror(ctx context.Context, projectName, repoName, chartBlobDigest string) error
}

// SeverityGateFn is the block_on_severity hook signature. A nil handler
// field is treated as an always-allow no-op.
type SeverityGateFn func(ctx context.Context, repoID int64, digest string) error

// ManifestMaxBytes caps manifest body size. Pushing >4 MiB manifest JSON
// yields 413 MANIFEST_INVALID.
...
type Handler struct {
	...
	// preference to r.Host when non-empty.
	externalHostnames []string

	// helmMirror: post-commit hook. nil-safe — when unset, manifestPut's
	// helm-detection branch is a silent no-op.
	helmMirror HelmMirrorHook
}
...
func New(d Deps) *Handler {
	...
	// Default lifted from 64 MiB to 512 MiB so docker clients that fall
	// back to single-PUT push don't hard-fail on common base images
...
// Mount registers the /v2 subrouter on parent. Layout:
//
...
//
// The blob and manifests/tags/catalog routes register inside the guarded
// subroute (the Group that sits behind AnonymousReadOK + VerifyBearer).
// Doing so is mandatory — every /v2 sub-path must pass through both
// middlewares.
func (h *Handler) Mount(parent chi.Router) {
	...
	parent.Route("/v2", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// <project>/<type>/<repo> triple per the URL convention;
			// the handlers validate it into that triple and resolve the
			// repo via FindByTriple.
			//
			...
			// Wrap OCI write paths (blob uploads + manifest PUTs) in
			// MirrorGuard so mirror-flagged docker/helm-OCI repos reject
			// uploads with 403 repo.repo_is_mirror. {type} comes from chi
			// URL params so the basic MirrorGuard variant is sufficient here.
			...
			// Manifest routes. reference is either a tag or a
			...
			// Tag routes. DELETE is a mutating operation and must be gated
			// by MirrorGuard so mirror-flagged repos reject it with 403
			// repo.repo_is_mirror (GET /tags/list is a read and stays open).
