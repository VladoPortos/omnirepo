...
func TestProtectedRoute_WWWAuthenticateChallenge(t *testing.T) {
	...
	// Must include realm, service, AND scope. Without scope,
...
func TestProtectedRoute_ValidBearer_Passes(t *testing.T) {
	...
	// The /v2/_catalog endpoint is a real project-scoped listing.
	// Authenticated non-super-admin with no memberships and no public
	// repos visible → 200 with empty list.
...
// presents it. The verifier MUST reject with 401.
...
func TestAnonymousReadOnPublicRepo_Passes(t *testing.T) {
	...
	// chi has no route registered, so we expect 404.
...
// fall through, leaving VerifyBearer to 401. We target /v2/_catalog as a
// guarded route whose path flows through extractRepoFromV2URL.
//
// The contract we can prove here is: a private repo's public_read lookup
// returns (false, true), and that path yields 401 at the VerifyBearer
// stage. This is exercised by
...
func TestPrivateRepoPublicReadLookup_ReturnsFoundFalse(t *testing.T) {
	...
	// (Once the real route is registered, VerifyBearer in the middleware
	// chain would 401 instead. Both outcomes prove the anonymous branch
