// Package oci — /v2 manifest state machine.
//
// Invariants:
//
...
func (h *Handler) manifestPut(w http.ResponseWriter, r *http.Request) {
	...
	// If reference is digest form, it MUST equal mfDigest.
	...
	// Mirror Helm OCI chart pushes into the traditional
	...
	// mirroring for signed charts — breaking the forward-only mirror
...
func (h *Handler) manifestGetOrHead(w http.ResponseWriter, r *http.Request, writeBody bool) {
	...
	// Severity gate hook. No-op when nil. When the gate returns
...
func (h *Handler) manifestDelete(w http.ResponseWriter, r *http.Request) {
	...
	// A parse failure here used to be swallowed, leaving refs=nil
...
//     after commit.
//
// Behaviour matches the manifestPut tx body:
...
func (h *Handler) writeManifestWithRefcounts(
	...
	// Learn whether this digest is new in the repo BEFORE Insert (which
	// is a no-op when the (repo,digest) row already exists) so the refcount
	// delta below can be made idempotent.
	...
	if autoScan && h.scans != nil {
		// Skip enqueue for manifests Trivy can't scan (attestation
