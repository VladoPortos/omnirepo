...
// TestRefDeltaOnTagOverwrite is the critical gate.
...
// TestManifestRePush_DoesNotInflateRefCount is the re-push regression: pushing the
// same manifest to the same tag repeatedly (docker push retries, unchanged CI
// re-pushes) must NOT bump blob ref_count. incRefs previously ran on every PUT
// with no matching decRef, so re-pushes leaked refs and the blobs could never
// be GC'd.
func TestManifestRePush_DoesNotInflateRefCount(t *testing.T) {
	f := newManifestFixture(t, false)
	cfg := f.seedBlob([]byte("cfg-rp"))
	l1 := f.seedBlob([]byte("layer-rp"))
	body := buildManifest(cfg, l1)

	for i := 0; i < 3; i++ {
		resp := f.putManifest("latest", body)
		resp.Body.Close()
		if resp.StatusCode != http.StatusCreated {
	...
	for _, d := range []string{cfg, l1} {
		...
		if err != nil || b == nil {
			t.Fatalf("stat %s: b=%v err=%v", d, b, err)
		}
		if b.RefCount != 1 {
			t.Fatalf("%s ref_count=%d want 1 (re-push must not inflate)", d, b.RefCount)
		}
	}

	// And the manifest is still pullable after the repeated pushes.
	g := f.getManifest("latest")
	g.Body.Close()
	if g.StatusCode != http.StatusOK {
		t.Fatalf("get after re-push: %d", g.StatusCode)
	}
}
...
// TestManifestDelete_MalformedBodyRejected is the malformed-body regression. If a
...
// --------------------------------------------------------------------------
// MirrorGuard rejects OCI manifest PUT on mirror-flagged repos.
// --------------------------------------------------------------------------
