...
// identityClaims is the JWT payload: identity only, NO scope/access
...
func (h *Handler) resolveActor(ctx context.Context, actorID int64, kind string) (auth.Actor, error) {
	...
	case string(auth.ActorKindAPIKey):
		...
		case "project":
			...
			// Thread the minted role so viewer project keys cannot push:
			// ResolveMembership defaults an empty APIKeyRole to "maintainer".
			// NULL role = pre-v1.5 backfilled key → "maintainer".
			if k.Role != nil {
				a.APIKeyRole = *k.Role
			} else {
				a.APIKeyRole = "maintainer"
			}
		default:
