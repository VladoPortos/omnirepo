...
// TestPyPIDelete_TxFailure_FileAndRowIntact is the fault-injection
...
// 500 (the silent-discard regression guard).
...
// TestPyPIDelete_TrashMoveFailure_PropagatesError is the
// regression-guard. The silent `_, _ = h.trash.Move(...)`
// discard in pypi/upload_legacy.go's deletePackage was replaced with explicit error
...
//
// The DB row is gone — the tx committed fine. That is the atomic
// ordering (tx commits before trash.Move). The file remains at its original
// storage path, which is the documented trade-off (orphaned
...
func TestPyPIDelete_TrashMoveFailure_PropagatesError(t *testing.T) {
	...
	// THE REGRESSION-GUARD INVARIANT: a failing trash.Move
	// surfaces as HTTP 500. The earlier silent discard would have
	// returned 204 here.
	if resp.StatusCode != http.StatusInternalServerError {
	...
	// Atomic ordering side-effect: the DB row is gone (tx
