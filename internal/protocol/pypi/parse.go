...
// two layers disagree and the mirror path fails late. PEP 427 mandates
// lowercase but we've seen real mirrors serve uppercase for DOS-legacy
// reasons.
//
// The positional parts[1] is validated against PEP 440 (pep440.go) —
// malformed version slots now return an error rather than propagating a
// non-version string through the sync pipeline.
...
//
// Leftmost valid wins on ambiguity. If no boundary's right-hand side
// validates, the filename is malformed.
//
// Using LastIndex("-") — the earlier implementation — mis-attributed
// `1.0.0-rc1` style versions to the version slot as `rc1` and pushed the
// leading release segment into the name. The single-shot
// "first -<digit> wins" still mis-attributed adversarial shapes
...
func parseSdistFilename(base string) (string, string, error) {
	...
	for i := 1; i < len(stem)-1; i++ {
		...
		// -<digit> boundary. Leftmost valid wins.
	}
