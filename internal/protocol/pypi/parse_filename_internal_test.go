...
// mis-attributed to the version slot.
//
...
func TestParseSdistFilename(t *testing.T) {
	cases := []struct {
		...
		// Dashed pre-release suffix — the regression case.
		...
		// Multi-candidate scan with pep440.Validate gate.
		//
		// First boundary "f-" has candidate "2do-1.0.0" which
...
// TestParseWheelFilename exercises PEP 427 split — locks current
// behaviour so the sync_handler.go reroute doesn't regress wheel parsing
// silently.
func TestParseWheelFilename(t *testing.T) {
	cases := []struct {
		...
		// Case-insensitive extension match — isInstallableExt accepts
		// uppercase .WHL so the parser must too.
		{"REQUESTS-2.33.1-PY3-NONE-ANY.WHL", "REQUESTS", "2.33.1", false},
		{"foo-1.0.0-py3-none-any.Whl", "foo", "1.0.0", false},

		// Validate gate on parts[1] — malformed version slot now returns
		// an error (previously returned parts[0], parts[1] with no
		// version-shape check).
...
// TestIsSafeMirrorFilename — upstream-fed filenames reach
