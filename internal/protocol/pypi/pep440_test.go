...
// surface. Positive rows exercise every shape (canonical release, epoch,
// pre-release — canonical + dashed legacy form, implicit post, dev, local
// with dot and underscore separators). Negative rows pin the rejection
// boundary, including the motivating case `2do-1.0.0` that the old
// `parseSdistFilename` heuristic mis-attributed to the version slot.
//
// Error shape is locked to the canonical prefix
// `pypi: invalid PEP 440 version:` — callers wrap with their
...
func TestValidate(t *testing.T) {
	...
	cases := []struct {
		...
		// Positive: hyphenated pre-release (legacy form regression).
		{"1.0.0-rc1", false},
		...
		{"2do-1.0.0", true}, // motivating case
