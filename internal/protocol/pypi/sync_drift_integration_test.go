...
// PyPI mirror drift-purge integration test. Seed 3 wheels via a fake
// /simple/ upstream, enable repo.drift_purge, then drop one entry from the
// upstream response and re-sync — assert the missing wheel lands in trash
// with kind pypi_file_drift + the mirror.drift_purged audit event emits +
// the sync_jobs.summary.drift_purged int == 1.
...
func newPyPIDriftFixture(t *testing.T) *pypiDriftFixture {
	...
	const projName = "dp"
	const repoName = "r1"
	pid, err := projectsRepo.Create(ctx, projName, "pypi drift")
	if err != nil {
...
// TestPyPIMirrorSync_DriftPurge_SkipOnFailedSync proves a sync
...
func TestPyPIMirrorSync_DriftPurge_SkipOnFailedSync(t *testing.T) {
	...
	if got := pypiTrashCount(t, f.trashRoot, "pypi_file_drift"); got != 0 {
		t.Errorf("after failed sync: trash count = %d, want 0", got)
	}
...
// TestPyPIMirrorSync_DriftPurge_EmptyUpstreamGuard proves that when
...
func TestPyPIMirrorSync_DriftPurge_EmptyUpstreamGuard(t *testing.T) {
	...
	// sync_jobs.summary.drift_purged absent on skipped runs.
	if _, present := pypiSummaryDriftPurged(t, f.db, jobID2); present {
