...
// Mirror-sync skip-continue integration test.
//
// TestSyncHandler_SkipsMalformedFilename feeds the sync loop a PEP 691
...
// TestSyncHandler_SkipsMalformedFilename. Upstream serves one valid + one
// malformed filename; the malformed one must be skipped via
// EvtSyncFileSkipped without failing the sync, and the valid one must land
// in pypi_files + on disk.
func TestSyncHandler_SkipsMalformedFilename(t *testing.T) {
	...
	// "foo-2do" negative row). Sync must skip, not fail.
	...
	mux.HandleFunc("/packages/badpkg-2do.tar.gz", func(w http.ResponseWriter, r *http.Request) {
		// Parse-before-download invariant: the malformed filename
	...
	pid, err := projectsRepo.Create(ctx, "skippkg", "skip test")
	if err != nil {
	...
	// Assertion (b): parse-before-download — no package GET for the
	// malformed filename.
	if badFetched {
		t.Fatalf("malformed filename was fetched from upstream — parse-before-download regressed")
	}
