...
// integration coverage for PyPI): when an upstream serves cap+1 bytes for
// either an artifact body OR a metadata index (the /simple/<project>/ page),
// the full mirror sync flow must fail with an error whose Error() text
// contains the streamio.Err{Artifact|Metadata}TooLarge sentinel string AND
// commit zero new rows to pypi_files.
//
...
// value — the sanitised text preserves the streamio sentinel string. The
// errors.Is contract at the helper layer is covered by Plan 05-03's
// sync_oversize_test.go + upstream_parse_oversize_test.go.
...
func newPyPIOversizedFixture(t *testing.T, upstreamClient *http.Client) *pypiOversizedFixture {
	...
	pid, err := projectsRepo.Create(ctx, "pp", "STREAMIO-08 oversized pypi")
	if err != nil {
...
// TestPyPISync_OversizedArtifactRejected proves STREAMIO-08 (audit #4) for
// PyPI: when upstream returns cap+1 bytes for a .whl body, sync fails with
// an error whose text contains streamio.ErrArtifactTooLarge AND zero new
// pypi_files rows commit.
...
// TestPyPISync_OversizedMetadataRejected proves STREAMIO-08 (audit #5) for
// PyPI: when upstream returns cap+1 bytes for the /simple/<project>/ page,
// sync fails with an error whose text contains streamio.ErrMetadataTooLarge
// AND zero pypi_files rows commit.
