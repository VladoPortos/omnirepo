...
// Byte-level progress emission for PyPI sync.
//
// Pre-computes totalBytes from summed PEP 691 file.size entries, wraps
// per-file downloads with jobs.CountingReader. Test serves a fake Simple
// index + two .whl-shaped files and verifies the sync_jobs row carries
// advanced progress_bytes + a "done" or "pulling <filename>" step after
// sync completes.
