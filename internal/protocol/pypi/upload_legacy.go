...
// that already has a pypi_files row in this repo. PyPI semantics require the
// mutation to fail with 409 rather than silently overwriting a released
// artifact. The sentinel is returned from commitPyPIRow and matched at the
// call sites to emit a protocol-native envelope. The sync path uses
// PyPIFilesRepo.Insert directly (not commitPyPIRow) so idempotent re-mirror
// remains unaffected.
...
func (h *Handler) handleLegacyUpload(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "content":
			// Reject duplicate content parts. PEP 503 + twine send
			// exactly one content part per upload; a second would
			// silently overwrite tmpPath and leak the first temp file
			// (the deferred cleanup only handles the LATEST path).
			...
			_ = part.Close()
			// tf.Close() flushes the file. On disk-full / writeback
			// failure the close error surfaces here even if io.Copy
			// reported success. Treat as a write failure.
	...
	// Serialize pre-check → Put → commit → rollback for this (repo,
	// filename) pair. Without the lock, two concurrent first-uploads of
	// the same filename would both pass FindByFilename, both `PathStore.Put`
	// (last-rename-wins on disk), and then only one would commit a
	// pypi_files row — leaving the DB winner's digest pointing at the
	// loser's bytes. The lock is keyed by storageKey so different
	// filenames don't contend.
	unlock := h.lockUpload(storageKey)
	defer unlock()

	// Pre-check existence BEFORE PathStore.Put because pathstore.Put is
	// not content-addressed — it overwrites any prior blob at the same
	// storage key, and the rollback path's Delete would then unlink the
	// **winning** upload's on-disk file. With the storageKey mutex above
	// held, this read + the writer-tx commit below form one atomic
	// section per-(repo, filename).
	...
	// pattern that pulled the full body back into RAM.
	...
	if err := h.commitPyPIRow(r, res, parsed); err != nil {
		// Only delete the blob we just Put when the commit failed AND we
		// did not race into an existing row. The inner Tx-check fires only
		// on the narrow race where another upload committed between our
		// pre-check and our Insert — those bytes now belong to the winner,
		// so deleting would destroy their data. Plain commit failures (DB
		// error, etc.) still rollback.
		...
		// Roll back the on-disk artifact when metadata tx fails.
...
// (PyPI semantics make released artifacts immutable — returns
...
func (h *Handler) deletePackage(w http.ResponseWriter, r *http.Request) {
	...
	// Atomic delete ordering: stat first to set fileOnDisk (preserves
	// partial-state heal). Commit the DB tx BEFORE trash.Move so a tx
	// rollback never moves the file. The prior silent-discard of
	// trash.Move's error is replaced with explicit error propagation —
	// operator must see trash failures.
