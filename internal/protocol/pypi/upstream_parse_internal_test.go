...
// TestIsInstallableExt — F-07.4 (wt3). Only .whl / .tar.gz / .tgz / .zip
// are installable from a PEP 503 simple index in 2026. Mirror sync must
// skip legacy .egg / .exe / .msi entries upstream still serves for some
// projects so the inline version parser in sync_handler.go's collect
// pass doesn't mangle the "version" column.
