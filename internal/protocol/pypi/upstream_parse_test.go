...
// TestPyPIUpstreamTrailingSimpleIsIdempotent is the live-verification
// regression for the silent "0 files synced" bug.
//
// Operator-entered upstream URLs naturally end in `/simple/` — that's
// what PEP 503 documents and what the MirrorConfigSection placeholder
// hints at (`https://pypi.org/simple/`). Before the fix,
// ParseUpstreamSimpleIndex appended `/simple/` unconditionally so
// `https://pypi.org/simple/` became `https://pypi.org/simple/simple/`,
// which pypi.org answers as the (legitimate-shape) response for a
// nonexistent project literally named "simple" with an empty files list.
// The parser unmarshaled that happily and returned an empty project list;
// the sync handler then finished "done" with zero files, silently — same
// class as the APT filter.Suites drift (commit f11ff39): REST/upstream
// wire shape drifts from handler expectation, and the pre-existing unit
// tests (which pass an upstream like `srv.URL` — no trailing /simple/)
// never exercised the operator form that triggers the bug.
//
