...
// metadataAddMemberWithRole inserts a project_members row with an explicit role
// ("maintainer" | "viewer") so RBAC tests can exercise the maintainer gate.
func metadataAddMemberWithRole(db *metadata.DB, projectID, userID int64, role string) error {
	_, err := db.Writer.Exec(`INSERT INTO project_members(project_id, user_id, role) VALUES (?, ?, ?)`, projectID, userID, role)
	return err
}
...
// TestRawWrite_RequiresMaintainer checks that RAW PUT/DELETE must
// require the maintainer role, not bare project membership. A viewer-role
// member is denied (403); a maintainer succeeds. Before the fix, the role-blind
// actorIsProjectMember let any member write.
func TestRawWrite_RequiresMaintainer(t *testing.T) {
	f := newRawFixture(t)
	pid, _ := f.seedRepo("proj1", "raw1", false, false) // f.userID added as a (default-role=maintainer) member

	// Seed a second user as a VIEWER member of the same project.
	viewerLogin := "raw-viewer"
	viewerPw := "raw-viewer-password-1234567"
	vhash, err := auth.HashPassword(viewerPw)
	if err != nil {
		t.Fatalf("hash viewer pw: %v", err)
	}
	vuid, err := f.users.Create(context.Background(), viewerLogin, "v@example.com", vhash, false, false)
	if err != nil {
		t.Fatalf("create viewer user: %v", err)
	}
	if err := metadataAddMemberWithRole(f.db, pid, vuid, "viewer"); err != nil {
		t.Fatalf("seed viewer member: %v", err)
	}

	putAs := func(login, pw string) int {
		req, _ := http.NewRequest(http.MethodPut, f.srv.URL+"/proj1/raw/raw1/notes/x.txt", bytes.NewReader([]byte("data")))
		req.Header.Set("Authorization", f.basicAuth(login, pw))
	...
	// Viewer IS a member but NOT a maintainer → 403.
	if code := putAs(viewerLogin, viewerPw); code != http.StatusForbidden {
		t.Fatalf("viewer PUT status=%d want 403 (maintainer required)", code)
	}
	// Maintainer → 201.
	if code := putAs(f.login, f.password); code != http.StatusCreated {
		t.Fatalf("maintainer PUT status=%d want 201", code)
	}

	// DELETE as viewer is also denied.
	dreq, _ := http.NewRequest(http.MethodDelete, f.srv.URL+"/proj1/raw/raw1/notes/x.txt", nil)
	dreq.Header.Set("Authorization", f.basicAuth(viewerLogin, viewerPw))
	...
	if dresp.StatusCode != http.StatusForbidden {
		t.Fatalf("viewer DELETE status=%d want 403", dresp.StatusCode)
	}
}
...
// TestRawGet_CrossProjectPrivateBlocked guards against a cross-project leak. Before
