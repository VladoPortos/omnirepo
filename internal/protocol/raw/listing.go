...
// listingItem is the JSON-encoded shape of one directory entry.
...
// listDir serves a directory listing for absDir. Content negotiation:
// Accept: application/json → JSON array; default → minimal HTML.
//
...
func (h *Handler) listDir(w http.ResponseWriter, r *http.Request, _ resolved, absDir string) {
	...
	// Minimal HTML — no styling; rich UI rendering is the web UI's concern.
