...
// for testdata/sample.rpm. The handler enforces a filename-matches-NEVRA
// guard, so every test upload path must use this exact name or the handler
// will 400 with filename_mismatch.
...
// TestRPMPut_RejectsFilenameNEVRAMismatch pins the rule that when the URL
// filename does not match the RPM header's NEVRA, the put handler must
// reject with 400 filename_mismatch before anything touches disk.
// Primary.xml builds <location href> from canonicalFilename(), so
...
// --------------------------------------------------------------------------
// MirrorGuard rejects RPM uploads on mirror repos.
// --------------------------------------------------------------------------
