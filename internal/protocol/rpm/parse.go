...
// MarshalFiles encodes the parsed file index as a JSON array for persistence
// on the rpm_packages row (H-7), so regen can rebuild filelists.xml. Best-
// effort: returns "[]" on error — an upload must not fail over this metadata.
func MarshalFiles(files []File) string {
...
// UnmarshalFiles reverses MarshalFiles for the regen path. Returns nil on a
// malformed/empty blob (filelists.xml then just omits files for that package,
// matching the pre-H-7 behavior rather than failing the whole regen).
func UnmarshalFiles(s string) []File {
	if s == "" {
		return nil
	}
	var out []File
