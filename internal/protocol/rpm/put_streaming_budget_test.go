...
// Plan 05-04 STREAMIO-07 (audit finding #3 budget guard): a 4 GiB
// authenticated RPM upload must complete within a documented heap-allocation
// budget (~50 MB on top of pre-upload baseline) rather than allocating
// proportional to artifact size. This is the on-merge forcing function for
// any future regression that re-introduces a `bytes.Buffer` for body bytes
// in put.go — the assertion will fail the moment HeapAlloc drift crosses
...
// TestRPMPut_MemoryBudget proves STREAMIO-07: a 4 GiB authenticated RPM
...
// in the bytes package just for the reader (avoids the grep-gate noise of
// adding a `bytes.Buffer`-adjacent token to a streaming-correctness test
// file).
