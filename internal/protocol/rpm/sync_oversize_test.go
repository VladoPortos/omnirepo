...
// Plan 05-03 STREAMIO-06 (audit #4): rpm mirror sync must fail explicitly
// with streamio.ErrArtifactTooLarge when an upstream serves cap+1 bytes
// for an artifact body. Before this plan, the helper used
// io.ReadAll(io.LimitReader(...)) which silently truncated to cap bytes —
// committing a hash-correct-but-truncated artifact. This test enforces
// the new sentinel behavior at the downloadAndHashWithProgress layer
// (package-private — internal test).
