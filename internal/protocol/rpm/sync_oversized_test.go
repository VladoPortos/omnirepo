...
// End-to-end integration coverage: when an upstream serves cap+1 bytes for
// either an artifact body OR a metadata index, the full mirror sync flow
// must fail explicitly via streamio.Err{Artifact|Metadata}TooLarge AND
// commit zero new rows to rpm_packages. This is the on-merge forcing
// function for any future regression that re-introduces the
// silent-truncation idiom (a bare io.LimitReader without a sentinel)
// anywhere in the rpm sync path.
//
// sync_oversize_test.go already wires the helper layer
...
func newRPMOversizedFixture(t *testing.T, upstreamClient *http.Client) *rpmOversizedFixture {
	...
	pid, err := projectsRepo.Create(ctx, "pp", "oversized rpm")
	if err != nil {
...
// TestRPMSync_OversizedArtifactRejected proves that when upstream returns
// cap+1 bytes for an .rpm body, sync fails via streamio.ErrArtifactTooLarge
// propagated through %w wrapping AND zero new rpm_packages rows are committed.
func TestRPMSync_OversizedArtifactRejected(t *testing.T) {
	...
	// through internal/httpx.SanitizeUpstreamErr which deliberately drops
	// the wrap chain via errors.New(scrubbed) to prevent credential leakage
	// from upstream-error byte strings. errors.Is therefore CANNOT walk back
	// to streamio.ErrArtifactTooLarge through Handle's return value. The
	// sentinel-propagation contract for the rpm sync helper layer is covered
	// by sync_oversize_test.go (internal test: downloadAndHashWithProgress →
	// errors.Is(err, streamio.ErrArtifactTooLarge) at the unsanitized
	// boundary). Here we assert the integration invariants:
...
// TestRPMSync_OversizedMetadataRejected proves that when upstream returns a
// primary.xml.gz that decompresses to cap+1 bytes, sync fails via
// streamio.ErrMetadataTooLarge AND zero rpm_packages rows commit.
