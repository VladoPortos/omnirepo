...
// primary.xml bytes from the upstream blob. Different
...
func ParseUpstream(
	...
	// Dispatch by suffix; gz | xz | uncompressed.
	dec, err := openPrimaryReader(primaryHref, primaryRaw)
	if err != nil {
		return 0, fmt.Errorf("rpm upstream: open primary: %w", err)
	}
	// Fail-explicit on cap+1 instead of silently truncating (a full-buffer
	// read through a LimitReader on the decompressed stream). Cap layer is
	// post-decompress so a small compressed body that decompresses to cap+1
	// is rejected.
	...
	// A failed Close on a decompressing reader indicates the upstream
