...
// Plan 05-03 STREAMIO-06 (audit #5): rpm upstream_parse must fail-explicit
// with streamio.ErrMetadataTooLarge whenever an upstream serves cap+1
// bytes for the raw HTTP body (repomd / primary.xml.gz) OR for the
// gunzipped primary.xml stream. Before this plan the helper used
// io.ReadAll(io.LimitReader(...)) which silently truncated metadata to
// cap bytes — committing a partial mirror with packages dropped.
