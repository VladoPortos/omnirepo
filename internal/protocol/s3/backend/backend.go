...
//   - Bucket provisioning is REST-driven. gofakes3's
...
// S3 buckets live in a global namespace so one sentinel string is
...
// MultipartBackend assertion lives in multipart.go.
...
// PutObject SHA intercept. The intercept stages the inbound body to a temp
// file here before either rejecting on SHA mismatch or forwarding the
// verified bytes to gofakes3 — the directory is created lazily by the
// intercept on first use.
func (b *Backend) TmpRoot() string { return b.tmpRoot() }

// BucketRoot exposes the on-disk bucket directory. Used by the chi-side
// PutObject intercept's destructive-overwrite test to assert that a rejected
// PUT does not touch the dst path. Production callers continue to use the
// unexported bucketRoot via PutObject / GetObject — this accessor exists
// strictly for test-side path resolution.
...
// bucketNameRe matches the AWS-subset bucket-name regex.
var bucketNameRe = regexp.MustCompile(`^[a-z0-9][a-z0-9.\-]{2,62}$`)

// validateBucketName enforces: length 3..63, lowercase alnum + dot +
...
// project is soft-deleted. The two filters are independent —
...
// owning project is soft-deleted. This is the public entry point that the
// middleware uses for auth.Can dispatch.
...
// use the REST API. Production wiring keeps DefaultProjectID=0 and
...
//
// A mkdir failure AFTER the DB row committed used to leave a bucket row
// with no on-disk directory — unreachable from subsequent object writes
// but blocking name reuse. We now compensate: if mkdir
...
// BucketExists is lock-free by design (concurrent with PutObject).
...
//
// Defensive filter: even though the REST upstream gates this endpoint on
// the project being live, the JOIN to projects + `p.deleted_at IS NULL`
// ensures a stale projectID for a soft-deleted project returns an empty
// list rather than leaking buckets. Belt-and-braces.
//
...
// or owning project soft-deleted.
...
// upstream metadata didn't include it. gofakes3's PutObject
