...
type fixture struct {
	...
	// fixtureCreateMPU. Lazily populated on first multipart
...
// TestCreateBucket_MkdirFailureCompensatesDBRow pins the compensation: when
...
// Every HeadObject (and GetObject) response MUST carry
...
// -- lookup-hardening tests ------------------------------------------------
//
// All seven tests below use a raw UPDATE on `projects.deleted_at` rather than
// going through ProjectsRepo.SoftDelete — the soft-delete cascade would mask
...
// filter that's already in place). We need to test the
// `p.deleted_at IS NULL` JOIN filter in isolation.

// rawSoftDeleteProject soft-deletes via raw SQL, decoupled from the
// soft-delete cascade. Used only by lookup-hardening tests in this file.
...
// TestBackend_FindBucketID_DeletedProject pins that soft-deleting
