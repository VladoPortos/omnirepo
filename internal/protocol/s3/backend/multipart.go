...
//
// Part-size bounds: UploadPart rejects > 5 GiB at the gate; the
...
// chi-side interceptCreateMultipartUpload middleware. The chi intercept
// hijacks `?uploads` POST requests
...
// `?uploads` POST route. The shim returns gofakes3.ErrInternal
...
// SweepOrphanMultiparts during boot recovery) call
...
// with AWS-spec pagination semantics.
//
...
//     and just logs the outcome;
//   - admin handler at POST /api/v1/admin/maintenance/sweep-multipart
//     surfaces them in the JSON response.
//
// One function — no parallel WithCounts variant; a grep gate enforces this
// contract. Caller computes the cutoff from cfg.S3.MultipartRetention
...
func (b *Backend) SweepOrphanMultiparts(ctx context.Context, cutoff time.Time) (swept int, cleanedDirs int, err error) {
	...
	for _, up := range stale {
		...
		// Thread the ctx through abort so app-shutdown
...
// (matchesPrefix was removed — prefix filtering moved into
