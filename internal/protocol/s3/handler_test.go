...
func setupTestFixture(t *testing.T) *testFixture {
	...
	deps := &s3handler.Deps{
		...
		Backend:   be,
		Skew:      15 * time.Minute,
		Hostnames: []string{"omnirepo.example.com"},
	}
...
// Test 5: Bearer token (not SigV4) → 403 InvalidAccessKeyId.
...
func TestS3Handler_VHostRewrite(t *testing.T) {
	...
	// Build request to /file.txt with Host: mybucket.omnirepo.example.com
	req, _ := http.NewRequest(http.MethodGet, f.server.URL+"/file.txt", nil)
	vhost := "mybucket.omnirepo.example.com"
...
// Test 10: Path-style with Host: omnirepo.example.com → no rewrite.
