...
// Plan 02-04 Task 1 — interceptCreateMultipartUpload middleware tests.
//
// Coverage matrix (per <behavior> in 02-04-PLAN.md):
//
...
// (joined to projects WHERE deleted_at IS NULL — LIFECYCLE-06). The bucket
// that newInterceptEnv created on disk has no DB row by default, so we add
