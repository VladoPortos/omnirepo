...
network fetch (constraint from `CLAUDE.md`: "zero outbound network calls at
runtime"; we extend that to zero outbound calls at test time).
...
## Test cases vendored (Phase 04-01 minimum set)
...
to keep the testdata footprint minimal; add selectively if Plan 04 or
Plan 06 uncovers a missed edge case.
