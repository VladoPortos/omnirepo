...
// MUST return ErrInvalidAccessKeyId for BOTH missing and revoked keys
...
type VerifyResult struct {
	...
	// the content-sha256 check; multipart paths ignore it.
...
//  2. Reject if Service != "s3" (region-agnostic but service-strict).
...
func dispatchBody(r *http.Request, cs, secret string, parsed *parsedAuthz, amzDate string) (BodyMode, string, error) {
	...
	default:
		...
		// the S3 handler hashing as it streams.
		if len(cs) != 64 {
