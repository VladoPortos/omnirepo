...
// Over-cap → MarkPermanentlyFailed with "too many findings".
...
func (h *Handler) Handle(ctx context.Context, scan *metadata.Scan) error {
	...
	case "docker":
		// Classify before materializing so we never hand Trivy a body
	...
	case "rpm", "deb", "pypi", "helm":
		// Package-style artifacts. Materialize the archive (and best-
...
// sanitize strips local filesystem paths from s.
