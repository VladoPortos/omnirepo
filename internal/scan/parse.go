...
// byte slice: treat empty stdout as a failure, not "no vulns".
...
// fields at any level are tolerated (DisallowUnknownFields = false).
...
// and Terraform when those are present in the scan root.
...
// break OmniRepo. Snapshot tests under testdata/trivy/ lock
...
func ParseTrivyJSON(b []byte) (Result, error) {
	...
	for _, block := range r.Results {
		...
		// Helm chart scans (and any other misconfig-heavy protocol)
		...
		for i, m := range block.Misconfigurations {
			...
			summary[sev]++
			// Trivy always emits ID today, but guard against a
