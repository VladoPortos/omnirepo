...
// --offline-scan --skip-db-update; a non-conforming impl is a defect.
package scan
...
// Runner is the stable façade over Trivy. All methods are safe to call
...
type Runner interface {
	...
	// Filesystem scans an extracted directory tree (used for RAW + the
	// package protocols that materialize package contents to disk).
	...
	// Callers MUST validate outPath is inside the sboms data root.
...
// rows. Summary is always initialized with all 5 severity keys present
// (value 0 when absent) so downstream code never hits map-miss edge cases.
type Result struct {
