...
// exec.CommandContext with an argv slice (no shell interpolation) and
// includes the air-gap flags.
...
// baseFlags returns the mandatory flag set. Centralized so the air-gap
// invariant is impossible to drop accidentally — grep tests assert these
// appear in EVERY exec call path.
//
// Scanners: vuln + secret + misconfig. Misconfig was added so Helm chart
// scans surface Kubernetes misconfigurations (e.g., containers using images
// from untrusted registries). Vuln stays for OS-package + language-lockfile
// CVEs; secret stays for the existing RAW pathway.
func (t *trivyRunner) baseFlags() []string {
	return []string{
		...
		// The misconfig scanner fetches its checks bundle (trivy-checks) from
		// a remote OCI registry (ghcr.io) on first use; --offline-scan does
		// NOT cover that. --skip-check-update is the dedicated air-gap flag
		// (per Trivy's air-gap guide) and must sit alongside the other
		// --skip-*-update flags or the first misconfig scan reaches out.
		"--skip-check-update",
