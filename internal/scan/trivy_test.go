...
func TestTrivyRunnerImageInvokesRequiredFlags(t *testing.T) {
	...
	// Every token REQUIRED by D-22 + the image subcommand path.
	required := []string{
		"image",
		"--input",
		ociDir,
	...
	// T-02-03-02: --insecure must never leak into argv.
...
func TestTrivyRunnerFilesystemInvokesFsSubcommand(t *testing.T) {
	...
	for _, tok := range []string{"--offline-scan", "--skip-db-update", "--cache-dir", "/cache"} {
