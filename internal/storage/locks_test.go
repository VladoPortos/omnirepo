...
func TestLocksSamePointerForSameKey(t *testing.T) {
	l := storage.NewLocks()
	k := storage.RepoKey{Project: "dxc", Type: "docker", Repo: "x"}
	m1 := l.For(k)
	m2 := l.For(k)
	if m1 != m2 {
...
func TestLocksDifferentPointersForDifferentKeys(t *testing.T) {
	l := storage.NewLocks()
	a := l.For(storage.RepoKey{Project: "dxc", Type: "docker", Repo: "x"})
	b := l.For(storage.RepoKey{Project: "dxc", Type: "rpm", Repo: "x"})
	if a == b {
