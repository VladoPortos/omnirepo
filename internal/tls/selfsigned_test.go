...
func TestGenerateSelfSigned_BasicFields(t *testing.T) {
	certPEM, keyPEM, err := GenerateSelfSigned(
		[]string{"omnirepo.dxc.local", "artifacts.dxc.local", "myhost", "localhost"},
		2*365*24*time.Hour,
...
func TestSelfSignedSANContainsLocalhostAndIPs(t *testing.T) {
	certPEM, _, err := GenerateSelfSigned(
		[]string{"omnirepo.dxc.local", "artifacts.dxc.local", "myhost"},
		time.Hour, 2048,
	)
	if err != nil {
		t.Fatalf("generate: %v", err)
	...
	dnsExpected := map[string]bool{
		"omnirepo.dxc.local":  false,
		"artifacts.dxc.local": false,
		"myhost":              false,
		"localhost":           false,
	}
