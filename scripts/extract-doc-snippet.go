...
//
// The canonical `scheduled-sync` fixture is embedded in the docs. Used by
// `make lint-docs` to pipe the documented bash snippet into
// shellcheck, so the doc IS the lint fixture — no sidecar script.
//
// The scanner is deliberately line-based rather
...
// the canonical fixture deliberately avoids it), that nested closer will close
