// Phase 3 air-gap extension (D-32): probes the four new package-protocol
...
// SYNC-05 unreachable-upstream path with a deterministic failure bound:
...
// the per-test budget regardless of CI network-stack quirks (T-03-07-04).
//
...
// the CI job wrapper (Phase 1 invariant).
package airgap
...
// TestPhase3RoutesAirGap boots the app on loopback with all Phase 3
...
func TestPhase3RoutesAirGap(t *testing.T) {
	...
	// generation, D-02) actually fire — bootstrap inserts repos rows
	...
	// cookie for the four repo creates and the SYNC-05 enqueue below.
	sessionCookie := loginAdminViaREST(t, client, httpBase, adminLogin, adminPassword)

	// Enroll the super-admin as a project member so the SYNC-05 enqueue
	...
	// (eager signing-key generation, D-02) actually fire. Each call must
	...
	// 3. SYNC-05 unreachable-upstream path. Enqueue a helm sync against the
...
// cookie. The cookie is reused for the four repo creates and the SYNC-05
