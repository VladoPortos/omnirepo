...
// TestLifecycleConformance is the LIFECYCLE-11 end-to-end gate. One in-process
...
// denies, then RESTORE → assert every surface works again (D-21 regression
// guard).
//
...
//     exact code documented in CONTEXT D-20 step 7); REST API key returns
//     401 with the existing auth.unauthenticated envelope; search returns
//     zero results.
//   - Phase C (AfterRestore_*): every surface MUST work again. This is the
//     D-21 regression guard — proves Restore actually un-cascades, not
...
func TestLifecycleConformance(t *testing.T) {
	...
	t.Run("BeforeSoftDelete_SearchFindsAllPackages", func(t *testing.T) {
		// POSITIVE SANITY ASSERTION (W6): require ≥1 result whose Name
	...
	t.Run("AfterSoftDelete_RESTAPIKey401", func(t *testing.T) {
		...
		if status != http.StatusUnauthorized {
			t.Fatalf("post-delete REST status=%d, want 401 (LIFECYCLE-07 + 01-01 cascade)", status)
		}
	})
	...
	t.Run("AfterSoftDelete_SearchAsAdminFindsZero", func(t *testing.T) {
		for _, q := range []string{
			...
			results := searchAsAdmin(t, fx, q)
			if len(results) != 0 {
				t.Fatalf("post-delete search %q: got %d results, want 0 (LIFECYCLE-08 + LIFECYCLE-09)",
					q, len(results))
			}
		}
	})
	...
	// =========================================================================
	// PHASE C — after restore: every surface MUST work again (D-21 symmetry).
	// =========================================================================
	...
	t.Run("AfterRestore_SearchFindsAllPackages", func(t *testing.T) {
		for _, q := range []string{
			...
			results := searchAsAdmin(t, fx, q)
			if len(results) == 0 {
				t.Fatalf("post-restore search %q: 0 results, want ≥1 (LIFECYCLE-09 reindex)", q)
			}
		}
	})
}
...
// the denial set documented in CONTEXT D-20 step 7. The exact code depends
// on which gate fired first:
//
//   - InvalidAccessKeyId — the SigV4 verifier saw FindByAKID return
//     ErrS3AccessKeyNotFound (LIFECYCLE-05 hardening from plan 01-02 OR the
//     plan 01-01 cascade — either way, the key collapses to "missing").
//   - NoSuchBucket — the bucket lookup saw the project-soft-delete filter
//     and returned no row (LIFECYCLE-06 hardening from plan 01-02).
