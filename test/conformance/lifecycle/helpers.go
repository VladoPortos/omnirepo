...
// SearchResult mirrors internal/api/search.go searchResultItem (verified at
// planning time — JSON tags pinned exactly to the canonical envelope, NOT
// discovered at runtime). Mismatching this shape would silently decode an
// empty slice and produce false-positive "0 results" assertions even when
// the search filter is broken — TestLifecycleConformance.BeforeSoftDelete_
// SearchFindsAllPackages forces a positive sanity assertion that catches
// envelope decode bugs before the post-soft-delete zero-result test can
// mask them.
...
// fixture for LIFECYCLE-11. Mirrors test/conformance/s3/helpers.go.
...
// PruneRepoFTS drops the FTS rows in the cascade plan-01-03 added).
//
...
// (verified at planning time, see <interfaces> in 01-05-PLAN.md). deb_packages
// uses PRAGMA table_info('deb_packages') at runtime to discover NOT NULL
// columns + a defensive INSERT — required because deb_packages references
// apt_suites.id (FK) and the apt_suites row must be inserted first. PRAGMA
// is the W5 schema-drift-proof escape hatch (deterministic).
...
// searchResponse envelope pinned at planning time (items + next_cursor) —
// see internal/api/search.go:106-109. Mismatch here would mask bugs as
...
// After soft-delete, FindByPrefixSha returns ErrNotFound → 401 (the LIFECYCLE-07
// hardening from plan 01-02 is what makes this work).
