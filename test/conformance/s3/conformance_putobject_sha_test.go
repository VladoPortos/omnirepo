...
// aws-sdk-go-v2 conformance smoke for the chi-side PutObject SHA enforcement.
//
...
// XAmzContentSHA256Mismatch. This is exactly the attack shape we guard
// against (signed SHA-A but sent SHA-B with internal client-side coercion).
...
func TestS3PutObjectSHAMismatch_Conformance(t *testing.T) {
	...
	// retryMax=1 so the bare error is observable, matching the
	// negative-test pattern in conformance_test.go.
