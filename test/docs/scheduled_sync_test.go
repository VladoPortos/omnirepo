// Package docs_test — Phase 10 Plan 05 CRONDOCS-01..03 structural
// assertions over docs/operations/scheduled-sync.md.
//
// The doc IS the test fixture for the plan 10-06 shellcheck extractor:
...
// missing (CRONDOCS-01 requires it to exist).
...
// TestScheduledSyncDocStructure asserts every D-17..D-22 invariant
// that reduces to a string-presence check against the doc body.
func TestScheduledSyncDocStructure(t *testing.T) {
	body := readDoc(t)

	// Required substrings, keyed by the invariant / decision they back.
	cases := []struct {
		name string
		want string
	}{
		{"D-22 shellcheck-id comment", "<!-- shellcheck-id: scheduled-sync -->"},
		...
		{"D-21 backoffLimit 0", "backoffLimit: 0"},
		{"D-21 curlimages/curl image", "curlimages/curl"},
		{"D-21 Secret-mounted key", "secretKeyRef:"},
		{"D-18 real upstream URL", "archive.ubuntu.com"},
		{"D-17 protocol substitution: rpm", "REPO_TYPE=rpm"},
		{"D-17 protocol substitution: pypi", "REPO_TYPE=pypi"},
		{"D-17 protocol substitution: helm", "REPO_TYPE=helm"},
		{"D-17 protocol substitution: docker", "REPO_TYPE=docker"},
...
// TestScheduledSyncDoc_ExactlyOneShellcheckTag guards the plan 10-06
...
// break plan 10-06's line-based extractor.
