...
//
// Exercises the S3 protocol against a *live* omnirepo process started
// out-of-band (no in-process boot). Mirrors the conformance suite but
// targets the real HTTP port and uses a provisioned bucket/key/secret.
//
// Run with:
//
...
func loadEnv(t *testing.T) liveEnv {
	...
	if e.endpoint == "" || e.bucket == "" || e.akid == "" || e.secret == "" {
		t.Skip("live S3 test requires OMNI_S3_{ENDPOINT,BUCKET,AKID,SECRET}")
	}
...
// by TestLiveS3_Multipart6MiB. Confirms DeleteObject's synchronous unlink
// (no trash path for S3).
...
// TestLiveS3_SeedDemoObjects populates the test bucket with a small
