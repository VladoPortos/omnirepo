/**
 * Playwright coverage for creating a Git mirror repo via CreateRepoDialog.
 * Drives the widened MirrorConfigSection (HTTPS+PAT only; no filter widget;
 * passive LFS warning) end-to-end.
 *
 * Scenarios:
 ...
 *
 * NOT covered here (covered by backend tests):
...
test.describe('Create repo: Git mirror', () => {
  ...
  test('Git mirror happy path: LFS warning + no filter widget + POST body shape', async ({
    ...
    // MirrorConfigSection now visible for type='git'
    ...
    // Passive LFS warning is visible under the URL
    ...
    // No Filters widget for git — assert the Filters
    ...
    // mirror_filter is OMITTED entirely for git mirrors:
    // git has no per-ref filter API, so sending an empty
  ...
  test('Git mirror cred picker filters to kind=basic only', async ({
    page,
    request,
  }) => {
