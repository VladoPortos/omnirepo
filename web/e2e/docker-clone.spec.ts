/**
 * Phase 8 / plan 08-03 — Playwright coverage for the Docker clone modal.
 *
 ...
 * wall.
 *
 * Endpoint delta from plan 08-03's sketch: the hook polls
 * `/projects/{name}/repos/{type}/{repo}/sync-jobs/{id}` (per-repo scope)
 * rather than the plan's `/api/v1/jobs/{id}` (no such endpoint exists).
...
test.describe('Docker clone modal (Phase 8 / plan 08-03)', () => {
