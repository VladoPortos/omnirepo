/**
 * v1.5 Phase 6 / plan 06-08 — Playwright smoke for the drift_purge
 * checkbox in MirrorConfigSection (DRIFTPURGE-04, D-15/D-16).
 *
 * Covers:
 *   1. Maintainer toggles drift_purge on a mirror repo, saves, reloads,
 *      and the checkbox state persists.
 *   2. Viewer sees the checkbox visible but DISABLED (D-15 maintainer
 *      gate via useRoleFor).
 */
...
test.describe('drift_purge toggle (Phase 6 / plan 06-08)', () => {
  ...
  test('viewer sees drift_purge disabled (D-15)', async ({ page, request }) => {
    const ts = Date.now();
    const { project, repo } = await seedPypiMirrorRepo(
      request,
      `drift-v-${ts}`,
