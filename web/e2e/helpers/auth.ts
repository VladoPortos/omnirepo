 ...
 *
 * Earlier, every spec carried its own inline `loginAsAdmin` /
 * `changePasswordIfNeeded` helpers hardcoding `password: 'changeme'`.
 * The actual seeded password — set by `global-setup.ts` via the
 * first-run `/api/v1/setup/superadmin` endpoint — is `AdminTest1!` with
 * `must_change_password=0`. Every spec failed on the first login attempt
 * before ever reaching the now-defunct change-password dance.
 *
 ...
 *
 * Intentionally stricter than the earlier inline helpers:
 ...
 *   });
 *
 * Specs that need a non-admin actor (viewer tests, etc.) do:
...
export async function seedUserWithProjectRole(
  ...
  // project member, masking real failures.
 ...
 * discarded after the session cookie is set.
 */
