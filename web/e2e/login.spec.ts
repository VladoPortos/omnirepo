 ...
 *
 * Post-v1.4 (F-15.4): global-setup seeds the super-admin with
 * `must_change_password=0`, so the forced-change path is exercised via
 * a second user provisioned through `POST /admin/users` rather than
 * the admin itself. See web/e2e/helpers/auth.ts for the helper.
 */
