 ...
 *
 * The strict-mode logic now treats a PathUnescape
 * failure as a fall-through to the NUL-byte check rather than producing a
 ...
 *    encoded form, and fetch the href.
 * 4. Assert bytes round-trip exactly.
 *
 * Negative coverage (traversal regression guard):
...
test.describe('raw upload + download with literal % in path', () => {
  ...
  test('residual %2e%2e traversal still rejected on PUT (regression guard)', async ({
    request,
  }) => {
    ...
    // `%2e%2e` (chi preserves %2e/%2E literally — that's the
    // routing-layer protection). The strict block must then re-decode
    // and reject `..`. Asserts the fix did NOT weaken traversal protection.
    const url = `/${encodeURIComponent(project)}/raw/${encodeURIComponent(
      repo,
