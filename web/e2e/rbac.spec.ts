/**
 * RBAC — maintainer/viewer split (RBAC-01..07)
 *
 * Phase 2 Plan 07: end-to-end coverage for the maintainer/viewer RBAC
 * matrix. Exercises the full D-39 scenario set:
 *
 ...
 *
 * Auth pattern: Phase 1 canonical beforeEach (D-38).
 * D-09 / D-34: self-demote Dialog uses first-person copy verified verbatim.
 */
...
test.describe('RBAC — maintainer/viewer split (RBAC-01..07)', () => {
  ...
  test('demote-to-viewer — Select viewer opens Dialog with Confirm', async ({
    ...
    // Body copy verbatim (UI-SPEC Copywriting Contract).
    await expect(
      page.getByText(
  ...
  test('self-demote-allowed-when-other-maintainer — Dialog self-copy + Confirm works', async ({
    ...
    // D-09 + D-34: first-person self-demote copy (UI-SPEC Copywriting Contract).
    await expect(
