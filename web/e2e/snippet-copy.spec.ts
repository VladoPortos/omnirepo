/**
 * CopyButton aria-live + clipboard contract.
 *
 ...
 *
 * Scope boundary: this spec covers ONLY the aria-live + clipboard wire
 * contract. Per-protocol snippet correctness (emitted-string shape) is
 * covered by web/src/lib/__tests__/snippets.test.ts (vitest). Full
 * per-protocol UI coverage is deferred to a later audit spec.
 */
...
test.describe('SnippetPanel copy-to-clipboard', () => {
  ...
  test('copies first snippet and announces "Copied to clipboard" via aria-live', async ({
    ...
    // aria-label "Copy Login" per SnippetList wiring.
    const copyBtn = page
