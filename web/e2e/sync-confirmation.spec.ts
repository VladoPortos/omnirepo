/**
 * Playwright regression for the "Sync complete" success pill rendered
 * by SyncNowButton.
 *
 * Covers:
 *   A. Pill appears when progress.status === 'done'.
 *   B. Pill auto-dismisses after exactly 8s.
 *   C. Re-click clears the pill immediately and the progress block
 *      reappears (progress XOR confirmation).
 *   D. axe-core WCAG AA clean while the pill is visible.
 *   E. Helm fallback: total_bytes == 0 + current_step="chart 5 of 5"
 *      surfaces the step verbatim instead of "0 MB".
 *
  ...
  return [
    {
      ...
      // Running jobs always serialise files_synced as 0. The full count
      // lands in the terminal tick.
      files_synced: 0,
      created_at: now,
      updated_at: now,
    },
...
test.describe('Sync complete pill', () => {
  ...
  test('A+B+D: pill appears on done, is a11y-clean, auto-dismisses at 8s', async ({
    ...
    await installMocks(page, project, repo, {
      ...
      // Full literal shape now that backend surfaces files_synced at
      // completion.
      files_synced: 42,
    });
    ...
    // A) Pill renders after the job flips to done, with the full
    ...
    // Those are owned by the design system / library layer, not by
    // this feature. We scope to "pill is axe-core clean" — not "the
    // whole page" — so the audit include filter matches.
  ...
  test('E: Helm fallback — total_bytes==0, currentStep "chart 5 of 5" surfaced verbatim', async ({
    ...
    // the mocked JobDetail payload, not from the repo type. The shape
    // we're asserting is `Sync complete · chart 5 of 5`.
    await installMocks(page, project, repo, {
      status: 'done',
      progress_bytes: 0,
      total_bytes: 0,
