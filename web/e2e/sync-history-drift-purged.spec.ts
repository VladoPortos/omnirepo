/**
 * SyncHistoryDialog "Drift purged: N" sub-line.
 *
 * The dialog calls
 ...
 * N==0 (run-evidence) is suppressed so non-drift mirror jobs stay
 * visually clean.
 *
...
test.describe('SyncHistoryDialog drift_purged line', () => {
  ...
  test('renders "Drift purged: N" for jobs with summary.drift_purged > 0; suppresses for 0/absent', async ({
    ...
    // suppressed — the absence side), and absent summary key (also
    // suppressed — drift never ran for this job).
    const mockBody = {
      items: [
        {
          id: 103,
