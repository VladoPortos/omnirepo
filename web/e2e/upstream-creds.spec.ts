/**
 * Playwright coverage for the ProjectSettingsPage
 * Upstream credentials tab.
 *
...
test.describe('Upstream credentials tab', () => {
  ...
  test('create cred happy path: empty -> dialog -> row; secret never rendered', async ({
    ...
    // The password we typed is NEVER rendered back to DOM.
  ...
  test('edit preserves password when blank: PATCH body omits password/token keys', async ({
    ...
    // Captured PATCH body does NOT carry `password` or `token`.
  ...
  // Regression: the CRED_KINDS dropdown previously had 6 entries.
  // It must be exactly 5 with no "deb alias" row.
  ...
  test('secrets never disclosed: full DOM scan; secret123 never appears', async ({
    ...
    // DEFENSIVE: secret123 is never anywhere in the rendered DOM.
    await expect(page.getByText(SECRET)).not.toBeVisible();
