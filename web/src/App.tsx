/**
 * App root with React Router 7 data router.
...
// Dev-only error-class story page.
...
// entire import is tree-shaken from the production bundle. The Playwright
// CI build opts in explicitly via VITE_OMNIREPO_DEV=true; regular
...
// Primitives story page. Same tree-shake gate as ErrorClassStoryPage
// above — dev builds only.
...
// StatusBadge matrix story page. Same tree-shake gate — the Playwright
// suite snapshots the 24 badge permutations exposed by this page.
...
// can restore the deep-link after a successful sign-in. State
...
// Super-admin guard. AuthGuard already ensures isAuthenticated, so
...
export const router = createBrowserRouter([
  ...
  },
  {
    ...
    children: [
      ...
      // Project settings page (currently only the Upstream credentials
      // tab). More specific path than the detail page below, so React
      // Router 7 picks this first for /projects/{name}/settings.
      {
      ...
      // Repo settings page (Mirror config card plus generic settings).
      // More specific suffix than the RepoDetailRouter match below, so
      ...
      {
        path: 'admin',
        // Every /admin/* route is super-admin-only. The backend
      ...
      // Nested catch-all keeps NotFoundPage inside AppShell (sidebar,
      ...
      // id="not-found" lets Breadcrumbs detect the catch-all and stop
