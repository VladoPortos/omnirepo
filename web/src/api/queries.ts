 ...
 * Query keys structured as ['entity', ...params].
 */
 ...
 * internal/protocol/oci/promote.go; this hook closes the gap where the UI
 ...
 * Session-authed REST shim for OCI v2 tag deletion. The OCI
 ...
 * protocol families. The protocol DELETE
...
// useRepoContentLoadMore exposes append-forward pagination over the
// paginated repo-content endpoint via useInfiniteQuery — fits the
// /load-more pattern used by the per-row-scan feature (commit 8ffe66c).
// Call .loadMore() to fetch and concatenate the next window; .hasMore
// tracks whether the backend is still returning a next_offset. The
// target repo is part of the query key, so navigating between repos
// never leaks rows across.
export function useRepoContentLoadMore(
  ...
  // Keyed under the same ['repo-content', project, type, name] prefix as
  // useRepoContent so existing invalidations (uploads, deletes, rescans,
  // clone) refetch the loaded pages too.
  const q = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      fetchRepoContentPage(projectName, repoType, repoName, {
        limit: pageSize,
        offset: pageParam,
      }),
    initialPageParam: 0,
    ...
    staleTime: 15_000,
    // Mirror useRepoContent's scan polling: while any loaded row is
    // mid-scan, poll so the severity badge updates as scans finish.
    refetchInterval: (query) => {
      const pages = query.state.data?.pages;
      if (!pages) return false;
      const scanning = pages.some((p) =>
        p.items.some((r) => r.scan_severity === 'scanning'),
      );
      return scanning ? 3_000 : false;
    },
  });

  const items = useMemo<RepoContentEntry[]>(
  ...
  return {
    ...
    // Before the first page lands hasNextPage is false; report hasMore as
    // true while loading so consumers keep their "more may come" state,
    // matching the previous accumulator implementation.
    hasMore: q.data ? hasNextPage : true,
    loadMore,
 ...
 * shape. Timestamps are RFC3339 strings or null when no matching
 * sync_jobs row exists.
 */
...
/**
 * useAdminJobsSummary — TanStack hook for the Background Jobs card.
 *
 ...
 * ActionTriggerGC). staleTime 60s — counts don't need to update every
 * request.
 */
...
/**
 * useAdminTLSCurrent — TanStack hook for the TLS Cert Expiry card.
 *
 * Pass `enabled = !!currentUser?.is_super_admin` so non-admins never
 ...
 *     detected but has no version metadata. The card maps
...
/**
 * useAdminTrivyDBStatus — TanStack hook for the Trivy DB Freshness
...
/**
 * DBHealth mirrors the GET /api/v1/admin/db/health response shape
 * (internal/api/admin_db_health.go:handleDBHealth). The full shape is
 * quoted in-comment below each field so future refactors spot drift
 * quickly.
 *
 * Key invariants (enforced by regression tests):
 *   - `wal.warn_over_bytes` is server-sourced (currently 100 MB) — the
 *     frontend never hardcodes it (threshold locked on the server).
 *   - `can_run_now` / `next_available_at` are server-computed — the
 *     frontend never does client-side time math beyond a one-shot
 *     render-time minutes computation for the tooltip (static-on-render
 *     invariant).
 *   - `running=true` triggers 5s polling via refetchInterval below;
 *     polling stops the moment the server reports running=false.
...
export type DBHealth = {
  integrity: {
     ...
     *  "boot_*_failed: <err>" from the boot path; "" before first
     *  run; "panicked" from the panic-recovery path. */
    status: string;
  ...
  wal: {
     ...
     *  100 MB. Currently 100 MB (104857600). */
...
/**
 * useAdminDBHealth — TanStack hook for the SQLite Health card. Pass
 * `enabled = !!currentUser?.is_super_admin` so non-admins never issue a
 * request (server gate = RequireCan(ActionTriggerGC)).
 *
 * Polling model (fire-and-forget with 5s refetch window):
...
export function useAdminDBHealth(enabled: boolean) {
  return useQuery<DBHealth>({
    ...
    refetchInterval: (query) => {
      ...
      if (!d) return false;
      // 5 s default polling while lease is held (any 5-30 s is
      // acceptable).
 ...
 * (manual integrity_check trigger). Returns `{ job_started_at }`
...
// -- Project Members --
//
// The ProjectDetailPage picker needs the full user list so the operator
// can pick a non-member to add. The admin-users hook in UsersPage.tsx is
// local, so we expose a thin shared query here.
...
export function useAdminUserList({ enabled = true }: { enabled?: boolean } = {}) {
  return useQuery({
    ...
    // Chase next_cursor so the project-member picker isn't capped at the
    // API's default page size. The server enforces MaxPageLimit=200 per
    // request; we request the max and loop so deployments with >200 users
    // still see every account.
    queryFn: async () => {
...
export function useMaintenance() {
  // The public status endpoint (/maintenance/status) returns only the
...
/**
 * useSyncRepo — POST /sync on a mirror repo with an empty body enqueues
 * a sync job that reads its config from the repo row
 * (mirror_upstream_url, mirror_filter_json, mirror_cred_id,
 * scan_on_sync) — see internal/httpx/sync_rest.go's 3-way branch. The
 * 409 mirror.sync.in_flight envelope surfaces via ErrorEnvelopeRenderer
 ...
 */
// SyncRepoVariables is the optional body shape accepted by the
// /sync endpoint for mirror repos. Empty/undefined sends a body-less
// POST (reads config from the repo row). When `forceDriftThreshold` is
// true the body is `{"force_drift_threshold":true}`, the operator-confirm
// flag that bypasses the percent-threshold drift-purge guard for the
// next sync.
...
export function useSyncRepo(
  ...
  return useMutation<SyncEnqueueResponse, Error, SyncRepoVariables | void>({
    mutationFn: (vars) => {
      ...
      // narrow exception `{"force_drift_threshold":true}` which the
      // backend accepts as a per-call confirmation flag.
      const body =
 ...
 *
 * Consumers: CloneImageDialog credential picker and the
...
/**
 * useCreateUpstreamCred — POST /projects/{name}/upstream-creds. On
 * success invalidates the list cache so both the CloneImageDialog and
 * MirrorConfigSection cred pickers, plus the UpstreamCredsTab table, all
 * refetch.
 *
 ...
 *
 * Blank-preserves-existing contract: the caller MUST omit
 ...
 * the next sync on those repos fails with a
