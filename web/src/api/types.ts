...
// Paginated content response. Total is the row count without
...
export interface User {
  ...
  // and this row is soft-deleted. Absent on live rows so existing callers
  // stay unaffected.
  deleted_at?: string;
}
...
export interface MeResponse {
   ...
   * and by older servers (hence optional). Empty map means the caller is
...
export interface ProjectMember {
  ...
  login: string;
  email: string;
  /** Role of this member in the project. Required — the server always sends it. */
...
export interface MirrorConfigValue {
  ...
  mirror_cred_id: number | null;
  scan_on_sync: boolean;
  // Opt-in: when true, a successful mirror sync soft-deletes local rows
  // whose upstream key vanished. Default false on upgrade. Mirror-only —
  // server rejects drift_purge=true on non-mirror repos with envelope code
...
export interface RepoCreate {
  ...
  // Mirror creation fields. The backend
  ...
  mirror_cred_id?: number | null;
  scan_on_sync?: boolean;
  // Optional create-time opt-in (default false).
  drift_purge?: boolean;
}
...
export interface RepoPatch {
  ...
  // Mirror-repo editable fields. is_mirror and
  // mirror_upstream_url are NOT in this shape: the backend rejects them
  // with 400 repo.mirror_url_immutable. Only filter, cred, and
  // scan_on_sync may change post-creation.
  mirror_filter?: AnyFilter;
  mirror_cred_id?: number | null;
  scan_on_sync?: boolean;
  // drift_purge may change post-creation.
  drift_purge?: boolean;
}
 ...
 * /api/v1/projects/{name}/repos/{type}/{repo}/sync-jobs/{id}
 * (`internal/api/repos_list.go:syncJobItem`). progress_bytes,
...
export interface JobDetail {
  ...
  current_step: string;
  /**
   * Count of files newly added during the sync. Written once at sync
   * completion by each protocol handler (not via the throttled progress
   * path) so it's 0 for running jobs. The success pill uses this to render
   * "Sync complete · N files · X MB". Backend COALESCEs missing rows to 0.
   */
  files_synced: number;
  /**
   ...
   *     run completes. Absent for non-mirror or non-drift_purge syncs.
   *   - drift_blocked: integer count, written when the percent-
   *     threshold guard tripped. Operator must override.
 ...
 * `internal/protocol/oci/pull_external.go:PullExternalRequest`. The
 * backend uses `src_image` + `dst_tag` (NOT `src` + `retag_as`). A
 * `scan_override` field is NOT accepted by the backend endpoint —
 ...
 * credential picker AND by the Upstream credentials tab on
 * ProjectSettingsPage.
 *
 * CRITICAL SECURITY PROPERTY: password and token fields
 ...
 * alias was removed. Clients that still submit
 ...
 * CONTRACT: password and token fields, when OMITTED from
...
// Backend returns nullable fields (actor/ip/outcome/etc.) with
