...
// Protocols that support the mirror flag.
// Docker uses a different clone model (per-click modal, see
// CloneImageDialog); Raw has no upstream-index protocol to follow.
// The set includes 'git' (HTTPS+PAT mirror via go-git/v6
// PlainCloneContext + FetchContext, all-refs, see
...
export function CreateRepoDialog({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (showMirrorSection && mirrorCfg.is_mirror) {
      ...
      // Helm mirrors also accept oci:// for OCI-only upstreams
      // (e.g. registry-1.docker.io/bitnamicharts/*). Other repo
    ...
    if (showMirrorSection && mirrorCfg.is_mirror) {
      ...
      // Git mirrors are all-refs, no filter support. Omit
