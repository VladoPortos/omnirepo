/**
 * SyncHistoryDialog — lists recent sync jobs for a mirror repo,
...
// tripped). Malformed JSON also returns null — the dialog still renders
// the row, just without the drift line.
//
// The SyncHistoryDialog surfaces this count as a dedicated per-job line
// so operators can see drift activity at a glance without
// cross-referencing audit events.
...
// drift_blocked count if present and > 0. Surfaces the percent-threshold
// guard's "this sync would have purged N rows; operator must confirm"
// event, written by each protocol handler via
// SyncJobsRepo.SetSummaryDriftBlocked.
...
export function SyncHistoryDialog({
  ...
  // The percent-threshold guard surfaces only on the LATEST job
  // (jobs[0] — backend orders DESC by id). Older blocked jobs are
  // historical; surfacing them all would clutter the dialog and
  // re-running the override would only act on the current state of
  // the mirror anyway.
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      {/*
        Confirm dialog. Mounted as a sibling Dialog so its open state is
        independent of the history dialog. The confirm copy spells out
        the irreversibility (purged rows go to trash but restoring
        requires a re-sync).
      */}
