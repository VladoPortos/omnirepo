/**
 * ErrorEnvelopeRenderer — renders an ApiErrorEnvelope per the Phase 6
 * UI-SPEC class-by-class contract (ERR-02, ERR-04, ERR-05, ERR-07).
 *
 ...
 * XSS surface declared in threat T-06-03-02.
 */
...
interface ClassStyle {
  icon: LucideIcon;
  /** bg + text + border utility classes referencing the phase-6 status tokens. */
  colorClass: string;
...
/**
 * classStyles maps ApiErrorClass → visual treatment. Uses the NEW
 * status-token utility classes declared by UI-SPEC §Color "NEW status
 * token set". Tailwind 4 does not fail compile on unknown utilities —
 * it simply no-ops until the tokens are declared (plan 06-06 installs
 * the status tokens into web/src/index.css).
 */
...
function OperatorDeepLinkButton({
  ...
  return (
    <Button
      ...
      size="sm"
      onClick={() => {
        // Same-origin SPA navigation only — UI-SPEC ERR-05 + threat
        // T-06-03-03: operator_route is a path, not a URL, and the
        // air-gap invariant prevents any external URL here from being
        // functional even if it were injected.
