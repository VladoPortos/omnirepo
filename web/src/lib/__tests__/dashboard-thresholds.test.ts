/**
 * Tests for dashboard-thresholds.
 *
 * Covers the threshold defaults and override handling for every pure
 * threshold function. Boundary coverage (just-below / equal /
 * just-above) for each numeric input across the status-token →
 * threshold mapping.
 */
...
describe('dbHealthVariant', () => {
  ...
  // from the response payload shape. Kept local to the test so
  // call-site intent stays self-documenting.
  ...
  it("status='', wal=0 → healthy (never-run, no WAL bloat — rejects 'unknown' variant)", () => {
    expect(dbHealthVariant('', 0, ONE_HUNDRED_MB)).toBe('healthy');
  });

  it("status='unknown', wal=0 → healthy (boot-failure-preread — rejects 'unknown' variant)", () => {
