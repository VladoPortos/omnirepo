 ...
 * to StatusVariant.
 *
 * Consumed by the Composition row cards on DashboardPage
 ...
 *     intentionally maps ONLY onto healthy/warning/failure; inventing
 *     'disabled'/'maintenance' returns here would expand the
 *     StatusBadge variant enum.
 *   - Six individual override objects (one per function) were chosen
 *     over a single aggregated `DashboardThresholds` blob — simpler to
 *     test, simpler for admins to edit, and typed per-function so you
 *     can't accidentally pass Trivy overrides to storageVariant.
 *   - Boundary semantics follow the threshold table verbatim: warn
...
/**
 * storageVariant — maps used/total bytes to a StatusVariant.
 *
 ...
 * in the last 24h) to a StatusVariant.
 *
 *   count == 0               → healthy
...
/**
 * jobsVariant — maps the admin/jobs/summary response shape to
 * ONE of ONLY {healthy, warning, failure}. The Jobs card is limited to
 * those three variants; introducing 'disabled' or 'maintenance'
 * returns here would expand the StatusBadge variant enum. A future
 * change that needs a fourth semantic state (e.g. 'idle-never-run')
 * must extend the variant set first.
 *
 * Decision table:
 ...
 * StatusVariant. Self-signed (no uploaded cert) is disabled
 ...
 * StatusVariant. An uninitialised Trivy DB is disabled because
...
// -----------------------------------------------------------------------------
// SQLite Health (C-7, admin-only)
// -----------------------------------------------------------------------------

/**
 * dbHealthVariant — maps the GET /api/v1/admin/db/health payload's
 * integrity.status + wal.bytes onto a 3-state StatusVariant.
 *
 * Precedence rules:
 ...
 *
 * Notes on '' / 'unknown':
 *   There is intentionally no 'unknown' variant. When the
 ...
 * 100 MB, keeping the no-hardcoded-threshold invariant.
 *
 * Returns a subset of StatusVariant ('healthy' | 'warning' | 'failure')
 * matching the 3-state locked-down set.
 */
