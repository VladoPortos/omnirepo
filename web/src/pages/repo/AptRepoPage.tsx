/**
 * APT/Debian repository detail page.
  ...
  return (
    <RepoPageLayout repo={repo}>
      <div className="space-y-4">
        {/* Mirror Sync Now affordance — visible only on is_mirror=true repos,
            reads upstream config from the repo row. */}
        {repo.is_mirror && (
          <SyncNowButton
        ...
        {packages.length === 0 ? (
          canUpload ? (
            <EmptyState
              ...
              // The mirror empty state must not claim "Upload your first
              // artifact using the snippet below" because uploads to mirrors
              // return 403 repo_is_mirror. The signing-key + apt-source
              // snippets are read-only; retarget the description so the two
              // halves agree.
              description={
                repo.is_mirror
