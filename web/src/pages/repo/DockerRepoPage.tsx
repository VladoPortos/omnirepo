...
export function DockerRepoPage({ repo }: DockerRepoPageProps) {
  ...
  // Promote dialog form state. Split `dst_project/dst_repo` from one
  // combined input so the placeholder "project/repo" can keep its familiar
  // shape while validation and the submit payload stay typed separately.
  ...
  // Delete-tag confirm state. Stores the tag pending deletion so we can
  // surface it in the AlertDialog and in toast outcomes.
  ...
  // /content now returns one row per (image, tag). Map into the DockerTag
  // shape the table renders. Scan status derives from the unified
  // scan_severity: '' = never scanned, 'scanning'/'failed' carry their own
  // status, everything else is a done scan.
  ...
  return (
    <RepoPageLayout repo={repo}>
      <div className="space-y-4">
        ...
        {/* Tag table — empty state when no artifacts yet */}
        {tags.length === 0 ? (
          canUpload ? (
            <EmptyState
              icon={Terminal}
          ...
          <DataTable
            ...
            stickyFirstColumn
            // Render the tag detail panel inline as an accordion row
            // immediately under the clicked row rather than letting it float
            // at the bottom of a 3-viewport-long list.
            isRowExpanded={(row) =>
      ...
      {/* Pull External dialog — CloneImageDialog replaces the previous
          stub block. */}
      <CloneImageDialog
        open={cloneOpen}
