...
export function GitRepoPage({ repo }) {
    ...
    return (_jsx(RepoPageLayout, { repo: repo, children: _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3", children: [_jsx(RefSelector, { refs: refs, currentRef: currentRef, onRefChange: handleRefChange, loading: refsLoading }), _jsxs("div", { className: "flex items-center gap-2 rounded-md border bg-muted/30 px-3 py-1.5", children: [_jsx(GitBranch, { className: "size-4 text-muted-foreground" }), _jsx("code", { className: "text-xs", children: cloneUrl }), _jsx(CopyButton, { text: cloneUrl })] })] }), _jsxs(Tabs, { defaultValue: "files", value: tab, onValueChange: (v) => { setTab(v); setViewingCommit(null); }, children: [_jsxs(TabsList, { children: [_jsx(TabsTrigger, { value: "files", children: "Files" }), _jsx(TabsTrigger, { value: "commits", children: "Commits" }), _jsx(TabsTrigger, { value: "refs", children: "Refs" }), _jsx(TabsTrigger, { value: "compare", children: "Compare" })] }), _jsx(TabsContent, { value: "files", children: viewingFile ? (showBlame ? (_jsx(BlameViewer, { projectName: projectName, repoName: repo.name, currentRef: currentRef, filePath: viewingFile, onBack: handleBack })) : (_jsx(FileViewer, { file: fileData, loading: fileLoading, onBack: handleBack, onBlame: () => setShowBlame(true), downloadUrl: `/api/v1/projects/${projectName}/repos/${repo.name}/git/blob/${currentRef}/${viewingFile}?raw=1` }))) : (_jsx(FileTree, { entries: treeEntries, loading: treeLoading, currentPath: currentPath, onNavigate: handleNavigate, onBack: currentPath ? handleBack : undefined })) }), _jsx(TabsContent, { value: "commits", children: viewingCommit ? (_jsx(CommitDetail, { projectName: projectName, repoName: repo.name, sha: viewingCommit, onBack: () => setViewingCommit(null) })) : (_jsx(CommitLog, { projectName: projectName, repoName: repo.name, currentRef: currentRef, onCommitClick: setViewingCommit })) }), _jsx(TabsContent, { value: "refs", children: _jsxs("div", { className: "space-y-4 py-4", children: [refs.filter((r) => r.type === 'branch').length > 0 && (_jsxs("div", { children: [_jsx("h3", { className: "mb-2 text-sm font-semibold", children: "Branches" }), _jsx("div", { className: "space-y-1", children: refs
                                                    .filter((r) => r.type === 'branch')
                                                    .map((r) => (_jsxs("div", { className: "flex items-center justify-between rounded-md border px-3 py-2", children: [_jsx(Button, { variant: "link", size: "sm", className: "h-auto p-0", onClick: () => { handleRefChange(r.name); setTab('files'); }, children: r.name }), _jsx("code", { className: "text-xs text-muted-foreground", children: r.sha.slice(0, 8) })] }, r.name))) })] })), refs.filter((r) => r.type === 'tag').length > 0 && (_jsxs("div", { children: [_jsx("h3", { className: "mb-2 text-sm font-semibold", children: "Tags" }), _jsx("div", { className: "space-y-1", children: refs
                                                    .filter((r) => r.type === 'tag')
                                                    .map((r) => (_jsxs("div", { className: "flex items-center justify-between rounded-md border px-3 py-2", children: [_jsx(Button, { variant: "link", size: "sm", className: "h-auto p-0", onClick: () => { handleRefChange(r.name); setTab('files'); }, children: r.name }), _jsx("code", { className: "text-xs text-muted-foreground", children: r.sha.slice(0, 8) })] }, r.name))) })] })), refs.length === 0 && (_jsx("p", { className: "text-center text-sm text-muted-foreground", children: "No refs found. Push some code to get started." }))] }) }), _jsx(TabsContent, { value: "compare", children: _jsx(BranchCompare, { projectName: projectName, repoName: repo.name, refs: refs, defaultBase: refs.find((r) => r.type === 'branch' && r.name === 'main')?.name, defaultHead: currentRef !== 'main' ? currentRef : '' }) })] })] }) }));
}
