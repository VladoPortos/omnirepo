...
export function GitRepoPage({ repo }: GitRepoPageProps) {
  ...
  // Empty state for empty git repos: no refs = no commits pushed yet.
  // When zero refs, show an EmptyState with the git push snippet inline
  // instead of the empty Files/Commits/Refs tabs dance.
  //
  // Mirror repos get a different empty state — "Mirror is empty" +
  // SyncNowButton — because the push snippet is misleading (mirror repos
  // refuse receive-pack with 403 mirror.push_rejected). Refs appear after
          ...
          {repo.is_mirror ? (
            <div className="space-y-4">
              <SyncNowButton
                ...
                // No filter widget for git mirrors (all-refs mode).
            ...
            >
              <SnippetList
                ...
                // Hide push instructions on mirror repos. This branch is
                // only reached when !repo.is_mirror, but passing the prop
                // explicitly documents the contract (SnippetList is a
                // reusable primitive and other callers may pipe a
                // mirror-state value through).
  ...
  return (
    <RepoPageLayout repo={repo}>
      <div className="space-y-4">
        {/* Mirror Sync Now affordance, parallel to HelmRepoPage /
            AptRepoPage / RpmRepoPage / PypiRepoPage. Renders only when
            is_mirror is true so dev repos stay unchanged. */}
        {repo.is_mirror && (
          <SyncNowButton
            projectName={projectName ?? ''}
            repoType="git"
            repoName={repo.name}
            upstreamUrl={repo.mirror_upstream_url}
            // No filter UI for git mirrors (all-refs mirror mode).
            filterSummary={undefined}
          />
        )}
